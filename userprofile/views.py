...
def UserProfile(request):
    if "useremail" in request.session:
        ...
        context = {"users": user, "address": address}
    else:
...
def DefaultAddress(request):
    if request.method == "POST":
        ...
        address_id = request.POST.get(
            "default_address"
        )  # getting the address selected by the user
        ...
        try:
            ...
            )  # Raise Http404 to indicate a not found error
...
def OrderDetails(request, order_id):
    ...
    status = order.status
    context = {"order_items": order_items, "order": order, "status": status}
...
def OrderCancellation(request, order_id):
    ...
    order_items = OrderItem.objects.filter(order=order)
    print(
        "This all are my order items for a undestandngs-----------------", order_items
    )
    print(
        "This all are my order items for a undestandngs-----------------", order_items
    )
