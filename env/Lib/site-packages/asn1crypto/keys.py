...
class ECPrivateKey(Sequence):
    ...
    def __setitem__(self, key, value):
        ...
        if key == "private_key":
            ...
            key == "parameters"
            and isinstance(self["parameters"], ECDomainParameters)
            and self["parameters"].name != "implicit_ca"
        ):
...
class PrivateKeyInfo(Sequence):
    ...
    def wrap(cls, private_key, algorithm):
        ...
        if not isinstance(private_key, byte_cls) and not isinstance(
            private_key, Asn1Value
        ):
            raise TypeError(
                unwrap(
                    """
    ...
    def __setitem__(self, key, value):
        ...
        if (
            (key == "private_key_algorithm" or key == "private_key")
            and algorithm["algorithm"].native == "ec"
            and isinstance(algorithm["parameters"], ECDomainParameters)
            and algorithm["parameters"].name != "implicit_ca"
            and isinstance(self["private_key"], ParsableOctetString)
            and isinstance(self["private_key"].parsed, ECPrivateKey)
        ):
    ...
    def hash_algo(self):
        ...
        byte_len = (
            math.log(self["private_key_algorithm"]["parameters"]["q"].native, 2) / 8
        )
...
class PublicKeyInfo(Sequence):
    ...
    def wrap(cls, public_key, algorithm):
        ...
        if not isinstance(public_key, byte_cls) and not isinstance(
            public_key, Asn1Value
        ):
            raise TypeError(
                unwrap(
                    """
