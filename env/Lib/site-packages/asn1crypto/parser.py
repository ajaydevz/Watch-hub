...
def _parse(encoded_data, data_len, pointer=0, lengths_only=False, depth=0):
    ...
    if length_octet >> 7 == 0:
        ...
        if length_octets:
            if data_len < pointer + length_octets:
                raise ValueError(
                    _INSUFFICIENT_DATA_MESSAGE % (length_octets, data_len - pointer)
                )
            pointer += length_octets
            contents_end = pointer + int_from_bytes(
                encoded_data[pointer - length_octets : pointer], signed=False
            )
            ...
            while (
                data_len < contents_end + 2
                or encoded_data[contents_end : contents_end + 2] != b"\x00\x00"
            ):
    ...
    if contents_end > data_len:
        raise ValueError(
            _INSUFFICIENT_DATA_MESSAGE % (contents_end - pointer, data_len - pointer)
        )
