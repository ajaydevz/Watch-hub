...
class Name(Choice):
    ...
    def build(cls, name_dict, use_printable=False):
        ...
        name_dict = OrderedDict(
            sorted(
                name_dict.items(), key=lambda item: NameType.preferred_ordinal(item[0])
            )
        )
        ...
        for attribute_name, attribute_value in name_dict.items():
            ...
            elif attribute_name in set(
                ["dn_qualifier", "country_name", "serial_number"]
            ):
                value = DirectoryString(
                    name="printable_string", value=PrintableString(attribute_value)
                )
            else:
                value = DirectoryString(
                    name=encoding_name, value=encoding_class(attribute_value)
                )

            rdns.append(
                RelativeDistinguishedName(
                    [NameTypeAndValue({"type": attribute_name, "value": value})]
                )
            )
...
class EntrustVersionInfo(Sequence):
    _fields = [("entrust_vers", GeneralString), ("entrust_info_flags", BitString)]
...
class Certificate(Sequence):
    ...
    def issuer_serial(self):
        ...
        if self._issuer_serial is None:
            self._issuer_serial = (
                self.issuer.sha256 + b":" + str_cls(self.serial_number).encode("ascii")
            )
    ...
    def authority_issuer_serial(self):
        ...
        if self._authority_issuer_serial is False:
            ...
            if akiv and akiv["authority_cert_issuer"].native:
                issuer = self.authority_key_identifier_value["authority_cert_issuer"][
                    0
                ].chosen
                ...
                self._authority_issuer_serial = (
                    issuer.sha256 + b":" + str_cls(authority_serial).encode("ascii")
                )
            else:
    ...
    def delta_crl_distribution_points(self):
        ...
        if self._delta_crl_distribution_points is None:
            self._delta_crl_distribution_points = (
                self._get_http_crl_distribution_points(self.freshest_crl_value)
            )
    ...
    def ca(self):
        ...
        return (
            self.basic_constraints_value and self.basic_constraints_value["ca"].native
        )
    ...
    def is_valid_domain_ip(self, domain_ip):
        ...
        if is_domain:
            ...
            for valid_domain in self.valid_domains:
                encoded_valid_domain = (
                    valid_domain.encode("idna").decode("ascii").lower()
                )
        ...
        for valid_ip in self.valid_ips:
            valid_family = (
                socket.AF_INET if valid_ip.find(".") != -1 else socket.AF_INET6
            )
