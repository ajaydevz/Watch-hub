...
class Asn1Value(object):
    ...
    def load(cls, encoded_data, strict=False, **kwargs):
        ...
        if not isinstance(encoded_data, byte_cls):
            raise TypeError(
                "encoded_data must be a byte string, not %s" % type_name(encoded_data)
            )
    ...
    def __init__(
        ...
        try:
            ...
            args = e.args[1:]
            e.args = (
                e.args[0] + "\n    while constructing %s" % type_name(self),
            ) + args
            raise e
    ...
    def retag(self, tagging, tag=None):
        ...
        if not isinstance(tagging, dict):
            tagging = {tagging: tag}
        new_obj = self.__class__(
            explicit=tagging.get("explicit"), implicit=tagging.get("implicit")
        )
        new_obj._copy(self, copy.deepcopy)
        return new_obj
        ...
        if self._header is None or force:
            ...
            if self.explicit is not None:
                for class_, tag in self.explicit:
                    header = (
                        _dump_header(class_, 1, tag, header + self.contents) + header
                    )
    ...
    def dump(self, force=False):
        ...
        if self._header is None or force:
            ...
            if self.explicit is not None:
                for class_, tag in self.explicit:
                    self._header = (
                        _dump_header(class_, 1, tag, self._header + self._contents)
                        + self._header
                    )
...
class Primitive(Asn1Value):
    ...
    def __eq__(self, other):
        ...
        self_bases = (set(self.__class__.__bases__) | set([self.__class__])) - set(
            [Asn1Value, Primitive, ValueMap]
        )
        other_bases = (set(other.__class__.__bases__) | set([other.__class__])) - set(
            [Asn1Value, Primitive, ValueMap]
        )
...
class _IntegerBitString(object):
    ...
    def _as_chunk(self):
        ...
        unused_bits = _int_to_bit_tuple(
            value & ((1 << unused_bits_len) - 1), unused_bits_len
        )
...
class Sequence(Asn1Value):
                ...
                for key in keys:
                    ...
                    if check_existing:
                        ...
                        if (
                            ...
                            and self.children[index] is not VOID
                        ):
    ...
    def __setitem__(self, key, value):
        ...
        new_value = self._make_value(
            field_name, field_spec, value_spec, field_params, value
        )
    ...
    def _setup(self):
        ...
        if cls._oid_pair is not None:
            cls._oid_nums = (
                cls._field_map[cls._oid_pair[0]],
                cls._field_map[cls._oid_pair[1]],
            )

        for index, field in enumerate(cls._fields):
            has_callback = (
                cls._spec_callbacks is not None and field[0] in cls._spec_callbacks
            )
            ...
            while again:
                ...
                if field < field_len:
                    ...
                    if field_params and (
                        ...
                        if (
                            self._field_ids[field] != (parts[0], parts[2])
                            and field_spec != Any
                        ):
                    ...
                    if field_spec is None or (
                        spec_override and issubclass(field_spec, Any)
                    ):
                ...
                # Handle situations where an optional or defaulted field definition is incorrect
                    ...
                    while prev_field >= 0:
                        ...
                        if len(prev_field_info) < 3:
                            break
                        if (
                            "optional" in prev_field_info[2]
                            or "default" in prev_field_info[2]
                        ):
    ...
    def _parsed_time(self):
        ...
        if groups["zulu"]:
            ...
            tz = create_timezone(
                sign
                * timedelta(
                    hours=int(groups["dhour"]), minutes=int(groups["dminute"] or 0)
                )
            )
        ...
        if groups["fraction"]:
            ...
            fract = (
                Fraction(int(groups["fraction"]), 10 ** len(groups["fraction"]))
                * 1000000
            )
...
def _basic_debug(prefix, self):
    ...
    if self._header:
        print(
            "%s  Header: 0x%s"
            % (prefix, binascii.hexlify(self._header or b"").decode("utf-8"))
        )

    has_header = (
        self.method is not None and self.class_ is not None and self.tag is not None
    )
    if has_header:
    ...
    if self.explicit is not None:
        for class_, tag in self.explicit:
            print(
                ...
                % (prefix, CLASS_NUM_TO_NAME_MAP.get(class_), tag)
            )
        if has_header:
    ...
    if self._trailer:
        print(
            "%s  Trailer: 0x%s"
            % (prefix, binascii.hexlify(self._trailer or b"").decode("utf-8"))
        )

    print(
        "%s  Data: 0x%s"
        % (prefix, binascii.hexlify(self.contents or b"").decode("utf-8"))
    )
...
def _parse_build(encoded_data, pointer=0, spec=None, spec_params=None, strict=False):
    ...
    if strict and new_pointer != pointer + encoded_len:
        ...
        raise ValueError(
            "Extra data - %d bytes of trailing data were provided" % extra_bytes
        )
