...
def random_base32(
    length: int = 32, chars: Sequence[str] = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ234567")
) -> str:
...
def random_hex(
    length: int = 40, chars: Sequence[str] = list("ABCDEF0123456789")
) -> str:
    if length < 40:
        raise ValueError("Secrets should be at least 160 bits")
...
def parse_uri(uri: str) -> OTP:
    ...
    for key, value in parse_qsl(parsed_uri.query):
        if key == "secret":
            ...
            if (
                "issuer" in otp_data
                and otp_data["issuer"] is not None
                and otp_data["issuer"] != value
            ):
                raise ValueError(
