...
def validate_tls_hostname(
    ...
    bad_key_usage = (
        cert.key_usage_value and "digital_signature" not in cert.key_usage_value.native
    )
    bad_ext_key_usage = (
...
def _parse_iss_serial(iss_serial: cms.IssuerSerial, err_msg_prefix: str) -> bytes:
    """
...
def _check_ac_signature(
    ...
    digest_allowed = validation_context.algorithm_policy.signature_algorithm_allowed(
...
def check_ac_holder_match(holder_cert: x509.Certificate, holder: cms.Holder):
    ...
    if not isinstance(entity_name, core.Void):
        holder_dn = extract_dir_name(entity_name, "Could not identify AC holder DN")
    ...
    if not isinstance(obj_digest_info, core.Void):
        raise NotImplementedError("Object digest info is currently not supported")
    return mismatches
...
class _PathValidationState:
    ...
    def init_pkix_validation_state(
        ...
        if parameters is not None and trust_anchor_params is not None:
            ...
            initial_permitted_subtrees = PermittedSubtrees(
                parameters.initial_permitted_subtrees or default_permitted_subtrees()
            )
            ...
            initial_excluded_subtrees = ExcludedSubtrees(
                parameters.initial_excluded_subtrees or default_excluded_subtrees()
            )
            ...
            parameters = parameters or trust_anchor_params or PKIXValidationParams()
            ...
            initial_policy_mapping_inhibit = parameters.initial_policy_mapping_inhibit
            initial_permitted_subtrees = PermittedSubtrees(
                parameters.initial_permitted_subtrees or default_permitted_subtrees()
            )
            initial_excluded_subtrees = ExcludedSubtrees(
        ...
        state = _PathValidationState(
            ...
            inhibit_any_policy=(0 if initial_any_policy_inhibit else path_length + 1),
            policy_mapping=(0 if initial_policy_mapping_inhibit else path_length + 1),
            # Steps 1 g-j
    ...
    def update_policy_restrictions(self, cert: x509.Certificate):
        ...
        if policy_constraints:
            ...
            # Step 3 i 2
            inhibit_policy_mapping = policy_constraints["inhibit_policy_mapping"].native
            if inhibit_policy_mapping is not None:
                self.policy_mapping = min(self.policy_mapping, inhibit_policy_mapping)
...
async def intl_validate_path(
    ...
    completed_path: ValidationPath = ValidationPath(trust_anchor, interm=[], leaf=None)
    ...
    if isinstance(trust_anchor, CertTrustAnchor):
        ...
        validation_context.record_validation(trust_anchor.certificate, completed_path)
...
def _check_validity(validity: Validity, moment, tolerance, proc_state: ValProcState):
...
async def _check_revocation(
    ...
    rev_check_policy = validation_context.revinfo_policy.revocation_checking_policy
    rev_rule = (
    ...
    status_good = (
        ocsp_status_good and rev_rule != RevocationCheckingRule.CRL_AND_OCSP_REQUIRED
    )
    ...
    crl_required_by_policy = rev_rule.crl_mandatory or (
        not status_good and rev_rule == RevocationCheckingRule.CRL_OR_OCSP_REQUIRED
    )
    crl_fetchable = rev_rule.crl_relevant and cert_has_crl
    if crl_required_by_policy or (crl_fetchable and not status_good):
        try:
            await verify_crl(cert, path, validation_context, proc_state=proc_state)
            revocation_check_failed = False
    ...
    expected_revinfo = rev_rule.strict or (
        revinfo_declared and rev_rule == RevocationCheckingRule.CHECK_IF_DECLARED
    )
    ...
    if not soft_fail:
        if not status_good and matched and revocation_check_failed:
            ...
            maybe_stale_cutoff = ocsp_suspect_stale_since or crl_suspect_stale_since
