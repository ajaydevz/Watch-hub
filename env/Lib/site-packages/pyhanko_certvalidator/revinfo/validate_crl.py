...
async def _find_candidate_crl_issuer_certs(
    ...
    *,
    ...
    candidates = cert_registry.retrieve_by_name(delegated_issuer, cert_issuer_cert)
    ...
    if not candidates and cert_registry.fetcher is not None:
        ...
        # if there is one
        async for cert in cert_registry.fetcher.fetch_crl_issuers(certificate_list):
            # filter by name
    ...
    *,
    ...
    try:
        ...
        if not issuing_authority_identical:
            temp_override = proc_state.describe_cert(never_def=True) + " CRL issuer"
...
def _match_dps_idp_names(
    ...
    if crl_dps:
        ...
        has_dp_name = True
        general_name = x509.GeneralName(name="directory_name", value=crl_authority_name)
        if general_name in idp_general_names:
            idp_dp_match = True
...
def _handle_crl_idp_ext_constraints(
    ...
    if crl_idp["only_contains_attribute_certs"].native:
        errs.append("CRL only contains attribute certificates", certificate_list)
        return False
...
async def _handle_single_crl(
    ...
    crl_issuer = validation_context.revinfo_manager.check_crl_issuer(certificate_list)
    ...
    control_time = timing.validation_time if timing.point_in_time_validation else None
...
def _maybe_get_delta_crl(
    ...
    candidate_delta_lists = delta_lists_by_issuer.get(crl_authority_name.hashable, [])
    ...
    for certificate_list_cont in certificate_lists:
        ...
        try:
            ...
            if certificate_list.delta_crl_indicator_value is None:
                complete_lists_by_issuer[issuer_hashable].append(certificate_list_cont)
            else:
                delta_lists_by_issuer[issuer_hashable].append(certificate_list_cont)
...
def _process_crl_completeness(
    ...
    if checked_reasons != VALID_REVOCATION_REASONS:
        ...
        if not errs.failures:
            errs.append("The available CRLs do not cover all revocation reasons", None)
...
async def verify_crl(
    ...
    exc = _process_crl_completeness(checked_reasons, total_crls, errs, proc_state)
    if exc is not None:
        raise exc
