...
async def _validate_delegated_ocsp_provenance(
    ...
    ocsp_ee_name_override = proc_state.describe_cert(never_def=True) + " OCSP responder"
    ...
    if responder_cert.ocsp_no_check_value is not None:
        ...
        responder_chain = _delegated_ocsp_response_path(responder_cert, issuer, ee_path)
...
def _ocsp_allowed(responder_cert: x509.Certificate):
    ...
    return (
        extended_key_usage is not None and "ocsp_signing" in extended_key_usage.native
    )
...
def _match_ocsp_certid(
    ...
    serial_mismatch = response_cert_id["serial_number"].native != cert_serial_number
    ...
    if name_mismatch:
        errs.append("OCSP response issuer name hash does not match", ocsp_response)
        return False
    ...
    if key_hash_mismatch:
        errs.append("OCSP response issuer key hash does not match", ocsp_response)
        return False
    return True
...
async def verify_ocsp_response(
    ...
    errs = _OCSPErrs()
    ocsp_responses = await validation_context.revinfo_manager.async_retrieve_ocsps(
        cert, cert_issuer
    )
...
async def collect_relevant_responses_with_paths(
    ...
    ocsp_responses = await revinfo_manager.async_retrieve_ocsps(cert, cert_issuer_auth)
