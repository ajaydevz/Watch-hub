...
def get_certid(
    ...
    *,
    ...
    cert_id = ocsp.CertId(
        {
            "hash_algorithm": algos.DigestAlgorithm({"algorithm": certid_hash_algo}),
...
def process_ocsp_response_data(
    ...
    if status != "successful":
        raise errors.OCSPValidationError(
            "OCSP server at %s returned an error. Status was '%s'." % (ocsp_url, status)
        )
...
async def queue_fetch_task(
    ...
    try:
        ...
        logger.debug(f"Received completion signal for job with tag {repr(tag)}.")
        ...
        try:
            ...
            logger.debug(f"New fetch job with tag {repr(tag)} threw an exception: {e}")
            result = e
        results[tag] = result
...
def gather_aia_issuer_urls(cert: Union[x509.Certificate, cms.AttributeCertificateV2]):
    if isinstance(cert, x509.Certificate):
...
async def complete_certificate_fetch_jobs(fetch_jobs):
    for fetch_job in asyncio.as_completed(fetch_jobs):
        try:
            ...
            logger.warning(
                f"Error during certificate fetch job, skipping... " f"(Error: {e})",
            )
            continue
