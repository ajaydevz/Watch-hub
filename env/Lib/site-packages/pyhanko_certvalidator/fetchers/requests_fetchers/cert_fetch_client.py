...
class RequestsCertificateFetcher(CertificateFetcher, RequestsFetcherMixin):
    ...
    def __init__(self, user_agent=None, per_request_timeout=10, permit_pem=True):
    ...
    async def fetch_certs(self, url, url_origin_type):
        ...
        async def task():
            try:
                ...
                results = await self._grab_certs(url, url_origin_type=url_origin_type)
    ...
    async def _grab_certs(self, url, *, url_origin_type):
        ...
        try:
            ...
            content_type = None
        certs = unpack_cert_content(response.content, content_type, url, permit_pem)
        return list(certs)
