...
def extract_dir_name(names: x509.GeneralNames, err_msg_prefix: str) -> x509.Name:
    try:
        name: x509.Name = next(
...
def issuer_serial(cert: Union[x509.Certificate, cms.AttributeCertificateV2]) -> bytes:
    if isinstance(cert, x509.Certificate):
...
def get_ac_extension_value(attr_cert: cms.AttributeCertificateV2, ext_name: str):
    try:
        return next(
...
def get_declared_revinfo(cert: Union[x509.Certificate, cms.AttributeCertificateV2]):
    if isinstance(cert, x509.Certificate):
        aia = cert.authority_information_access_value
...
def validate_sig(
    ...
    if sig_algo == "dsa" and public_key_info["algorithm"]["parameters"].native is None:
        raise DSAParametersUnavailable("DSA public key parameters were not provided.")
    ...
    if sig_algo == "rsassa_pkcs1v15":
        ...
        raise NotImplementedError(f"Signature mechanism {sig_algo} is not supported.")
