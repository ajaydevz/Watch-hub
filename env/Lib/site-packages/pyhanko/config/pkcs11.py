...
class PKCS11PinEntryMode(enum.Enum):
    ...
    def parse_mode_setting(value: Any) -> "PKCS11PinEntryMode":
        if isinstance(value, str):
            ...
            return PKCS11PinEntryMode.PROMPT if value else PKCS11PinEntryMode.SKIP
...
class PKCS11SignatureConfig(api.ConfigurableMixin):
    ...
    def check_config_keys(cls, keys_supplied: Set[str]):
        ...
        super().check_config_keys(
            {k for k in keys_supplied if k not in ("token_label", "token-label")}
        )
    ...
    def process_entries(cls, config_dict):
        ...
        if cert_file is not None:
            config_dict["signing_certificate"] = load_cert_from_pemder(cert_file)

        if "key_id" in config_dict:
            config_dict["key_id"] = _process_pkcs11_id_value(config_dict["key_id"])
        ...
        if "cert_id" in config_dict:
            config_dict["cert_id"] = _process_pkcs11_id_value(config_dict["cert_id"])
        elif (
            "cert_label" not in config_dict and "signing_certificate" not in config_dict
        ):
            raise ConfigurationError(
        ...
        if "token_label" in config_dict:
            warnings.warn(
                "'token_label' is deprecated, use 'token_criteria.label' " "instead",
                DeprecationWarning,
            )
