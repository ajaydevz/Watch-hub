...
class CLIConfig:
    ...
    def _get_validation_settings_raw(self, name=None):
        ...
        try:
            ...
            raise ConfigurationError(f"There is no validation context named '{name}'.")

    def get_validation_context(self, name: Optional[str] = None, as_dict: bool = False):
        """
    ...
    def get_signer_key_usages(self, name: Optional[str] = None) -> KeyUsageConstraints:
        """
...
def parse_cli_config(yaml_str) -> CLIRootConfig:
    ...
    return CLIRootConfig(
        **process_root_config_settings(config_dict),
        config=CLIConfig(**process_config_dict(config_dict), raw_config=config_dict),
    )
...
def process_config_dict(config_dict: dict) -> dict:
    ...
    default_stamp_style = config_dict.get("default-stamp-style", DEFAULT_STAMP_STYLE)
