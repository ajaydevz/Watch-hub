...
@click.option(
    ...
    required=False,
    multiple=True,
    help="certificate(s) corresponding to entities that " "can decrypt the output file",
    type=click.Path(readable=True, dir_okay=False),
)
def encrypt_file(infile, outfile, password, recipient):
    if password and recipient:
        raise click.ClickException("Specify either a password or a list of recipients.")
...
def decrypt_with_pemder(infile, outfile, key, cert, passfile, force, no_pass):
    ...
    sedk = crypt.SimpleEnvelopeKeyDecrypter.load(key, cert, key_passphrase=passphrase)

    _decrypt_pubkey(sedk, infile, outfile, force)


def _decrypt_pubkey(sedk: crypt.SimpleEnvelopeKeyDecrypter, infile, outfile, force):
    with pyhanko_exception_manager():
        with open(infile, "rb") as inf:
            r = PdfFileReader(inf)
            if r.security_handler is None:
...
def decrypt_with_pkcs12(infile, outfile, pfx, passfile, force):
    if passfile is None:
        ...
        passfile.close()
    sedk = crypt.SimpleEnvelopeKeyDecrypter.load_pkcs12(pfx, passphrase=passphrase)
