...
class PemderPlugin(SigningCommandPlugin):
    ...
    def create_signer(self, context: CLIContext, **kwargs) -> ContextManager[Signer]:
...
def _pemder_signer(
    ...
    return signer_from_pemder_config(pemder_config, provided_key_passphrase=passphrase)
...
def _pkcs12_signer(ctx: CLIContext, pfx, chain, passfile, p12_setup, no_pass):
    ...
    if p12_setup:
        ...
        raise click.ClickException(
            "Either the PFX argument or the --p12-setup option " "must be provided."
        )
    else:
    ...
    if pkcs12_config.pfx_passphrase is not None:
        ...
        passphrase = getpass.getpass(prompt="PKCS#12 passphrase: ").encode("utf-8")
        if not passphrase:
            _warn_empty_passphrase()
            passphrase = None
    else:
    ...
    return signer_from_p12_config(pkcs12_config, provided_pfx_passphrase=passphrase)
