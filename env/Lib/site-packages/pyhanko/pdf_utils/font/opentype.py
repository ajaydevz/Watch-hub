...
def _format_simple_glyphline_from_buffer(
    ...
    def _emit_subsegment():
        ...
        current_tj_segments.append(f"<{''.join('%04x' % cid for cid in subsgmt_cids)}>")
        subsgmt_cids = []
        if tj_adjust:
...
def _format_cid_glyphline_from_buffer(
    ...
    if no_complex_positioning:
        ops, total_len = _format_simple_glyphline_from_buffer(buf, cid_width_callback)
    ...
    if x_pos or y_pos:
        commands.append(b"%g %g Td" % (_glyph_to_user(x_pos), _glyph_to_user(y_pos)))
    ...
    def _emit():
        ...
        for source_start, targets in decls:
            if use_bfrange:
                ...
                yield (f"<{source_start:04x}> <{source_end:04x}> [{target_values}]")
            else:
                target = targets[0]
                yield (f'<{source_start:04x}> <{hexlify(target).decode("ascii")}>')
...
class GlyphAccumulator(FontEngine):
    ...
    def __init__(
        ...
        try:
            ...
            self.cidfont_obj = CIDFontType2(tt, base_ps_name, self.subset_prefix)
