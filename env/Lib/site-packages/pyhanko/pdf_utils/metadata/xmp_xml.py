...
def _xmp_struct_to_xml(description: ElementTree.Element, value: model.XmpStructure):
    for k, v in value:
...
def _populate_xmp_with_meta(
    ...
    if author is not None:
        fields[model.DC_CREATOR] = model.XmpValue(model.XmpArray.ordered([author]))
...
def meta_from_xmp(roots: List[model.XmpStructure]):
    ...
    subject = get_and_apply(all_fields, model.DC_DESCRIPTION, _simplify_meta_str)
...
BOM_REGISTRY = {
    "\ufeff".encode(enc): enc for enc in ("utf-8", "utf-16be", "utf-16le", "utf32")
}
...
def _proc_xmp_arr(elem: ElementTree.Element, lang: Optional[str]) -> model.XmpArray:
    name = _name(elem)
    if name is None:
        raise ValueError
...
def _proc_xmp_value(elem: ElementTree.Element, lang: Optional[str]) -> model.XmpValue:
    ...
    if parse_type == "Resource":
        ...
        raise XmpXmlProcessingError(f"Parse type {parse_type!r} is not supported")
