...
class PdfFileReader(PdfHandler):
    ...
    def _get_object_from_stream(self, idnum, stmnum, idx):
        ...
        for i in range(stream["/N"]):
            try:
                ...
                if self.strict:
                    raise PdfStrictReadError("Object stream header possibly corrupted")
                else:
                    return generic.NullObject()
            if objnum != idnum:
            ...
            generic.read_non_whitespace(stream_data, seek_back=True, allow_eof=True)
            return obj

        if self.strict:
            raise PdfStrictReadError(
                "Object not found in stream, " "this is a fatal error in strict mode"
            )
        else:
            return generic.NullObject()
    ...
    def encrypt_dict(self) -> Optional[generic.DictionaryObject]:
        ...
        if isinstance(encrypt_ref, generic.IndirectObject):
            encrypt_dict = self.get_object(encrypt_ref.reference, never_decrypt=True)
        elif not self.strict:
            encrypt_dict = encrypt_ref
        else:
            raise misc.PdfReadError("Encryption settings must be an indirect reference")
    ...
    def get_object(
        ...
        if transparent_decrypt and isinstance(obj, generic.DecryptedObjectProxy):
            obj = obj.decrypted
    ...
    def _read_object(
        ...
        if marker is None:
            ...
            idnum, generation = read_object_header(self.stream, strict=self.strict)
...
class RawPdfPath:
    ...
    def walk_nodes(
        ...
        for ix, entry in enumerate(self.path):
            ...
            raise misc.PdfReadError(f"Type error in path {self} at position {ix}.")
        if isinstance(current_obj, generic.IndirectObject) and transparent_dereference:
            yield elem, current_obj.get_object()
        else:
            yield elem, current_obj

    def access_on(self, from_obj, dereference_last=True) -> generic.PdfObject:
        walk = self.walk_nodes(from_obj, transparent_dereference=dereference_last)
        obj = from_obj
        try:
            while True:
                _, obj = next(walk)
...
class HistoricalResolver(PdfHandler):
    ...
    def get_object(self, ref: generic.Reference, as_metadata_stream: bool = False):
        cache = self.cache
        try:
            obj = cache[ref]
        except KeyError:
            ...
            try:
                ...
                logger.warning(f"Could not determine history of {ref} in xref sections")
                last_change = None
    ...
    def root_ref(self) -> generic.Reference:
        ...
        return generic.Reference(idnum=ref.idnum, generation=ref.generation, pdf=self)
