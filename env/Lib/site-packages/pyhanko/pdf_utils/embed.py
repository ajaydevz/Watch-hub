...
class EmbeddedFileObject(generic.StreamObject):
    ...
    def write_to_stream(self, stream, handler=None, container_ref=None):
        ...
        if handler is not None and not self._has_crypt_filter:
            ...
            if ef_filter_name is not None and ef_filter_name != stream_filter_name:
        ...
        if params is not None:
            ...
            if params.creation_date is not None:
                param_dict["/CreationDate"] = generic.pdf_date(params.creation_date)
            if params.modification_date is not None:
                param_dict["/ModDate"] = generic.pdf_date(params.modification_date)

        super().write_to_stream(stream, handler=handler, container_ref=container_ref)
...
def embed_file(pdf_writer: writer.BasePdfFileWriter, spec: FileSpec):
    ...
    if ef_stream is None:
        raise misc.PdfWriteError("File spec does not have an embedded file stream")
    ...
    if "/Kids" in ef_name_tree:
        raise NotImplementedError("Only flat name trees are supported right now")
