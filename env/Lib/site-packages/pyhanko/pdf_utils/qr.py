...
class PdfStreamQRImage(BaseImage):
    ...
    def format_qr_color(self):
        return (b"%g %g %g rg\n" % self.qr_color) + (b"%g %g %g RG" % self.qr_color)
...
class PdfFancyQRImage(PdfStreamQRImage):
    ...
    def draw_centerpiece(self):
        ...
        centerpiece_commands.extend(
            rounded_square(c_x, c_y, c_sz, self.centerpiece_corner_radius * c_sz)
        )
...
def rounded_square(x_pos: float, y_pos: float, sz: float, rad: float) -> List[bytes]:
    """
