...
class PubKeyCryptFilter(CryptFilter, abc.ABC):
    ...
    def add_recipients(
        ...
        if not self.acts_as_default and self.recipients:
            raise misc.PdfError(
                "A non-default crypt filter cannot have multiple sets of " "recipients."
            )
    ...
    def as_pdf_object(self):
        ...
        result["/EncryptMetadata"] = generic.BooleanObject(self.encrypt_metadata)
        return result
...
def construct_envelope_content(seed: bytes, perms: int, include_permissions=True):
    assert len(seed) == 20
...
def _choose_ecdh_settings(
    pub_key_info: PublicKeyInfo,
) -> Tuple[hashes.HashAlgorithm, KeyEncryptionAlgorithmId, KeyEncryptionAlgorithmId]:
...
def _ecdh_recipient(
    ...
    originator_key: Union[X25519PrivateKey, X448PrivateKey, EllipticCurvePrivateKey]
    ...
    encrypted_data = keywrap.aes_key_wrap(wrapping_key=kek, key_to_wrap=envelope_key)
    return _format_kari(
        rid=rid,
...
def _recipient_info(
    ...
    if algorithm_name == "rsa":
        rid = cms.RecipientIdentifier({"issuer_and_serial_number": iss_serial_rid})
        ...
        raise NotImplementedError(f"Cannot encrypt for key type '{algorithm_name}'")
...
def construct_recipient_cms(
    ...
    rec_infos = [
        _recipient_info(envelope_key, cert, policy=policy) for cert in certificates
    ]
...
class SimpleEnvelopeKeyDecrypter(EnvelopeKeyDecrypter, SerialisableCredential):
    ...
    def decrypt_with_exchange(
        ...
        originator_pub_key_info: PublicKeyInfo = originator_identifier.chosen.untag()
        ...
        mismatch_msg = (
            "Originator's public key is not compatible " "with selected private key"
        )
        if isinstance(priv_key, EllipticCurvePrivateKey):
            ...
            ecdh_value = priv_key.exchange(ECDH(), peer_public_key=originator_pub_key)
...
def read_seed_from_recipient_cms(
    ...
    if content_type != "enveloped_data":
        raise misc.PdfReadError(
            "Recipient CMS content type must be enveloped data, not " + content_type
        )
    ...
    encrypted_envelope_content = encrypted_content_info["encrypted_content"].native
    ...
    if cipher_name in with_iv:
        ...
        raise misc.PdfReadError(f"Cipher {cipher_name} is not allowed in PDF 2.0.")
...
def _read_generic_pubkey_cf_info(cfdict: generic.DictionaryObject):
    try:
        ...
        raise misc.PdfReadError("PubKey CF dictionary must have /Recipients key")
    if isinstance(recipients, generic.ByteStringObject):
        recipients = (recipients,)
    recipient_objs = [cms.ContentInfo.load(x.original_bytes) for x in recipients]
...
class PubKeySecurityHandler(SecurityHandler):
    ...
    def build_from_certs(
        ...
        **kwargs,
        ...
        """
        subfilter = (
            PubKeyAdbeSubFilter.S5 if use_crypt_filters else PubKeyAdbeSubFilter.S4
        )
        cfc = None
    ...
    def __init__(
        ...
        if crypt_filter_config is None:
            if version == SecurityHandlerVersion.RC4_40:
                ...
                raise misc.PdfError("Failed to impute a reasonable crypt filter config")
        super().__init__(
            version,
            legacy_keylen,
            crypt_filter_config,
    ...
    def read_cf_dictionary(
        ...
        cf = build_crypt_filter(cls._known_crypt_filters, cfdict, acts_as_default)
        if cf is None:
            raise misc.PdfReadError(
    ...
    def process_crypt_filters(
        ...
        if cfc is not None and subfilter != PubKeyAdbeSubFilter.S5:
            ...
            raise misc.PdfReadError("/adbe.pkcs7.s5 handler requires crypt filters.")
        return cfc
    ...
    @classmethod
    def instantiate_from_pdf_object(cls, encrypt_dict: generic.DictionaryObject):
    ...
    def as_pdf_object(self):
        ...
        if self.subfilter == PubKeyAdbeSubFilter.S5:
            ...
            result["/Recipients"] = generic.ArrayObject(
                generic.ByteStringObject(recp.dump()) for recp in default_cf.recipients
            )
        return result
