...
def _r6_hash_algo(
    ...
    while round_no < 64 or last_byte_val > round_no - 32:
        ...
        e = aes_cbc_encrypt(key=k[:16], data=k1, iv=k[16:32], use_padding=False)[1]
...
class StandardSecuritySettingsRevision(misc.VersionEnum):
    ...
    def as_pdf_object(self) -> generic.PdfObject:
        val = self.value
        return generic.NullObject() if val is None else generic.NumberObject(val)
...
class StandardSecurityHandler(SecurityHandler):
    ...
    _known_crypt_filters: Dict[generic.NameObject, CryptFilterBuilder] = {
        ...
        generic.NameObject("/AESV2"): lambda _, __: StandardAESCryptFilter(keylen=16),
        generic.NameObject("/AESV3"): lambda _, __: StandardAESCryptFilter(keylen=32),
    ...
    def build_from_pw_legacy(
        ...
        **kwargs,
        ...
        if rev > StandardSecuritySettingsRevision.RC4_OR_AES128:
            raise ValueError(f"{rev} is not supported by this bootstrapping method.")
        if rev == StandardSecuritySettingsRevision.RC4_BASIC:
            keylen_bytes = 5
        elif use_aes128 and rev == StandardSecuritySettingsRevision.RC4_OR_AES128:
            keylen_bytes = 16
        ...
        if rev == StandardSecuritySettingsRevision.RC4_BASIC:
            ...
            u_entry, key = compute_u_value_r2(desired_user_pass, o_entry, perms, id1)
        else:
    ...
    def __init__(
        ...
        if crypt_filter_config is None:
            if version == SecurityHandlerVersion.RC4_40:
                ...
                version >= SecurityHandlerVersion.AES256 and crypt_filter_config is None
            ):
                # there's a reasonable default config that we can fall back
                # to here
                crypt_filter_config = _std_aes_config(32)
            else:
                raise misc.PdfError("Could not impute a reasonable crypt filter config")
        super().__init__(
            version,
            legacy_keylen,
            crypt_filter_config,
    ...
    @classmethod
    def gather_encryption_metadata(cls, encrypt_dict: generic.DictionaryObject) -> dict:
        """
        ...
        def _get_bytes(x: generic.PdfObject) -> bytes:
            if not isinstance(x, (generic.TextStringObject, generic.ByteStringObject)):
    ...
    def authenticate(self, credential, id1: Optional[bytes] = None) -> AuthResult:
        """
        ...
        if rev >= StandardSecuritySettingsRevision.AES256:
            ...
            if id1 is None:
                raise misc.PdfReadError("id1 must be specified for legacy encryption")
    ...
    def _authenticate_r6(self, password) -> Tuple[AuthStatus, Optional[bytes]]:
        ...
        if _r6_password_authenticate(pw_bytes, o_entry_split, self.udata):
            ...
            key = _r6_derive_file_key(pw_bytes, o_entry_split, self.oeseed, self.udata)
