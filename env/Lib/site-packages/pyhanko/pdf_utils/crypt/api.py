...
class SecurityHandlerVersion(misc.VersionEnum):
    ...
    def check_key_length(self, key_length: int) -> int:
        if self == SecurityHandlerVersion.RC4_40:
            ...
            not (5 <= key_length <= 16) and self <= SecurityHandlerVersion.RC4_OR_AES128
        ):
...
class SecurityHandler:
    ...
    def read_cf_dictionary(
        ...
        :raise NotImplementedError:
            ...
            crypt filter method.
        """
        return build_crypt_filter(cls._known_crypt_filters, cfdict, acts_as_default)
...
class CryptFilterConfiguration:
    ...
    def __init__(
        ...
        def _select(name) -> CryptFilter:
            return IdentityCryptFilter() if name == IDENTITY else crypt_filters[name]
    ...
    def __contains__(self, item):
        return item == generic.NameObject("/Identity") or item in self._crypt_filters
