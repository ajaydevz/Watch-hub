            ...
            if generation > 0xFFFF:
                if strict:
                    raise PdfStrictReadError(
                        f"Illegal generation {generation} for " f"object ID {idnum}."
                    )
                continue
...
def _check_freed_refs(ix: int, section: XRefSection, all_sections: List[XRefSection]):
    ...
    for idnum, expected_next_generation in section.xref_data.freed.items():
        ...
        if ix < len(all_sections) - 1:
            ...
            if hybrid is not None:
                if idnum in hybrid.standard_xrefs or idnum in hybrid.xrefs_in_objstm:
                    # exemption!
                    continue
...
class XRefBuilder:
    ...
    def __init__(self, handler: PdfHandler, stream, strict: bool, last_startxref: int):
    ...
    def _read_xref_stream_object(self):
        ...
        xrefstream = generic.StreamObject.read_from_stream(stream, xrefstream_ref)
    ...
    def read_xrefs(self):
        ...
        while startxref is not None:
            ...
            if misc.skip_over_whitespace(stream):
                ...
                logger.debug(
                    "Encountered unexpected whitespace when looking " "for xref stream"
                )
...
def _read_object_header(stream, strict):
    ...
    if extra and strict:
        logger.warning(
            f"Superfluous whitespace found in object header " f"{idnum} {generation}"
        )
...
def read_object_header(stream, strict):
    ...
    try:
        ...
        raise ObjectHeaderReadError(f"Failed to read object header at {pos}") from e
...
class TrailerDictionary(generic.PdfObject):
    ...
    def write_to_stream(self, stream, handler=None, container_ref=None):
        raise NotImplementedError("TrailerDictionary object cannot be written directly")
...
class XRefCache:
    ...
    def get_historical_ref(self, ref, revision) -> Optional[Union[int, ObjStreamRef]]:
        """
