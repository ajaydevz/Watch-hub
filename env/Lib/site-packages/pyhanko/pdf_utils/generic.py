...
class BooleanObject(PdfObject):
    ...
    def __eq__(self, other):
        return isinstance(other, (BooleanObject, bool)) and bool(self) == bool(other)
...
class ArrayObject(list, PdfObject):
    ...
    def write_to_stream(
        ...
        for data in self:
            stream.write(b" ")
            data.write_to_stream(stream, handler=handler, container_ref=container_ref)
        stream.write(b" ]")
...
class IndirectObject(PdfObject, Dereferenceable):
    ...
    def read_from_stream(stream, container_ref: "Dereferenceable"):
        ...
        try:
            ...
            pos = hex(stream.tell())
            raise PdfReadError(f"Parse error on indirect object reference around {pos}")
    ...
    def read_tokens():
        ...
        while True:
            ...
            if tok == b">":
                ...
                raise PdfStreamError("Unexpected token in hex string: " + repr(tok))
            yield tok
            odd = not odd

    result = binascii.unhexlify(b"".join(read_tokens()) + (b"0" if odd else b""))
...
def _as_hex_digit(ascii_char):
    if 0x30 <= ascii_char <= 0x39:
        ...
        raise PdfReadError("Numeric escape in PDF name must use hexadecimal digits")
...
def _decode_name(name_bytes: bytes) -> "NameObject":
    ...
    try:
        while True:
            ...
            elif not (0x21 <= cur_byte <= 0x7E) or not is_regular_character(cur_byte):
                raise PdfReadError(
...
class NameObject(str, PdfObject):
    ...
    def write_to_stream(
        ...
        if not next(byte_iter) == 0x2F:
            raise PdfWriteError(
                f"Could not serialise name object {repr(self)}, " f"must start with /"
            )
        stream.write(b"/")
...
class DictionaryObject(dict, PdfObject):
    ...
    def __init__(self, dict_data=None):
        if dict_data is not None:
            super().__init__({_normalise_key(k): v for k, v in dict_data.items()})
        else:
            super().__init__()
    ...
    def __getitem__(self, key):
        ...
        if key == "/Metadata" and isinstance(raw_obj, IndirectObject):
            ...
            return handler.get_object(raw_obj.reference, as_metadata_stream=True)
        else:
    ...
    def read_from_stream(
        ...
        while True:
            ...
            if key not in data:
                ...
                err = "Multiple definitions in dictionary at byte " "%s for key %s" % (
                    hex(stream.tell()),
                    key,
                )
                if handler.strict:
...
class StreamObject(DictionaryObject):
    ...
    def add_crypt_filter(
        ...
        params["/Name"] = name
        self.apply_filter(pdf_name("/Crypt"), params=params, allow_duplicates=True)
    ...
    def apply_filter(
        ...
        *Note:* Calling this method on an encoded stream will first cause the
        ...
        if not cur_filters:
            ...
            self[pdf_name("/Filter")] = ArrayObject((filter_name,) + filter_names)
