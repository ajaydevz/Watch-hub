    ...
    for der_obj_type in der_stream_keys:
        ...
        if isinstance(value, generic.IndirectObject):
            ...
            try:
                ...
                _assert_stream_refs(der_obj_type, old_value, misc.PdfReadError, is_vri)
    ...
    def _check_vri(old, new, old_vri, new_vri):
        ...
        for key, old_vri_value in old_vri.items():
            ...
            if new_vri_dict != old_vri_value:
                ...
                raise SuspiciousModification(f"VRI key {key} was modified or deleted.")
        ...
        for key in new_vri_hashes - old_vri.keys():
            ...
            if not isinstance(new_vri_dict, generic.DictionaryObject):
                raise SuspiciousModification("VRI entries should be dictionaries")
            ...
            try:
                ts_ref = new_vri_dict.get_value_as_reference("/TS", optional=True)
...
class BaseFieldModificationRule(FieldMDPRule):
    ...
    def __init__(
        ...
        self.value_update_keys = (
            value_update_keys if value_update_keys is not None else VALUE_UPDATE_KEYS
        )
    ...
    def check_form_field(
        ...
        if old_field:
            ...
            if not previously_signed and now_signed:
                yield from qualify(ModificationLevel.LTA_UPDATES, field_ref_updates)
        ...
        try:
            ...
            for sigref_dict in sig_obj.raw_get("/Reference"):
                try:
                    ...
                    yield (
                        sig_whitelist,
                        FormUpdate(updated_ref=tp.reference, field_name=fq_name),
                    )
...
class GenericFieldModificationRule(BaseFieldModificationRule):
    ...
    def check_form_field(
        ...
        yield from qualify_transforming(
            ModificationLevel.FORM_FILLING,
            _allow_appearance_update(old_field, new_field, context.old, context.new),
            transform=FormUpdate.curry_ref(field_name=fq_name),
        )
        if self.allow_in_place_appearance_stream_changes:
            yield from qualify(
...
def _allow_in_place_appearance_update(
    ...
    for key in ("/N", "/R", "/D"):
        ...
        if isinstance(old_ap_val, generic.DictionaryObject):
            old_ap_stm_ref = old_ap_val.get_value_as_reference(key, optional=True)
            new_ap_stm_ref = new_ap_val.get_value_as_reference(key, optional=True)
            ...
            if (
                ...
                continue
            if isinstance(old_ap_raw, generic.IndirectObject):
                context = RelativeContext(old_ap_raw, relative_path=RawPdfPath(key))
            else:
...
def _walk_page_tree_annots(
    ...
    if old_kids != new_kids:
        raise SuspiciousModification("Unexpected change to page tree structure.")
