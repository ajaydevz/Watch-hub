...
class DocInfoRule(WhitelistRule):
    ...
    def apply(
        ...
        new_info = new.trailer_view.get_value_as_reference("/Info", optional=True)
        if new_info is None:
            return
        old_info = old.trailer_view.get_value_as_reference("/Info", optional=True)
        yield from map(
...
class MetadataUpdateRule(WhitelistRule):
    ...
    def __init__(self, check_xml_syntax=True, always_refuse_stream_override=False):
