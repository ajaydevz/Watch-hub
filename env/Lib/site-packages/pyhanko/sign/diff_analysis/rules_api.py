...
class RelativeContext(Context):
    ...
    def descend(self, path: Union[RawPdfPath, int, str]) -> "RelativeContext":
        ...
        if not isinstance(root, containers):
            raise misc.PdfReadError(f"Anchor {self.anchor} is not a container object")
...
class AbsoluteContext(Context):
    ...
    pdf_handler: PdfHandler = dataclass_field(repr=False, hash=False, compare=False)
    """
    ...
    def relative_view(self) -> RelativeContext:
        return RelativeContext.relative_to(self.pdf_handler.trailer_view, self.path)
