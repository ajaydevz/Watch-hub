...
def compare_key_refs(
    ...
    try:
        ...
        if old_value is not None:
            raise SuspiciousModification(f"Key {key} was deleted from dictionary")
...
def qualify_transforming(
    ...
    :return:
        ...
        modification level specified.
    """
    return misc.map_with_return(rule_result, lambda ref: (level, transform(ref)))
...
def compare_dicts(
    ...
    if not isinstance(new_dict, generic.DictionaryObject):
        raise SuspiciousModification("Dict is overridden by non-dict in new revision")
    ...
    for k in new_dict_keys:
        ...
        if new_val != old_val:
            if raise_exc:
                raise SuspiciousModification(
                    f"Values for dict key {k} differ:" f"{old_val} changed to {new_val}"
                )
            else:
                return False
