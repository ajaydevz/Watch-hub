...
class FieldComparisonSpec:
    ...
    def _old_annotation_paths(self):
        ...
        def _path_ok(pdf_path: RawPdfPath):
            ...
            return (
                descended and not rest and nxt == "/Annots" and isinstance(nxt_ix, int)
            )
...
def _list_fields(
    ...
    for field_name in names:
        ...
        try:
            new_field_type, new_field_ref, new_kids, _ = new_fields_by_name[field_name]
        except KeyError:
        ...
        if old_kids or new_kids:
            yield from _list_fields(
                ...
                old_path=(field_path + "/Kids" if field_path is not None else None),
...
class FormUpdatingRule:
    ...
    * it delegates most of the hard work to sub-rules (instances of
    ...
    def __init__(self, field_rules: List[FieldMDPRule], ignored_acroform_keys=None):
    ...
    def apply(
        ...
        acroform_context = Context.from_absolute(old, RawPdfPath("/Root", "/AcroForm"))
        ...
        dr_context = Context.from_absolute(old, RawPdfPath("/Root", "/AcroForm", "/DR"))
        old_dr, new_dr = yield from qualify_transforming(
            ...
            transform=FormUpdate.curry_ref(field_name=None, context_checked=dr_context),
        )
        if new_dr is not None:
            dr_deps = new.collect_dependencies(new_dr, since_revision=old.revision + 1)
