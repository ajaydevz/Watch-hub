...
def find_token(
    ...
    if slot_no is None:
        ...
        if errors:
            err_str = ", ".join(f"{field} is not {val!r}" for field, val in errors)
            raise PKCS11Error(
...
def select_pkcs11_signing_params(
    ...
    if signature_algo == "rsassa_pkcs1v15":
        if use_raw_mechanism:
            ...
            pre_sign_transform = _hash_fully(digest_algorithm, wrap_digest_info=True)
        else:
            kwargs["mechanism"] = RSA_MECH_MAP[digest_algorithm]
    elif signature_algo == "dsa":
        if use_raw_mechanism:
            kwargs["mechanism"] = Mechanism.DSA
            pre_sign_transform = _hash_fully(digest_algorithm, wrap_digest_info=False)
        else:
...
def _pull_cert(
    ...
    if len(results) == 1:
        ...
        err = _format_pull_err_msg(no_results=not results, label=label, cert_id=cert_id)
...
class PKCS11SigningContext:
    ...
    def __init__(self, config: PKCS11SignatureConfig, user_pin: Optional[str] = None):
        self.config = config
        self._session = None
    ...
    def __enter__(self):
        try:
            ...
            raise SigningError(f"PKCS#11 error: [{type(ex).__name__}] {ex}") from ex
