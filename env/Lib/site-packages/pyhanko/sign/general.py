        ...
        for attr in attrs:
            if attr["type"].native == name:
                if found_values is not None:
                    raise CMSStructuralError(f"Attribute {name!r} was duplicated")
...
def match_issuer_serial(
    ...
    if isinstance(expected_issuer, x509.GeneralNames):
        if len(expected_issuer) != 1 or expected_issuer[0].name != "directory_name":
            return False
    ...
    try:
        ...
        issuer_match = False
    return issuer_match and expected_issuer_serial["serial_number"] == serial_asn1
...
def process_pss_params(
    ...
    if md_name.casefold() != digest_algorithm.casefold():
        raise ValueError(
            f"PSS MD '{md_name}' must agree with signature " f"MD '{digest_algorithm}'."
        )  # pragma: nocover
    ...
    pss_padding = padding.PSS(mgf=padding.MGF1(algorithm=mgf_md), salt_length=salt_len)
...
def optimal_pss_params(
    ...
    return algos.RSASSAPSSParams(
        {
            "hash_algorithm": algos.DigestAlgorithm({"algorithm": digest_algorithm}),
...
def extract_signer_info(signed_data: cms.SignedData) -> cms.SignerInfo:
    ...
    try:
        ...
        raise CMSExtractionError("signer_infos should contain exactly one entry")
