...
class RevocationInfoGatheringSpec:
    ...
    fetcher_backend: FetcherBackend = field(default_factory=RequestsFetcherBackend)
...
class LocalKnowledge:
    ...
    nonrevoked_assertions: List[NonRevokedStatusAssertion] = field(default_factory=list)
    ...
    def assert_existence_known_at(
        ...
        for crl in self.known_crls:
            yield KnownPOE(
                ...
                poe_time=dt,
                poe_type=POEType.PROVIDED,
                validation_object=ValidationObject(ValidationObjectType.CRL, crl),
            )
...
def bootstrap_validation_data_handlers(
    ...
    if is_historical is None:
        hist = (
            timing_info.point_in_time_validation if timing_info is not None else False
        )
    else:
        hist = is_historical
