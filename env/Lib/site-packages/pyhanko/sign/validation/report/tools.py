...
def _summarise_attrs(
    ...
    try:
        commitment_type: cades_asn1.CommitmentTypeIndication = (
            find_unique_cms_attribute(signed_attrs, "commitment_type_indication")
        )
    ...
    try:
        sig_policy_ident: cades_asn1.SignaturePolicyIdentifier = find_cms_attribute(
    ...
    kwargs["byte_range"] = tuple(int(x) for x in embedded_sig.sig_object["/ByteRange"])
    # filter (SAFilterType)
    ...
    single_report = ts_11910202.SignatureValidationReportType(
        ...
        signer_information=ts_11910202.SignerInformationType(
            signer_certificate=ts_11910202.VOReferenceType(
                voreference=(f"{derive_validation_object_identifier(signer_cert_vo)}",),
            ),
        ),
...
def generate_report(
    ...
    report = ts_11910202.ValidationReport(
        ...
        signature_validation_objects=ts_11910202.ValidationObjectListType(
            tuple(_package_validation_object(vo) for vo in status.validation_objects)
        ),
    )
