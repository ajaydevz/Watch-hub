...
def _check_signing_certificate(cert: x509.Certificate, signed_attrs: cms.CMSAttributes):
    # TODO check certificate policies, enforce restrictions on chain of trust
    # TODO document and/or mark as internal API explicitly
...
def validate_algorithm_protection(
    ...
    if cms_algid_protection is not None:
        ...
        signed_sig_algorithm = cms_algid_protection["signature_algorithm"].native
...
def validate_sig_integrity(
    ...
    if signed_attrs_orig is core.VOID:
        ...
        try:
            content_type = find_unique_cms_attribute(signed_attrs, "content_type")
    ...
    intact = actual_digest == embedded_digest if embedded_digest is not None else valid
...
async def cms_basic_validation(
    ...
    *,
    ...
    if validation_context is not None:
        algorithm_policy = algorithm_policy or CMSAlgorithmUsagePolicy.lift_policy(
    ...
    if valid:
        try:
            validation_context.certificate_registry.register_multiple(other_certs)

            paths: CancelableAsyncIterator[ValidationPath]
            if validation_path is not None:
                paths = lift_iterable_async([validation_path])
            else:
                paths = validation_context.path_builder.async_build_paths_lazy(cert)
...
async def async_validate_cms_signature(
    ...
    """
    eff_key_usage_settings = status_cls.default_usage_constraints(key_usage_settings)
...
async def collect_timing_info(
    ...
    if content_tst_signed_data is not None:
        ...
        content_tst_validity = TimestampSignatureStatus(**content_tst_validity_kwargs)
...
async def process_certified_attrs(
    ...
    List[Union[PathValidationError, PathBuildingError, InvalidCertificateError]],
]:
    jobs = [
        async_validate_ac(ac, validation_context, holder_cert=signer_cert) for ac in acs
    ]
    results = []
    errors = []
...
async def collect_signer_attr_status(
    ...
    if signer_attrs is not None:
        ...
        if not isinstance(certified_asn1, core.Void):
            ...
            cades_acs = [
                attr.chosen for attr in certified_asn1 if attr.name == "attr_cert"
            ]
