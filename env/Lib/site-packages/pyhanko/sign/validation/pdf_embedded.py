...
def _extract_docmdp_for_sig(signature_obj) -> Optional[MDPPerm]:
    ...
    try:
        ...
        raise SignatureValidationError("Failed to read document permissions") from e
...
class EmbeddedPdfSignature:
    ...
    def __init__(
        ...
        try:
            ...
            raise misc.PdfReadError("Could not read /ByteRange entry in signature")
        ...
        if content_type == "data":
            ...
            self.external_md_algorithm = mi["hash_algorithm"]["algorithm"].native
    ...
    def self_reported_timestamp(self) -> Optional[datetime]:
        ...
        try:
            ...
            return generic.parse_pdf_date(st_as_pdf_date, strict=self.reader.strict)
    ...
    def fieldmdp(self) -> Optional[FieldMDPSpec]:
        ...
        try:
            ...
            raise SignatureValidationError("Failed to read /FieldMDP settings") from e
        self._fieldmdp = sp
        return sp
    ...
    def evaluate_modifications(
        ...
        if self.coverage < SignatureCoverageLevel.ENTIRE_REVISION:
            return SuspiciousModification("Nonstandard signature coverage level")
...
def _validate_sv_constraints(
    ...
    if (flags & SigSeedValFlags.APPEARANCE_FILTER) and sv_spec.appearance is not None:
        logger.warning(
    ...
    if (flags & SigSeedValFlags.LOCK_DOCUMENT) and sv_spec.lock_document is not None:
        doc_mdp = emb_sig.docmdp_level
        if (
    ...
    if (flags & SigSeedValFlags.DIGEST_METHOD) and sv_spec.digest_methods is not None:
...
async def async_validate_pdf_signature(
    ...
    embedded_sig.compute_integrity_info(diff_policy=diff_policy, skip_diff=skip_diff)
...
async def async_validate_pdf_timestamp(
    ...
    if embedded_sig.sig_object_type != "/DocTimeStamp":
        raise SignatureValidationError("Signature object type must be /DocTimeStamp")
