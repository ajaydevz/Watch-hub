...
class DocumentSecurityStore:
    ...
    def _embed_cert(self, cert):
        ...
        ref = self.writer.add_object(generic.StreamObject(stream_data=cert.dump()))
        self._mark_modified()
    ...
    def register_vri(self, identifier, *, certs=(), ocsps=(), crls=()):
        ...
        if ocsps:
            ocsp_refs = set(
                self._cms_objects_to_streams(ocsps, self._ocsps_seen, self.ocsps)
            )
        if crls:
            crl_refs = set(
        ...
        if identifier is not None:
            ...
            self.vri_entries[identifier] = self.writer.add_object(vri.as_pdf_object())
            self._mark_modified()
    ...
    def supply_dss_in_writer(
        ...
        *,
        ...
        dss.register_vri(identifier, certs=_certs(), ocsps=_ocsps(), crls=_crls())
...
async def collect_validation_info(
    ...
    revinfo_fetch_policy = validation_context.revinfo_policy.revocation_checking_policy
