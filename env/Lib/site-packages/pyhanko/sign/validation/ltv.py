...
def _strict_vc_context_kwargs(timestamp: datetime, validation_context_kwargs: dict):
...
def _instantiate_ltv_vc(emb_timestamp: EmbeddedPdfSignature, validation_context_kwargs):
    try:
...
async def _establish_timestamp_trust_lta(
    ...
    for ts_count, emb_timestamp in enumerate(timestamps):
        ...
        _strict_vc_context_kwargs(ts_status.timestamp, validation_context_kwargs)
        # read the DSS at the current revision into a new
        # validation context object
        current_vc = _instantiate_ltv_vc(emb_timestamp, validation_context_kwargs)
...
async def async_validate_pdf_ltv_signature(
    ...
    if force_revinfo:
        ...
        if ac_validation_context_kwargs is not None:
            ac_validation_context_kwargs[
                "revinfo_policy"
            ] = _strict_ltv_internal_revo_policy(retroactive_revinfo=retroactive)
    ...
    if validation_type == RevocationInfoValidationType.ADOBE_STYLE:
        dss = None
        current_vc = bootstrap_validation_context or ValidationContext(**vc_kwargs)
    else:
        ...
        if bootstrap_validation_context is None:
            current_vc = dss.as_validation_context(vc_kwargs, include_revinfo=False)
        else:
            current_vc = bootstrap_validation_context
    ...
    if validation_type != RevocationInfoValidationType.ADOBE_STYLE:
        ...
        if ts_trust_data.latest_dts:
            current_vc = _instantiate_ltv_vc(ts_trust_data.latest_dts, vc_kwargs)
    ...
    if earliest_good_timestamp_st is None:
        raise SignatureValidationError("LTV signatures require a trusted timestamp.")
    ...
    if validation_type == RevocationInfoValidationType.ADOBE_STYLE:
        ...
        if ac_validation_context_kwargs is not None:
            stored_ac_vc = dss.as_validation_context(ac_validation_context_kwargs)
    else:
        ...
        if ac_validation_context_kwargs is not None:
            stored_ac_vc = _instantiate_ltv_vc(latest_dts, ac_validation_context_kwargs)
