...
class DSSContentSettings(GeneralDSSContentSettings):
    ...
    def assert_viable(self):
        ...
        otherwise.
        """
        pre_sign = self.placement == SigDSSPlacementPreference.TOGETHER_WITH_SIGNATURE
    ...
    async def async_timestamp_pdf(
        ...
        *,
        ...
        if bytes_reserved is None:
            test_signature_cms = await timestamper.async_dummy_response(md_algorithm)
...
class PdfSigner:
    ...
    def _retrieve_seed_value_spec(self, sig_field) -> Optional[SigSeedValueSpec]:
    ...
    def register_extensions(self, pdf_out: BasePdfFileWriter, *, md_algorithm: str):
        if self.signature_meta.subfilter == SigSeedSubFilter.PADES:
            _ensure_esic_ext(pdf_out)

        try:
            sig_mech = self.signer.get_signature_mechanism_for_digest(md_algorithm)
        ...
        if sig_algo == "ed25519":
            ...
            if cert and sig_algo == "ecdsa" and _is_iso32002_curve(cert.public_key):
                _ensure_iso32002_ext(pdf_out)
    ...
    def init_signing_session(
        ...
        cms_writer = PdfCMSEmbedder(new_field_spec=self.new_field_spec).write_cms(
        ...
        if sv_spec is not None and sv_spec.timestamp_required and timestamper is None:
    ...
    async def async_digest_doc_for_signing(
        ...
        *,
        ...
        validation_info = await signing_session.perform_presign_validation(pdf_out)
        if bytes_reserved is None:
...
class PdfSigningSession:
    ...
    def __init__(
        ...
        self.system_time = system_time or datetime.now(tz=tzlocal.get_localzone())
    ...
    async def _perform_presign_ac_validation(
        ...
        ac_jobs = [
            async_validate_ac(ac, validation_context, holder_cert=signer.signing_cert)
    ...
    async def _perform_presign_signer_validation(self, validation_context, key_usage):
        signer = self.pdf_signer.signer
        # validate cert
        ...
        try:
            ...
            raise SigningError("The signer's certificate could not be validated") from e
        return signer_cert_validation_path

    async def _perform_prev_ts_validation(self, validation_context, prev_reader):
        signer = self.pdf_signer.signer
    ...
    def _apply_locking_rules(self) -> SigMDPSetup:
        ...
        if sv_spec is not None:
            ...
            sv_lock_values: List[MDPPerm] = sv_lock_lut.get(sv_spec.lock_document, [])
    ...
    def _enforce_seed_value_constraints(
        ...
        if sv_spec.cert is not None:
            ...
            sv_spec.cert.satisfied_by(pdf_signer.signer.signing_cert, validation_path)
        ...
        if (flags & SigSeedValFlags.SUBFILTER) and sv_spec.subfilters is not None:
            ...
            if selected_sf is not None and mandated_sf != selected_sf:
                raise SigningError(
                    ...
                    "but '%s' was requested." % (mandated_sf.value, selected_sf.value)
                )
        ...
        if (flags & SigSeedValFlags.ADD_REV_INFO) and sv_spec.add_rev_info is not None:
    ...
    def prepare_tbs_document(
        ...
        if self.use_pades and signature_meta.embed_validation_info:
            if validation_info is None:
                raise SigningError(
                    "Requested embedding of validation info, " "but none was collected"
                )
...
class PdfTBSDocument:
    ...
    async def async_finish_signing(
        ...
        await post_sign.post_signature_processing(rw_output, chunk_size=chunk_size)
...
class PdfPostSignatureDocument:
    ...
    async def post_signature_processing(
        ...
        if timestamper is not None:
            ...
            if w.security_handler is not None and instr.file_credential is not None:
