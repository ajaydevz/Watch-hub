...
def _cms_version(content_type: Union[str, core.ObjectIdentifier], has_attribute_certs):
...
async def format_signed_attributes(
    ...
    return await format_attributes(attr_provs, dry_run=dry_run, other_attrs=attrs)
    ...
    * :class:`~pyhanko.sign.pkcs11.PKCS11Signer` implements a signer that is
    ...
    def __init__(
        ...
        *,
        ...
        self.unsigned_attr_prov_spec: Optional[UnsignedAttributeProviderSpec] = None
    ...
    def get_signature_mechanism_for_digest(
        ...
        if self.signing_cert is None:
            raise SigningError("Could not set up a default signature mechanism.")
    ...
    def _unsigned_attr_provider_spec(self, timestamper: Optional[TimeStamper] = None):
    ...
    def signer_info(self, digest_algorithm: str, signed_attrs, signature):
        ...
        """
        digest_algorithm_obj = algos.DigestAlgorithm({"algorithm": digest_algorithm})
    ...
    def _package_signature(
        ...
        *,
        ...
        certs.add(cms.CertificateChoices(name="certificate", value=self.signing_cert))
    ...
    def _check_digest_algorithm(self, digest_algorithm: str):
        ...
        try:
            ...
            pass
        if implied_hash_algo is not None and implied_hash_algo != digest_algorithm:
            raise SigningError(
    ...
    async def async_sign(
        ...
        encap_content_info, content_type = _ensure_content_type(encap_content_info)
...
class SimpleSigner(Signer):
    ...
    def sign_raw(self, data: bytes, digest_algorithm: str) -> bytes:
        ...
        if mechanism == "rsassa_pkcs1v15":
            ...
            raise SigningError(
                f"The signature mechanism {mechanism} " "is unsupported by this signer."
            )
    ...
    def load_pkcs12(
        ...
        ca_chain = cls._load_ca_chain(ca_chain_files) if ca_chain_files else set()
        ...
        try:
            ...
            logger.error("Could not load key material from PKCS#12 file", exc_info=e)
            return None
    ...
    def load(
        ...
        other_certs = ca_chain if other_certs is None else ca_chain + other_certs
...
class GenericCMSSignedAttributeProviderSpec(SignedAttributeProviderSpec):
    ...
    def __init__(
        ...
        signing_cert: Optional[x509.Certificate],
        signature_mechanism: Union[Callable[[str], algos.SignedDigestAlgorithm], None],
...
class CAdESSignedAttributeProviderSpec(SignedAttributeProviderSpec):
    ...
    def signed_attr_providers(
        ...
        yield attributes.SigningCertificateV2Provider(signing_cert=self.signing_cert)
        attr_settings = self.attr_settings
