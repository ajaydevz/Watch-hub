...
class PreparedByteRangeDigest:
    ...
    def fill_with_cms(self, output: IO, cms_data: Union[bytes, cms.ContentInfo]):
        """
...
class BuildProps:
    ...
    def as_pdf_object(self) -> generic.DictionaryObject:
        ...
        :return:
            A PDF dictionary.
        """
        props = generic.DictionaryObject({pdf_name("/Name"): pdf_name("/" + self.name)})
        if self.revision:
...
class SignatureObject(PdfSignedData):
    ...
    def __init__(
        ...
        if prop_auth_time:
            self[pdf_name("/Prop_AuthTime")] = generic.NumberObject(prop_auth_time)
        if prop_auth_type:
