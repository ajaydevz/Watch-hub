...
def _process_certificate_info_response(response_data) -> CSCCredentialInfo:
    try:
        ...
        raise SigningError("Could not retrieve certificates from response") from e
    try:
        certs = [x509.Certificate.load(base64.b64decode(cert)) for cert in b64_certs]
    ...
    try:
        ...
        raise SigningError("Could not retrieve max batch size from response") from e
...
class CSCAuthorizationManager(abc.ABC):
    ...
    async def authorize_signature(self, hash_b64s: List[str]) -> CSCAuthorizationInfo:
        """
    ...
    def format_csc_auth_request(
        ...
        """
        result: Dict[str, Any] = {"credentialID": self.csc_session_info.credential_id}
...
class CSCSigner(Signer):
    ...
    async def format_csc_signing_req(
        ...
        auth_info: CSCAuthorizationInfo = await self.auth_manager.authorize_signature(
            tbs_hashes
        )
        ...
        if mechanism["parameters"].native is not None:
            ...
            req_data["signAlgoParams"] = base64.b64encode(params_der).decode("ascii")
    ...
    async def _ensure_batch(self, digest_algorithm) -> _CSCBatchInfo:
        while self._current_batch is not None and self._current_batch.initiated:
            ...
            logger.debug(
                f"Done waiting for commit: " f"new batch: {repr(self._current_batch)})"
            )
