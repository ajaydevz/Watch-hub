...
class AliasTest(BaseTest):
    ...
    def test_target(self):
        self.assertEqual(
            aliases.get("avatar", target="easy_thumbnails_tests.Profile.avatar"),
            {"size": (80, 80), "crop": True},
        )
        self.assertEqual(
            aliases.get("small", target="easy_thumbnails_tests.Profile.avatar"),
            {"size": (20, 20), "crop": True},
        )

    def test_partial_target(self):
        self.assertEqual(
            aliases.get("banner", target="easy_thumbnails_tests.Profile.avatar"),
        ...
        self.assertEqual(aliases.get("banner", target="easy_thumbnails_tests"), None)

    def test_target_fallback(self):
        # Unknown target.
        self.assertEqual(
            aliases.get("small", target="easy_thumbnails_tests.Profile.not_avatar"),
            {"size": (100, 100)},
        )
        # Known target with no matching alias (but a matching global alias).
        self.assertEqual(
            aliases.get("medium", target="easy_thumbnails_tests.Profile.avatar"),
            {"size": (300, 300)},
        )
        # Known target with no matching alias.
        self.assertEqual(
            aliases.get("invalid", target="easy_thumbnails_tests.Profile.avatar"), None
        )
    ...
    def test_all_no_global(self):
        self.assertEqual(
            aliases.all("easy_thumbnails_tests.Profile", include_global=False),
            {
                "banner": {"size": (600, 80), "crop": True},
                "large": {"size": (200, 200)},
            },
        ...
        self.assertEqual(
            aliases.all("easy_thumbnails_tests.Profile.avatar", include_global=False),
            {
                "avatar": {"size": (80, 80), "crop": True},
                "banner": {"size": (600, 80), "crop": True},
                "large": {"size": (200, 200)},
...
class GenerationBase(BaseTest):
    ...
    def tearDown(self):
        ...
        signals.saved_file.disconnect(self.get_signal_handler(), sender=models.Profile)
        super().tearDown()
