...
class ThumbnailTagTest(Base):
    ...
    def testTag(self):
        ...
        self.assertEqual(output, 'src="%s"' % expected_url)
        # as a string:
        output = self.render_template('src="{% thumbnail source strsize %}"')
...
class ThumbnailerFilterTest(ThumbnailerBase):
    def test_get(self):
        src = "{% with t=filename|thumbnailer %}" "{{ t.small.url }}{% endwith %}"
    ...
    def test_relative_name(self):
        src = (
            "{% with t=storage|thumbnailer:filename %}" "{{ t.small.url }}{% endwith %}"
        )
        output = self.render_template(src)
        expected = self.verify_thumbnail(
            (20, 20), settings.THUMBNAIL_ALIASES[""]["small"]
    ...
    def test_invalid(self):
        src = (
            "{% with t=invalid_filename|thumbnailer %}" "{{ t.small.url }}{% endwith %}"
        )
        output = self.render_template(src)
...
class ThumbnailerPassiveFilterTest(ThumbnailerBase):
    def test_check_generate(self):
        src = (
            "{% with t=filename|thumbnailer_passive %}" "{{ t.generate }}{% endwith %}"
        )
        output = self.render_template(src)
    ...
    def test_get_existing(self):
        ...
        src = (
            "{% with t=filename|thumbnailer_passive %}" "{{ t.small.url }}{% endwith %}"
        )
        output = self.render_template(src)
...
class ThumbnailerDataUriTest(ThumbnailerBase):
    def test_data_uri(self):
        src = "{% thumbnail source 25x25 as thumb %}" "{{ thumb|data_uri }}"
