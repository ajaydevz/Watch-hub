...
class ThumbnailCollectionCleaner:
    ...
    def _check_if_exists(self, storage, path):
        try:
            ...
            self.stderr.write(
                "Something went wrong when checking existance of {}:".format(path)
            )
            self.stderr.write(e)
            ...
            if not self._check_if_exists(storage, abs_source_path):
                ...
                for thumb in source.thumbnails.all():
                    ...
                    if self._check_if_exists(storage, abs_thumbnail_path):
                        ...
                        if verbosity > 0:
                            self.stdout.write(
                                "Deleting thumbnail: {}".format(abs_thumbnail_path)
                            )
    ...
    def print_stats(self):
        ...
        """
        self.stdout.write("{0:-<48}".format(datetime.now().strftime("%Y-%m-%d %H:%M ")))
