...
class CFHelpers:
    ...
    def cf_dictionary_to_dict(dictionary):
        ...
        CoreFoundation.CFDictionaryGetKeysAndValues(dictionary, keys, values)
    ...
    def cf_string_to_unicode(value):
        ...
        string_ptr = CoreFoundation.CFStringGetCStringPtr(value, kCFStringEncodingUTF8)
        string = None if is_null(string_ptr) else ffi.string(string_ptr)
        if string is None:
            buffer = buffer_from_bytes(1024)
            result = CoreFoundation.CFStringGetCString(
                value, buffer, 1024, kCFStringEncodingUTF8
            )
            if not result:
    ...
    def cf_number_from_integer(integer):
        ...
        return CoreFoundation.CFNumberCreate(
            CoreFoundation.kCFAllocatorDefault, kCFNumberCFIndexType, integer_as_long
        )
