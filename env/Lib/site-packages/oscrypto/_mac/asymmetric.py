    ...
    def public_key(self):
        ...
        if self._public_key is None:
            ...
            try:
                ...
                result = Security.SecItemExport(
                    self.sec_key_ref, 0, 0, null(), cf_data_private_pointer
                )
                ...
                if key.algorithm == "rsa":
                    public_asn1 = PublicKeyInfo(
                        {
                            "algorithm": PublicKeyAlgorithm(
                                {"algorithm": "rsa", "parameters": Null()}
                            ),
                            "public_key": RSAPublicKey(
                                {
                                    "modulus": key["private_key"].parsed["modulus"],
                                    "public_exponent": key["private_key"].parsed[
                                        "public_exponent"
                                    ],
                                }
                            ),
                    ...
                    public_asn1 = PublicKeyInfo(
                        {
                            "algorithm": PublicKeyAlgorithm(
                                {"algorithm": "dsa", "parameters": params.copy()}
                            ),
                            "public_key": Integer(
                                pow(
                                    params["g"].native,
    ...
    def public_key(self):
        ...
        if not self._public_key and self.sec_certificate_ref:
            if self.asn1.signature_algo == "rsassa_pss":
                ...
                asn1 = self.asn1.copy()
                asn1["tbs_certificate"]["subject_public_key_info"]["algorithm"][
                    "algorithm"
                ] = "rsa"
            ...
            res = Security.SecCertificateCopyPublicKey(
                sec_cert_ref, sec_public_key_ref_pointer
            )
...
def generate_pair(algorithm, bit_size=None, curve=None):
    ...
    try:
        ...
        result = Security.SecKeyCreatePair(
            ...
            SecurityConst.CSSM_KEYATTR_EXTRACTABLE
            | SecurityConst.CSSM_KEYATTR_PERMANENT,
        ...
        result = Security.SecItemExport(
            public_key_ref, 0, 0, null(), cf_data_public_pointer
        )
        handle_sec_error(result)
        ...
        cf_data_private_pointer = new(CoreFoundation, "CFDataRef *")
        result = Security.SecItemExport(
            private_key_ref, 0, 0, null(), cf_data_private_pointer
        )
        handle_sec_error(result)
        cf_data_private = unwrap(cf_data_private_pointer)
        private_key_bytes = CFHelpers.cf_data_to_bytes(cf_data_private)
...
def _load_x509(certificate):
    ...
    try:
        ...
        sec_key_ref = Security.SecCertificateCreateWithData(
            CoreFoundation.kCFAllocatorDefault, cf_source
        )
...
def _load_key(key_object):
    ...
    try:
        ...
        attr_array = CFHelpers.cf_array_from_list([Security.kSecAttrIsExtractable])

        import_export_params_pointer = new(
...
def _encrypt(certificate_or_public_key, data, padding):
    ...
    try:
        ...
        sec_transform = Security.SecEncryptTransformCreate(
            certificate_or_public_key.sec_key_ref, error_pointer
        )
        handle_cf_error(error_pointer)

        if padding:
            Security.SecTransformSetAttribute(
                sec_transform, Security.kSecPaddingKey, padding, error_pointer
            )
            handle_cf_error(error_pointer)
...
def _decrypt(private_key, ciphertext, padding):
    ...
    try:
        ...
        sec_transform = Security.SecDecryptTransformCreate(
            private_key.sec_key_ref, error_pointer
        )
        handle_cf_error(error_pointer)
...
def rsa_pss_verify(certificate_or_public_key, signature, data, hash_algorithm):
    ...
    if not verify_pss_padding(
        hash_algorithm, hash_length, certificate_or_public_key.bit_size, data, plaintext
    ):
...
def _verify(certificate_or_public_key, signature, data, hash_algorithm):
    ...
    if certificate_or_public_key.algorithm == "rsa" and hash_algorithm == "raw":
        ...
        if (
            result == SecurityConst.errSecVerifyFailed
            or result == SecurityConst.errSSLCrypto
        ):
            raise SignatureError("Signature is invalid")
        handle_sec_error(result)
        return
    ...
    try:
        ...
        sec_transform = Security.SecVerifyTransformCreate(
            certificate_or_public_key.sec_key_ref, cf_signature, error_pointer
        )
        handle_cf_error(error_pointer)
        ...
        if hash_algorithm in set(["sha224", "sha256", "sha384", "sha512"]):
            hash_length = {"sha224": 224, "sha256": 256, "sha384": 384, "sha512": 512}[
                hash_algorithm
            ]
...
def _sign(private_key, data, hash_algorithm):
    ...
    try:
        error_pointer = new(CoreFoundation, "CFErrorRef *")
        sec_transform = Security.SecSignTransformCreate(
            private_key.sec_key_ref, error_pointer
        )
        handle_cf_error(error_pointer)
