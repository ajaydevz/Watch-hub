...
def aes_cbc_no_padding_encrypt(key, data, iv):
    ...
    return (
        iv,
        _encrypt(
            Security.kSecAttrKeyTypeAES, key, data, iv, Security.kSecPaddingNoneKey
        ),
    )
...
def aes_cbc_pkcs7_encrypt(key, data, iv):
    ...
    return (
        iv,
        _encrypt(
            Security.kSecAttrKeyTypeAES, key, data, iv, Security.kSecPaddingPKCS7Key
        ),
    )
...
def rc2_cbc_pkcs5_encrypt(key, data, iv):
    ...
    return (
        iv,
        _encrypt(
            Security.kSecAttrKeyTypeRC2, key, data, iv, Security.kSecPaddingPKCS5Key
        ),
    )
...
def tripledes_cbc_pkcs5_encrypt(key, data, iv):
    ...
    return (
        iv,
        _encrypt(
            Security.kSecAttrKeyType3DES, key, data, iv, Security.kSecPaddingPKCS5Key
        ),
    )
...
def des_cbc_pkcs5_encrypt(key, data, iv):
    ...
    return (
        iv,
        _encrypt(
            Security.kSecAttrKeyTypeDES, key, data, iv, Security.kSecPaddingPKCS5Key
        ),
    )
...
def _encrypt(cipher, key, data, iv, padding):
    ...
    try:
        cf_dict = CFHelpers.cf_dictionary_from_pairs(
            [(Security.kSecAttrKeyType, cipher)]
        )
        ...
        if cipher != Security.kSecAttrKeyTypeRC4:
            Security.SecTransformSetAttribute(
                sec_transform, Security.kSecModeCBCKey, null(), error_pointer
            )
            handle_cf_error(error_pointer)
            ...
            Security.SecTransformSetAttribute(
                sec_transform, Security.kSecIVKey, cf_iv, error_pointer
            )
            handle_cf_error(error_pointer)
