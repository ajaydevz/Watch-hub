...
try:
    ...
    setattr(
        CoreFoundation,
        "kCFAllocatorDefault",
        CFAllocatorRef.in_dll(CoreFoundation, "kCFAllocatorDefault"),
    )
    setattr(
        CoreFoundation,
        "kCFBooleanTrue",
        CFTypeRef.in_dll(CoreFoundation, "kCFBooleanTrue"),
    )

    kCFTypeDictionaryKeyCallBacks = c_void_p.in_dll(
...
class CFHelpers:
    ...
    def cf_dictionary_to_dict(dictionary):
        ...
        CoreFoundation.CFDictionaryGetKeysAndValues(
            dictionary, _cast_pointer_p(keys), _cast_pointer_p(values)
        )
    ...
    def cf_string_to_unicode(value):
        ...
        string = CoreFoundation.CFStringGetCStringPtr(
            _cast_pointer_p(value), kCFStringEncodingUTF8
        )
        if string is None:
            buffer = buffer_from_bytes(1024)
            result = CoreFoundation.CFStringGetCString(
                _cast_pointer_p(value), buffer, 1024, kCFStringEncodingUTF8
            )
            if not result:
    ...
    def cf_data_from_bytes(bytes_):
        ...
        return CoreFoundation.CFDataCreate(
            CoreFoundation.kCFAllocatorDefault, bytes_, len(bytes_)
        )
