...
def extract_from_system(cert_callback=None, callback_only_on_failure=False):
    ...
    for domain in [
        SecurityConst.kSecTrustSettingsDomainUser,
        SecurityConst.kSecTrustSettingsDomainAdmin,
    ]:
        cert_trust_settings_pointer_pointer = new(CoreFoundation, "CFArrayRef *")
        res = Security.SecTrustSettingsCopyCertificates(
            domain, cert_trust_settings_pointer_pointer
        )
        ...
        for index in range(0, length):
            cert_pointer = CoreFoundation.CFArrayGetValueAtIndex(
                cert_trust_settings_pointer, index
            )

            trust_settings_pointer_pointer = new(CoreFoundation, "CFArrayRef *")
            res = Security.SecTrustSettingsCopyTrustSettings(
                cert_pointer, domain, trust_settings_pointer_pointer
            )
            ...
            if res == SecurityConst.errSecInvalidTrustSettings:
                ...
                if cert_hash in certificates:
                    _cert_callback(
                        cert_callback, certificates[cert_hash], "invalid trust settings"
                    )
            ...
            for settings_index in range(0, settings_length):
                settings_dict_entry = CoreFoundation.CFArrayGetValueAtIndex(
                    trust_settings_pointer, settings_index
                )
                ...
                policy_oid = settings_dict.get("kSecTrustSettingsPolicy", {}).get(
                    "SecPolicyOid", all_purposes
                )
            ...
            if all_purposes in reject_oids:
                if cert_hash in certificates:
                    _cert_callback(
                        cert_callback, certificates[cert_hash], "explicitly distrusted"
                    )
                    del certificates[cert_hash]
            else:
