...
_cipher_blacklist_regex = re.compile(
        ...
        if self and not self._done_handshake:
            ...
            if len(data) >= 3 and len(self._server_hello) == 0:
                ...
                valid_protocol_version = data[1:3] in set(
                    [b"\x03\x00", b"\x03\x01", b"\x03\x02", b"\x03\x03", b"\x03\x04"]
                )
...
class TLSSocket(object):
        ...
        try:
            if osx_version_info < (10, 8):
                ...
                session_context = Security.SSLCreateContext(
                    null(), SecurityConst.kSSLClientSide, SecurityConst.kSSLStreamType
                )

            result = Security.SSLSetIOFuncs(
                session_context, _read_callback_pointer, _write_callback_pointer
            )
            handle_sec_error(result)
            ...
            result = Security.SSLSetPeerDomainName(
                session_context, utf8_domain, len(utf8_domain)
            )
            handle_sec_error(result)

            if osx_version_info >= (10, 10):
                disable_auto_validation = (
                    self._session._manual_validation or self._session._extra_trust_roots
                )
                explicit_validation = (
                    not self._session._manual_validation
            ...
            supported_cipher_suites_pointer = cast(
                Security, "uint32_t *", cipher_buffer
            )
            ...
            supported_cipher_suites = array_from_pointer(
                Security, "uint32_t", supported_cipher_suites_pointer, supported_ciphers
            )
            good_ciphers = []
            ...
            result = Security.SSLSetEnabledCiphers(
                session_context, good_ciphers_pointer, num_good_ciphers
            )
            handle_sec_error(result)
            ...
            if osx_version_info < (10, 8) and osx_version_info >= (10, 7):
                ...
                do_validation = (
                    explicit_validation
                    and handshake_result == SecurityConst.errSSLServerAuthCompleted
                )

            if do_validation:
                trust_ref_pointer = new(Security, "SecTrustRef *")
                result = Security.SSLCopyPeerTrust(session_context, trust_ref_pointer)
                handle_sec_error(result)
                ...
                ocsp_oid_buffer = buffer_from_bytes(
                    SecurityConst.APPLE_TP_REVOCATION_OCSP
                )
                ...
                result = Security.SecPolicySearchCopyNext(
                    ocsp_search_ref, ocsp_policy_ref_pointer
                )
                handle_sec_error(result)
                ...
                crl_oid_buffer = buffer_from_bytes(
                    SecurityConst.APPLE_TP_REVOCATION_CRL
                )
                ...
                result = Security.SecPolicySearchCopyNext(
                    crl_search_ref, crl_policy_ref_pointer
                )
                handle_sec_error(result)
                ...
                policy_array_ref = CFHelpers.cf_array_from_list(
                    [ssl_policy_ref, crl_policy_ref, ocsp_policy_ref]
                )
            ...
            if handshake_result in handshake_error_codes:
                ...
                if chain:
                    ...
                    no_issuer = (
                        not self_signed
                        and result_code == SecurityConst.CSSMERR_TP_NOT_TRUSTED
                    )
                    expired = result_code == SecurityConst.CSSMERR_TP_CERT_EXPIRED
                    not_yet_valid = (
                        result_code == SecurityConst.CSSMERR_TP_CERT_NOT_VALID_YET
                    )
                    bad_hostname = (
                        result_code == SecurityConst.CSSMERR_APPLETP_HOSTNAME_MISMATCH
                    )
                    validity_too_long = (
                        result_code == SecurityConst.CSSMERR_TP_CERT_SUSPENDED
                    )
            ...
            if handshake_result in set(
                [SecurityConst.errSSLRecordOverflow, SecurityConst.errSSLProtocol]
            ):
                self._server_hello += _read_remaining(self._socket)
                raise_protocol_error(self._server_hello)

            if handshake_result in set(
                [SecurityConst.errSSLClosedNoNotify, SecurityConst.errSSLClosedAbort]
            ):
            ...
            server_auth_complete = (
                handshake_result == SecurityConst.errSSLServerAuthCompleted
            )
            manual_validation = (
                self._session._manual_validation and server_auth_complete
            )
            ...
            result = Security.SSLGetNegotiatedCipher(
                session_context, cipher_int_pointer
            )
            handle_sec_error(result)
            ...
            session_info = parse_session_info(self._server_hello, self._client_hello)
    ...
    def read(self, max_length):
        ...
        result = Security.SSLRead(
            self._session_context, read_buffer, to_read, processed_pointer
        )
        if self._exception is not None:
            exception = self._exception
            self._exception = None
            raise exception
        if result and result not in set(
            [SecurityConst.errSSLWouldBlock, SecurityConst.errSSLClosedGraceful]
        ):
    ...
    def _os_buffered_size(self):
        ...
        result = Security.SSLGetBufferedReadSize(
            self._session_context, num_bytes_pointer
        )
        handle_sec_error(result)
    ...
    def write(self, data):
        ...
        while data_len:
            ...
            result = Security.SSLWrite(
                self._session_context, write_buffer, data_len, processed_pointer
            )
            if self._exception is not None:
                exception = self._exception
                self._exception = None
    ...
    def _read_certificates(self):
        ...
        try:
            trust_ref_pointer = new(Security, "SecTrustRef *")
            result = Security.SSLCopyPeerTrust(self._session_context, trust_ref_pointer)
            handle_sec_error(result)
