...
def parse_session_info(server_handshake_bytes, client_handshake_bytes):
    ...
    for record_type, _, record_data in parse_tls_records(server_handshake_bytes):
        ...
        for message_type, message_data in parse_handshake_messages(record_data):
            ...
            compression = (
                message_data[compression_start : compression_start + 1] != b"\x00"
            )
...
def parse_handshake_messages(data):
    ...
    while pointer < data_len:
        ...
        yield (data[pointer : pointer + 1], data[pointer + 4 : pointer + 4 + length])
        pointer += 4 + length
...
def _parse_hello_extensions(data):
    ...
    while pointer < extensions_end:
        ...
        yield (extension_type, data[pointer + 4 : pointer + 4 + extension_length])
...
def raise_hostname(certificate, hostname):
    ...
    is_ip = (
        re.match("^\\d+\\.\\d+\\.\\d+\\.\\d+$", hostname) or hostname.find(":") != -1
    )
    if is_ip:
    ...
    message = (
        "Server certificate verification failed - %s does not match" % hostname_type
    )
...
def raise_lifetime_too_long(certificate):
    ...
    message = (
        "Server certificate verification failed - certificate lifetime is too long"
    )
    raise TLSVerificationError(message, certificate)
...
def raise_expired_not_yet_valid(certificate):
    ...
    if not_before > now:
        ...
        message = (
            "Server certificate verification failed - certificate not valid until %s"
            % formatted_before
        )
    elif not_after < now:
        formatted_after = not_after.strftime("%Y-%m-%d %H:%M:%SZ")
        message = (
            "Server certificate verification failed - certificate expired %s"
            % formatted_after
        )
...
def raise_protocol_error(server_handshake_bytes):
    ...
    if other_protocol:
        raise TLSError(
            "TLS protocol error - server responded using %s" % other_protocol
        )
...
def detect_other_protocol(server_handshake_bytes):
    ...
    if (
        server_handshake_bytes[0:4] == b"* OK"
        or server_handshake_bytes[0:9] == b"* PREAUTH"
    ):
        return "IMAP"
