...
class TLSSession(object):
    ...
    def _obtain_credentials(self):
        ...
        flags = (
            Secur32Const.SCH_USE_STRONG_CRYPTO | Secur32Const.SCH_CRED_NO_DEFAULT_CREDS
        )
...
class TLSSocket(object):
    ...
    def _extra_trust_root_validation(self):
        ...
        try:
            ...
            usage_identifiers[0] = cast(
                crypt32, "char *", Crypt32Const.PKIX_KP_SERVER_AUTH
            )
            usage_identifiers[1] = cast(
                crypt32, "char *", Crypt32Const.SERVER_GATED_CRYPTO
            )
            ...
            cert_enhkey_usage.rgpszUsageIdentifier = cast(
                crypt32, "char **", usage_identifiers
            )
            ...
            result = crypt32.CertGetCertificateChain(
                ...
                | Crypt32Const.CERT_CHAIN_REVOCATION_CHECK_CACHE_ONLY,
                null(),
                cert_chain_context_pointer_pointer,
            )
            handle_crypt32_error(result)

            cert_chain_policy_para_flags = (
                Crypt32Const.CERT_CHAIN_POLICY_IGNORE_ALL_REV_UNKNOWN_FLAGS
            )
            ...
            if num_chains == 1:
                ...
                if last_cert.sha256 in cert_hashes:
                    cert_chain_policy_para_flags |= (
                        Crypt32Const.CERT_CHAIN_POLICY_ALLOW_UNKNOWN_CA_FLAG
                    )

            ssl_extra_cert_chain_policy_para_pointer = struct(
                crypt32, "SSL_EXTRA_CERT_CHAIN_POLICY_PARA"
            )
            ssl_extra_cert_chain_policy_para = unwrap(
                ssl_extra_cert_chain_policy_para_pointer
            )
            ...
            ssl_extra_cert_chain_policy_para.pwszServerName = cast(
                crypt32, "wchar_t *", buffer_from_unicode(self._hostname)
            )
            ...
            cert_chain_policy_para.pvExtraPolicyPara = cast(
                crypt32, "void *", ssl_extra_cert_chain_policy_para_pointer
            )

            cert_chain_policy_status_pointer = struct(
                crypt32, "CERT_CHAIN_POLICY_STATUS"
            )
            ...
            cert_data = bytes_from_buffer(
                cert_context.pbCertEncoded, native(int, cert_context.cbCertEncoded)
            )
    ...
    def _handshake(self, renegotiate=False):
        ...
        try:
            ...
            if result not in set(
                [Secur32Const.SEC_E_OK, Secur32Const.SEC_I_CONTINUE_NEEDED]
            ):
            ...
            if out_buffers[0].cbBuffer > 0:
                token = bytes_from_buffer(
                    out_buffers[0].pvBuffer, out_buffers[0].cbBuffer
                )
            ...
            while result != Secur32Const.SEC_E_OK:
                ...
                if result == Secur32Const.SEC_E_INVALID_TOKEN:
                    ...
                    if out_buffers[1].cbBuffer > 0:
                        alert_bytes = bytes_from_buffer(
                            out_buffers[1].pvBuffer, out_buffers[1].cbBuffer
                        )
                        handshake_client_bytes += alert_bytes
                        alert_number = alert_bytes[6:7]
                        if alert_number == b"\x28" or alert_number == b"\x2b":
                            if (
                                "TLSv1.2" in self._session._protocols
                                and len(self._session._protocols) > 1
                            ):
                                chain = extract_chain(handshake_server_bytes)
                ...
                if (
                    result == Secur32Const.SEC_E_BUFFER_TOO_SMALL
                    or result == Secur32Const.SEC_E_MESSAGE_ALTERED
                ):
            ...
            if not renegotiate:
                ...
                self._buffer_size = (
                    self._header_size + self._message_size + self._trailer_size
                )
    ...
    def read(self, max_length):
        ...
        if not self._decrypt_data_buffer:
            ...
            self._decrypt_buffers[0].pvBuffer = cast(
                secur32, "BYTE *", self._decrypt_data_buffer
            )
        ...
        while output_len < max_length:
            ...
            result = secur32.DecryptMessage(
                self._context_handle_pointer, self._decrypt_desc, 0, null()
            )
    ...
    def write(self, data):
        ...
        if not self._encrypt_data_buffer:
            self._encrypt_data_buffer = buffer_from_bytes(
                self._header_size + self._message_size + self._trailer_size
            )
            ...
            self._encrypt_buffers[0].pvBuffer = cast(
                secur32, "BYTE *", self._encrypt_data_buffer
            )

            self._encrypt_buffers[1].BufferType = Secur32Const.SECBUFFER_DATA
            self._encrypt_buffers[1].pvBuffer = ref(
                self._encrypt_data_buffer, self._header_size
            )
            ...
            self._encrypt_buffers[2].pvBuffer = ref(
                self._encrypt_data_buffer, self._header_size + self._message_size
            )

        while len(data) > 0:
            to_write = min(len(data), self._message_size)
            write_to_buffer(
                self._encrypt_data_buffer, data[0:to_write], self._header_size
            )

            self._encrypt_buffers[1].cbBuffer = to_write
            self._encrypt_buffers[2].pvBuffer = ref(
                self._encrypt_data_buffer, self._header_size + to_write
            )

            result = secur32.EncryptMessage(
                self._context_handle_pointer, 0, self._encrypt_desc, 0
            )
    ...
    def shutdown(self):
        ...
        try:
            ...
            if _win_version_info >= (6, 2):
                ...
                buffers[0].pvBuffer = cast(
                    secur32, "BYTE *", buffer_from_bytes(b"\x01\x00\x00\x00")
                )
                ...
                result = secur32.ApplyControlToken(
                    self._context_handle_pointer, sec_buffer_desc_pointer
                )
                handle_error(result, TLSError)
    ...
    def _read_certificates(self):
        ...
        try:
            ...
            while not is_null(context_pointer):
                ...
                data = bytes_from_buffer(
                    context.pbCertEncoded, native(int, context.cbCertEncoded)
                )
                ...
                context_pointer = crypt32.CertEnumCertificatesInStore(
                    store_handle, context_pointer
                )
