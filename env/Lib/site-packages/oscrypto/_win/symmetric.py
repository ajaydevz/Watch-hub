...
def _advapi32_create_handles(cipher, key, iv):
    ...
    res = advapi32.CryptImportKey(
        context_handle, blob, len(blob), null(), flags, key_handle_pointer
    )
    handle_error(res)
    ...
    if cipher == "rc2":
        ...
        res = advapi32.CryptSetKeyParam(
            key_handle, Advapi32Const.KP_EFFECTIVE_KEYLEN, buf, 0
        )
        handle_error(res)
    ...
    try:
        ...
        if cipher == "rc2":
            ...
            res = bcrypt.BCryptSetProperty(
                alg_handle, BcryptConst.BCRYPT_EFFECTIVE_KEY_LENGTH, buf, 4, 0
            )
            handle_error(res)
...
def _advapi32_encrypt(cipher, key, data, iv, padding):
    ...
    try:
        ...
        res = advapi32.CryptEncrypt(key_handle, null(), True, 0, null(), out_len, 0)
        handle_error(res)
        ...
        res = advapi32.CryptEncrypt(
            key_handle, null(), True, 0, buffer, out_len, buffer_len
        )
        handle_error(res)
...
def _bcrypt_encrypt(cipher, key, data, iv, padding):
    ...
    try:
        ...
        res = bcrypt.BCryptEncrypt(
            key_handle, data, len(data), null(), null(), 0, null(), 0, out_len, flags
        )
        handle_error(res)
...
def _advapi32_decrypt(cipher, key, data, iv, padding):
    ...
    try:
        ...
        if cipher == "aes" and not padding and len(data) % 16 != 0:
            raise ValueError(
                "Invalid data - ciphertext length must be a multiple of 16"
            )
