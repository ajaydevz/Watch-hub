...
_module_values = {"backend": None, "backend_config": None, "ffi": None}
...
def use_openssl(libcrypto_path, libssl_path, trust_list_path=None):
    ...
    if not isinstance(libcrypto_path, str_cls):
        raise ValueError(
            "libcrypto_path must be a unicode string, not %s"
            % type_name(libcrypto_path)
        )

    if not isinstance(libssl_path, str_cls):
        raise ValueError(
            "libssl_path must be a unicode string, not %s" % type_name(libssl_path)
        )
    ...
    if trust_list_path is not None:
        if not isinstance(trust_list_path, str_cls):
            raise ValueError(
                "trust_list_path must be a unicode string, not %s"
                % type_name(trust_list_path)
            )
    ...
    with _backend_lock:
        ...
        if (
            _module_values["backend"] == "openssl"
            and _module_values["backend_config"] == new_config
        ):
            return

        if _module_values["backend"] is not None:
            raise RuntimeError(
                "Another part of oscrypto has already been imported, unable to force use of OpenSSL"
            )
...
def use_winlegacy():
    ...
    if sys.platform != "win32":
        ...
        raise EnvironmentError(
            "The winlegacy backend can only be used on Windows, not %s" % plat
        )
