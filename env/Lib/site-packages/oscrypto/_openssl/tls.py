        ...
        try:
            ...
            if sys.platform in set(["win32", "darwin"]):
                ...
                result = libssl.SSL_CTX_load_verify_locations(
                    ssl_ctx, trust_list_path.encode(path_encoding), null()
                )
            ...
            verify_mode = (
                LibsslConst.SSL_VERIFY_NONE
                if manual_validation
                else LibsslConst.SSL_VERIFY_PEER
            )
            ...
            if self._extra_trust_roots:
                ...
                for cert in self._extra_trust_roots:
                    ...
                    result = libssl.X509_STORE_add_cert(x509_store, oscrypto_cert.x509)
                    handle_openssl_error(result)
        ...
        try:
            ...
            while True:
                ...
                if error == LibsslConst.SSL_ERROR_WANT_READ:
                    ...
                    if (
                        info == dh_key_info_1
                        or info == dh_key_info_2
                        or info == dh_key_info_3
                    ):
                        raise_dh_params()
                    ...
                    if libcrypto_version_info < (1, 1):
                        ...
                        unknown_protocol_info = _homogenize_openssl3_error(
                            unknown_protocol_info
                        )
                    ...
                    tls_version_info_error = _homogenize_openssl3_error(
                        tls_version_info_error
                    )
                    ...
                    handshake_failure_info = _homogenize_openssl3_error(
                        handshake_failure_info
                    )
                    ...
                    unknown_hash_algo_info = _homogenize_openssl3_error(
                        unknown_hash_algo_info
                    )
                    ...
                    if info == cert_verify_failed_info:
                        ...
                        if oscrypto_cert and oscrypto_cert.asn1.hash_algo in set(
                            ["md5", "md2"]
                        ):
                            raise_weak_signature(oscrypto_cert)
