...
try:
    ...
    version_string = vffi.string(vffi.dlopen(libcrypto_path).SSLeay_version(0)).decode(
        "utf-8"
    )
except AttributeError:
    vffi = FFI()
    vffi.cdef("const char *OpenSSL_version(int type);")
    version_string = vffi.string(vffi.dlopen(libcrypto_path).OpenSSL_version(0)).decode(
        "utf-8"
    )
