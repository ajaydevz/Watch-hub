...
class PrivateKey(_PrivateKeyBase):
    ...
    def public_key(self):
        ...
        if self._public_key is None:
            ...
            pub_evp_pkey = libcrypto.d2i_PUBKEY(
                null(), buffer_pointer(pubkey_buffer), pubkey_length
            )
...
class Certificate(_CertificateBase):
    ...
    def public_key(self):
        ...
        if not self._public_key and self.x509:
            ...
            # masquerade it as a normal RSA key so the OID checks work
            if (
                libcrypto_version_info < (3,)
                and self.asn1.public_key.algorithm == "rsassa_pss"
            ):
    ...
    if algorithm == "rsa":
        ...
        try:
            ...
            result = libcrypto.DSA_generate_parameters_ex(
                dsa, bit_size, null(), 0, null(), null(), null()
            )
            handle_openssl_error(result)
        ...
        try:
            ...
            libcrypto.EC_KEY_set_asn1_flag(
                ec_key, LibcryptoConst.OPENSSL_EC_NAMED_CURVE
            )
...
def generate_dh_parameters(bit_size):
    ...
    try:
        ...
        result = libcrypto.DH_generate_parameters_ex(
            dh, bit_size, LibcryptoConst.DH_GENERATOR_2, null()
        )
        handle_openssl_error(result)
...
def _load_key(private_object):
    ...
    if (
        libcrypto_version_info < (1,)
        and private_object.algorithm == "dsa"
        and private_object.hash_algo == "sha2"
    ):
        raise AsymmetricKeyError(
            pretty_message(
                """
...
def rsa_oaep_encrypt(certificate_or_public_key, data):
    ...
    return _encrypt(
        certificate_or_public_key, data, LibcryptoConst.RSA_PKCS1_OAEP_PADDING
    )
...
def _decrypt(private_key, ciphertext, padding):
    ...
    try:
        ...
        res = libcrypto.RSA_private_decrypt(
            len(ciphertext), ciphertext, buffer, rsa, padding
        )
        handle_openssl_error(res)
    ...
    try:
        ...
        if libcrypto_version_info < (1,):
            if cp_is_rsa and rsa_pss_padding:
                ...
                dsa_sig = libcrypto.d2i_DSA_SIG(
                    null(), signature_pointer, len(signature)
                )
                if is_null(dsa_sig):
                    raise SignatureError("Signature is invalid")
                ...
                ec_key = libcrypto.EVP_PKEY_get1_EC_KEY(
                    certificate_or_public_key.evp_pkey
                )
                if is_null(ec_key):
                    handle_openssl_error(0)
            ...
            if rsa_pss_padding:
                ...
                if libcrypto_version_info < (3, 0):
                    res = libcrypto.EVP_PKEY_CTX_ctrl(
                        evp_pkey_ctx_pointer,
                        LibcryptoConst.EVP_PKEY_RSA,
                        LibcryptoConst.EVP_PKEY_OP_SIGN
                        | LibcryptoConst.EVP_PKEY_OP_VERIFY,
    ...
    if pkey_is_rsa and hash_algorithm == "raw":
        ...
        try:
            ...
            signature_length = libcrypto.RSA_private_encrypt(
                len(data), data, signature_buffer, rsa, LibcryptoConst.RSA_PKCS1_PADDING
            )
        ...
        if libcrypto_version_info < (1,):
            if pkey_is_rsa and rsa_pss_padding:
                ...
                res = libcrypto.EVP_SignFinal(
                    evp_md_ctx, signature_buffer, signature_length, private_key.evp_pkey
                )
                handle_openssl_error(res)
            ...
            res = libcrypto.EVP_DigestSignFinal(
                evp_md_ctx, signature_buffer, signature_length
            )
            handle_openssl_error(res)
