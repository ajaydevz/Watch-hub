...
def _encrypt(cipher, key, data, iv, padding):
    ...
    try:
        ...
        if cipher in set(["rc2", "rc4"]):
            res = libcrypto.EVP_EncryptInit_ex(
                evp_cipher_ctx, evp_cipher, null(), null(), null()
            )
        ...
        res = libcrypto.EVP_EncryptUpdate(
            evp_cipher_ctx, buffer, output_length, data, len(data)
        )
        handle_openssl_error(res)
...
def _decrypt(cipher, key, data, iv, padding):
    ...
    try:
        ...
        if cipher in set(["rc2", "rc4"]):
            res = libcrypto.EVP_DecryptInit_ex(
                evp_cipher_ctx, evp_cipher, null(), null(), null()
            )
            handle_openssl_error(res)
            res = libcrypto.EVP_CIPHER_CTX_set_key_length(evp_cipher_ctx, len(key))
        ...
        res = libcrypto.EVP_DecryptUpdate(
            evp_cipher_ctx, buffer, output_length, data, len(data)
        )
        handle_openssl_error(res)
