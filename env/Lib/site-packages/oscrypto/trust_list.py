...
_module_values = {"last_update": None, "certs": None}
                ...
                with open(ca_path, "wb") as f:
                    for cert, trust_oids, reject_oids in extract_from_system(
                        ...
                        if sys.platform == "darwin":
                            if (
                                trust_oids != empty_set
                                and any_purpose not in trust_oids
                                and apple_ssl not in trust_oids
                            ):
                                if cert_callback:
                                    cert_callback(
                                        Certificate.load(cert),
                                        "implicitly distrusted for TLS",
                                    )
                                continue
                            if reject_oids != empty_set and (
                                apple_ssl in reject_oids or any_purpose in reject_oids
                            ):
                                if cert_callback:
                                    cert_callback(
                                        Certificate.load(cert),
                                        "explicitly distrusted for TLS",
                                    )
                                continue
                            ...
                            if (
                                ...
                                continue
                            if reject_oids != empty_set and (
                                win_server_auth in reject_oids
        ...
        with memory_lock:
            if not _in_memory_up_to_date(cache_length):
                ...
                for cert_bytes, trust_oids, reject_oids in extract_from_system(
                    ...
                    certs.append(
                        (Certificate.load(cert_bytes), trust_oids, reject_oids)
                    )
...
def _in_memory_up_to_date(cache_length):
    ...
    return (
        _module_values["certs"]
        and _module_values["last_update"]
        and _module_values["last_update"] > time.time() - (cache_length * 60 * 60)
    )
