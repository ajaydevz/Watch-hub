...
def parse_private(data, password=None):
    ...
    try:
        ...
        decrypted_data = _decrypt_encrypted_data(
            encryption_algorithm_info, encrypted_data, password
        )
...
def _unarmor_pem(data, password=None):
    ...
    if pem_header in set(["RSA PRIVATE KEY", "DSA PRIVATE KEY", "EC PRIVATE KEY"]):
        ...
        return (
            "private key",
            algo,
            _unarmor_pem_openssl_private(headers, der_bytes, password),
        )
...
def _parse_pkcs12(data, password, load_private_key):
    ...
    if mac_data:
        ...
        computed_hmac = hmac.new(
            mac_key, auth_safe["content"].contents, hash_mod
        ).digest()
    ...
    for content_info in authenticated_safe:
        ...
        if isinstance(content, OctetString):
            _parse_safe_contents(
                content.native, certs, private_keys, password, load_private_key
            )
            ...
            decrypted_content = _decrypt_encrypted_data(
                encryption_algorithm_info, encrypted_content, password
            )

            _parse_safe_contents(
                decrypted_content, certs, private_keys, password, load_private_key
            )
    ...
    if len(certs) > 0:
        other_certs = sorted(
            list(certs.values()), key=lambda c: c.subject.human_friendly
        )

    return (key, cert, other_certs)


def _parse_safe_contents(
    safe_contents, certs, private_keys, password, load_private_key
):
    """
    ...
    for safe_bag in safe_contents:
        ...
        if isinstance(bag_value, CertBag):
            if bag_value["cert_id"].native == "x509":
                ...
                certs[_fingerprint(public_key_info, None)] = bag_value[
                    "cert_value"
                ].parsed
            ...
            decrypted_key_bytes = _decrypt_encrypted_data(
                encryption_algorithm_info, encrypted_key_bytes, password
            )
            ...
            _parse_safe_contents(
                bag_value, certs, private_keys, password, load_private_key
            )
