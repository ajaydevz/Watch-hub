...
CLOBBERID = (
    0  # set a constant Doc ID to allow comparison with other software like iText
)
...
class StandardEncryption:
    ...
    def prepare(self, document, overrideID=None):
        ...
        if self.revision == 5:
            ...
            self.key = encryptionkey(
                self.userPassword, self.O, self.P, internalID, revision=self.revision
            )
            if DEBUG:
                print("self.key (hex)  = %s" % hexText(self.key))
    ...
    def info(self):
        ...
        if not self.prepared:
            raise ValueError("encryption not prepared!")
        return StandardEncryptionDictionary(
            O=self.O,
            OE=self.OE,
            U=self.U,
            UE=self.UE,
    ...
    if DEBUG:
        print(
            ...
            % tuple(
                [
                    hexText(str(x))
                    for x in (encryptionkey, encodestring, revision, documentId, result)
                ]
            )
        )
    return result
...
"""
    ...
    if len(argv) > 0:
            ...
            if thisarg[0] in argv:
                ...
                if thisarg[0] in binaryrequired:
                    if argv[pos + 1] not in ("1", "0"):
                        raise ValueError(
                            "%s value must be either '1' or '0'!" % thisarg[1]
                        )
                try:
        ...
        if verbose:
            print(
                "wrote output file '%s'(%s bytes)\n  owner password is '%s'\n  user password is '%s'"
                % (SAVEFILE, filesize, OWNER, USER)
            )

        if len(argv) > 0:
            raise ValueError(
                "\nUnrecognised arguments : %s\nknown arguments are:\n%s"
                % (str(argv)[1:-1], known_modes)
            )
    else:
        print(usage)
