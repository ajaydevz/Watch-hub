...
try:
    ...
    def parsexml(
        xmlText, oneOutermostTag=0, eoCB=None, entityReplacer=None, parseOpts={}
    ):
...
def parsexmlSimple(
    xmltext, oneOutermostTag=0, eoCB=None, entityReplacer=unEscapeContentList
):
...
def parsexml0(
    ...
    if firstbracket < 0:
        ...
        if toplevel is not None:
            ...
            raise ValueError(
                "no tags at non-toplevel %s" % repr(xmltext[cursor : cursor + 20])
            )
    # D = {}
    L = []
    # look for start tag
    ...
    if toplevel is not None:
        ...
        raise ValueError(
            "non top level entry should be at start tag: %s" % repr(xmltext[:10])
        )
    # special case: CDATA
    elif (
        afterbracket2char == "!["
        and xmltext[firstbracket : firstbracket + 9] == "<![CDATA["
    ):
    ...
    # special case COMMENT
        ...
        afterbracket2char == "!-" and xmltext[firstbracket : firstbracket + 4] == "<!--"
    ):
        # print "in COMMENT"
        endcommentdashes = xmltext.find("--", firstbracket + 4)
        if endcommentdashes < firstbracket:
            raise ValueError(
                "unterminated comment %s" % repr(xmltext[cursor : cursor + 20])
            )
        endcomment = endcommentdashes + 2
        if xmltext[endcomment] != ">":
            raise ValueError(
                "invalid comment: contains double dashes %s"
                % repr(xmltext[cursor : cursor + 20])
            )
        ...
        if "=" not in tagcontent:
            ...
            if noclose:
                raise ValueError(
                    "unclosed start tag %s"
                    % repr(xmltext[firstbracket : firstbracket + 20])
                )
            cursor = startsearch
            ...
            while taglistindex < lasttaglistindex:
                ...
                if attentry[0] != '"':
                    raise ValueError(
                        "attribute value must start with double quotes" + repr(attentry)
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        "attvalue,attentry,attlist="
                        + repr((attvalue, attentry, attlist))
                    )
    ...
    while docontents is not None:
        ...
        if nextopenbracket < cursor:
            ...
            if name == NONAME:
                ...
                raise ValueError(
                    "no close bracket for %s found after %s"
                    % (name, repr(xmltext[cursor : cursor + 20]))
                )
        # is it a close bracket?
            ...
            if nextclosebracket < nextopenbracket:
                raise ValueError(
                    "unclosed close tag %s"
                    % repr(xmltext[nextopenbracket : nextopenbracket + 20])
                )
            ...
            if name != closename:
                ...
                raise ValueError(
                    ...
                    % (
                        ...
                        repr(name),
                        repr(closename),
                        repr(xmltext[cursor : cursor + 100]),
                    )
                )
        ...
        # first record any intervening content
            ...
            (parsetree, cursor) = parsexml0(
                xmltext,
                startingat=nextopenbracket,
                toplevel=None,
...
def test(dump=0):
    testparse(
        """<this type="xml">text &lt;&gt;<b>in</b> <funnytag foo="bar"/> xml</this>
                 <!-- comment -->
                 <![CDATA[
                 <this type="xml">text <b>in</b> xml</this> ]]>
