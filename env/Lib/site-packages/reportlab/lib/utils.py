...
def char2int(s):
    return (
        s
        if isinstance(s, int)
        else ord(s if isinstance(s, str) else s.decode("latin1"))
    )
        ...
        if c or r == []:
            r += list(
                map(
                    ...
                    list(
                        filter(
                            lambda x, pfn=pfn, fnmatch=fnmatch: fnmatch(x, pfn),
                            list(__rl_loader__._files.keys()),
                        )
                    ),
                )
            )
    ...
    try:
        ...
        if debug:
            ...
            print(
                "+++++ os.paths.isfile(%s)=%s"
                % (
                    ascii("./%s.py" % modulename),
                    ascii(os.path.isfile("./%s.py" % modulename)),
                )
            )
...
def open_for_read(name, mode="b"):
    ...
    if trustedHosts:
        ...
        trustedHosts = re.compile(
            "".join(("^(?:", "|".join(map(xre, trustedHosts)), ")\\Z"))
        )
        ...
        try:
            ...
            try:
                if trustedHosts is not None:
                    purl = urlparse(name)
                    if purl[0] and not (
                        (purl[0] in ("data", "file") or trustedHosts.match(purl[1]))
                        and (purl[0] in trustedSchemes)
                    ):
                        raise ValueError("Attempted untrusted host access")
                return BytesIO(
                    (datareader if name[:5].lower() == "data:" else rlUrlRead)(name)
                )
            except:
...
def rl_isdir(pn, os_path_isdir=os.path.isdir, os_path_normpath=os.path.normpath):
    ...
    return (
        len(
            list(
                filter(
                    lambda x, pn=pn: x.startswith(pn), list(__rl_loader__._files.keys())
                )
            )
        )
        > 0
...
def rl_getmtime(
    ...
    if os_path_isfile(pn) or _isFSD or __rl_loader__ is None:
        ...
        try:
            ...
            if isCompactDistro():
                ...
                dir = (
                    (dir == "." or not dir)
                    and _archive
                    or os.path.join(_archive, dir.replace("/", os.sep))
                )
...
class ImageReader:
        ...
        if _isPILImage(fileName):
            ...
            try:
                ...
                if imageReaderFlags != 0:
                    raise ValueError(
                        "imageReaderFlags values other than 0 are no longer supported; all images are interned now"
                    )
    ...
    def identity(self):
        ...
        return "[%s@%s%s%s]" % (
            ...
            hex(id(self)),
            ident and (" ident=%r" % ident) or "",
            fn and (" filename=%r" % fn) or "",
        )
    ...
    def check_pil_image_size(cls, im):
        ...
        if size > max_image_size:
            raise MemoryError(
                "PIL %s %s x %s image would use %s > %s bytes"
...
class DebugMemo:
        ...
        **kw
        ...
        for n, m in sys.modules.items():
            if (
                ...
                v = [
                    getattr(m, x, None) for x in ("__version__", "__path__", "__file__")
                ]
    ...
    specials = {
        "__module_versions": _show_module_versions,
...
def commajoin(l):
    ...
    >>> commasplit((commajoin([u'a ', u',b', u'c'])) == [u'a', u',b', u'c']
    """
    return ",".join([" " + asUnicode(i).replace(",", ",,") + " " for i in l])
    ...
    def datetime(self):
        if self.tzname:
            return datetime.datetime.fromtimestamp(
                self.t, FixedOffsetTZ(self.dhh, self.dmm, self.tzname)
            )
        else:
