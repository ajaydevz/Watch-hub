    ...
    def __repr__(self):
        return "%s(%s%s%s%s%s)" % (
            ...
            (
                ...
                and (",alpha=%s" % (fp_str(self.alpha * 100)))
                or ""
            ),
        )
...
class CMYKColorSep(CMYKColor):
    ...
    def __init__(
        self, cyan=0, magenta=0, yellow=0, black=0, spotName=None, density=1, alpha=1
    ):
        CMYKColor.__init__(
            self,
            cyan,
...
def HexColor(val, htmlOnly=False, hasAlpha=False):
    ...
    return Color(
        ((val >> 16) & 0xFF) / 255.0, ((val >> 8) & 0xFF) / 255.0, (val & 0xFF) / 255.0
    )
...
def linearlyInterpolatedColor(c0, c1, x0, x1, x):
    ...
    if c0.__class__ != c1.__class__:
        raise ValueError(
            "Color classes must be the same for interpolation!\nGot %r and %r'"
            % (c0, c1)
        )
    if x1 < x0:
    ...
    if x < x0 - 1e-8 or x > x1 + 1e-8:  # fudge factor for numerical problems
        raise ValueError(
            "Can't interpolate: x=%f is not between %f and %f!" % (x, x0, x1)
        )
    if x <= x0:
        return c0
    elif x >= x1:
    ...
    if cname == "Color":  # RGB
        ...
        if cmykDistance(c0, c1) < 1e-8:
            ...
            a = c0.alpha + x * (c1.alpha - c0.alpha) / dx
            return PCMYKColor(
                c * 100, m * 100, y * 100, k * 100, density=d * 100, alpha=a * 100
            )
    else:
...
class cssParse:
    def pcVal(self, v):
        ...
        try:
            ...
            raise ValueError(
                "bad percentage argument value %r in css color %r" % (v, self.s)
            )
        return c
    ...
    def alphaVal(self, v, c=1, n="alpha"):
        try:
            ...
            raise ValueError(
                "bad %s argument value %r in css color %r" % (n, v, self.s)
            )

    _n_c = dict(
        pcmyk=(4, 100, True, False),
        cmyk=(4, 1, True, False),
        hsl=(3, 1, False, True),
        rgb=(3, 1, False, False),
    )
...
class toColor:
    ...
    def __call__(self, arg, default=None):
        ...
        if isinstance(arg, (tuple, list)):
            ...
            return (
                len(arg) == 3
                and Color(arg[0], arg[1], arg[2])
                or CMYKColor(arg[0], arg[1], arg[2], arg[3])
            )
        elif isStr(arg):
            arg = asNative(arg)
            C = cssParse(arg)
            ...
            try:
                ...
                expr = ast.literal_eval(
                    arg
                )  # safe probably only a tuple or list of values
                return toColor(expr)
            ...
            if rl_config.toColorCanUse == "rl_safe_eval":
                ...
                if not self._G:
                    ...
                    self._G = {
                        s: C[s]
                        for s in """Blacker CMYKColor CMYKColorSep Color ColorType HexColor PCMYKColor PCMYKColorSep Whiter
                ...
                C = {
                    k: C[k]
                    for k in """Blacker CMYKColor CMYKColorSep Color ColorType HexColor PCMYKColor PCMYKColorSep Whiter
                        _chooseEnforceColorSpace _enforceCMYK _enforceError _enforceRGB _enforceSEP _enforceSEP_BLACK
                        _enforceSEP_CMYK _namedColors _re_css asNative cmyk2rgb cmykDistance color2bw colorDistance
...
def _enforceSEP_BLACK(c):
    ...
    if not isinstance(tc, CMYKColorSep):
        if (
            isinstance(tc, Color) and tc.red == tc.blue == tc.green
        ):  # ahahahah it's a grey
            tc = _CMYK_black.clone(density=1 - tc.red)
        elif not (
            isinstance(tc, CMYKColor) and tc.cyan == tc.magenta == tc.yellow == 0
        ):  # ie some shade of grey
