...
_mockumap = (
    None
    if os.environ.get("OFFLINE_MOCK", "1") != "1"
        ...
        if os.path.isdir(directory):
            ...
            self.files = list(
                map(
                    ...
                    list(
                        filter(
                            lambda x, pfx=pfx: x.startswith(pfx),
...
class RestrictedGlobDirectoryWalker(GlobDirectoryWalker):
    ...
    def __init__(self, directory, pattern="*", ignore=None):
        ...
        if isinstance(ignore, (tuple, list)):
            ...
            ip(ignore)
        self.ignorePatterns = (
            [_.replace("/", os.sep) for _ in ip.__self__]
            if os.sep != "/"
            else ip.__self__
        )
...
class ScriptThatMakesFileTest(unittest.TestCase):
    ...
    def runTest(self):
        ...
        if self.verbose:
            print(out)
        # status = p.close()
        assert os.path.isfile(self.outFileName), (
            "File %s not created!" % self.outFileName
        )
