...
class FontFinder:
    ...
    def getFont(self, familyName, bold=False, italic=False):
        ...
        raise KeyError(
            "Cannot find font %s with bold=%s, italic=%s" % (familyName, bold, italic)
        )
    ...
    def _getCacheFileName(self):
        ...
        fsEncoding = self._fsEncoding
        hash = md5(
            b"".join(asBytes(_, enc=fsEncoding) for _ in sorted(self._dirs))
        ).hexdigest()
        ...
        if self.useCache:
            ...
            if rl_isfile(cfn):
                try:
                    ...
                    if self.verbose >= 3:
                        print(
                            "loaded cached file with %d fonts (%s)"
                            % (len(self._fonts), cfn)
                        )
                    return
                except:
        ...
        for dirName in self._dirs:
            ...
            for fileName in fileNames:
                ...
                if ext.lower() in EXTENSIONS:
                    # it's a font
                    f = FontDescriptor()
                    f.fileName = fileName = os.path.normpath(
