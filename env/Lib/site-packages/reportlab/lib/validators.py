...
class OneOf(Validator):
    ...
    def __init__(self, enum, *args):
        ...
        if self._patterns:
            self._enum = tuple(
                (_ for _ in self._enum if not isinstance(_, _re_Pattern))
            )
...
isListOfListOfNoneOrNumber = SequenceOf(
    isListOfNoneOrNumber, "isListOfListOfNoneOrNumber"
)
