...
PYTHON = [
    ...
    "No, that's not dead, it's ,uhhhh, resting",
...
if rl_config.invariant:
    ...
    class RLMonkeyPatchRandom(random.Random):
        def randrange(
            self, start, stop=None, step=1, _int=int, _maxwidth=1 << random.BPF
        ):
...
if __name__ == "__main__":
    ...
    if argv:
        ...
        try:
            ...
            sys.stderr.write(
                " theme in chomsky|STARTUP|COMPUTERS|BLAH|BUZZWORD|STARTREK|PRINTING|PYTHON\n"
            )
            raise
    else:
        print(chomsky(5))
