...
__rl_unsafe__ = frozenset(
    """builtins breakpoint __annotations__ co_argcount co_cellvars co_code co_consts
...
class UntrustedAstTransformer(ast.NodeTransformer):
    ...
    def visit_Attribute(self, node):
        ...
        if isinstance(node.ctx, ast.Load):
            ...
            node = self.visit_children(node)
            new_value = ast.Call(
                func=ast.Name("__rl_sd__", ast.Load()), args=[node.value], keywords=[]
            )
    ...
    def visit_AugAssign(self, node):
        ...
        if isinstance(node.target, ast.Attribute):
            ...
            self.error(
                node, "Augmented assignment of object items and slices is not allowed."
            )
...
def astFormat(node):
    return ast.dump(
        copy.deepcopy(node), annotate_fields=True, include_attributes=True, indent=4
    )
...
class __RL_SAFE_ENV__:
    ...
    def __init__(self, timeout=None, allowed_magic_methods=None):
        ...
        self.allowed_magic_methods = (
            (
                __allowed_magic_methods__
                if allowed_magic_methods == True
                else allowed_magic_methods
            )
            if allowed_magic_methods
            else []
        )
        import builtins
        ...
        self.__rl_builtins__ = __rl_builtins__ = {
            _: __rl_missing_func__(_)
            for _ in dir(builtins)
            if callable(getattr(builtins, _))
        }
        ...
        __rl_builtins__[
            "__rl_iter_unpack_sequence__"
        ] = self.__rl_iter_unpack_sequence__
        __rl_builtins__["__rl_unpack_sequence__"] = self.__rl_unpack_sequence__
        __rl_builtins__["__rl_apply__"] = lambda func, *args, **kwds: self.__rl_apply__(
            func, args, kwds
        )
    ...
    def __rl_zip__(self, *args):
        return zip(
            *[
                self.__rl_args_iter__(self.__rl_getitem__(args, i))
    ...
    def __rl_mult__(self, a, b):
        if (hasattr(a, "__len__") and b * len(a) > self.__rl_max_len__) or (
            hasattr(b, "__len__") and a * len(b) > self.__rl_max_len__
        ):
            raise BadCode("excessive length")
        return a * b
    ...
    def __rl_apply__(self, func, args, kwds):
        ...
        if obj:
            if isinstance(obj, dict) and func.__name__ in (
                "pop",
                "setdefault",
                "get",
                "popitem",
        ...
        return func(
            *[a for a in self.__rl_getiter__(args)], **{k: v for k, v in kwds.items()}
        )
    ...
    def __rl_compile__(
        self, src, fname="<string>", mode="eval", flags=0, inherit=True, visit=None
    ):
        names_seen = {}
        if not visit:
            bcode = compile(
                src, fname, mode=mode, flags=flags, dont_inherit=not inherit
            )
        else:
    ...
    def __rl_safe_eval__(
        ...
        mode,
        timeout=None,
        allowed_magic_methods=None,
        __frame_depth__=3,
    ):
        bcode, ns = self.__rl_compile__(
            expr,
            fname="<string>",
            mode=mode,
            flags=0,
            inherit=True,
            visit=UntrustedAstTransformer(
                nameIsAllowed=self.__rl_is_allowed_name__
            ).visit,
        )
        if None in (l, g):
        ...
        self.__rl_limit__ = self.__time_time__() + (
...
class __rl_safe_eval__:
    ...
    def __call__(self, expr, g=None, l=None, timeout=None, allowed_magic_methods=None):
        if not self.env:
            self.env = __RL_SAFE_ENV__(
                timeout=timeout, allowed_magic_methods=allowed_magic_methods
            )
        return self.env.__rl_safe_eval__(
            expr,
            g,
            l,
            self.mode,
...
def rl_extended_literal_eval(expr, safe_callables=None, safe_names=None):
    ...
    try:
        ...
        ast.NameConstant
        safe_test = (
            lambda n: isinstance(n, ast.NameConstant)
            or isinstance(n, ast.Name)
            and n.id in safe_names
        )
        safe_extract = (
            lambda n: n.value if isinstance(n, ast.NameConstant) else safe_names[n.id]
        )
    ...
    def _convert(node):
        if isinstance(node, (ast.Str, ast.Bytes)):
            ...
            and isinstance(node.left, (ast.Num, ast.UnaryOp, astBinOp))
        ):
            ...
            and node.func.id in safe_callables
        ):
