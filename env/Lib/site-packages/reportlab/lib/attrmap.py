                ...
                try:
                    ...
                    if not validate(value):
                        raise AttributeError(
                            "Illegal assignment of '%s' to '%s' in class %s"
                            % (value, name, obj.__class__.__name__)
                        )
                except KeyError:
                    raise AttributeError(
                        "Illegal attribute '%s' in class %s"
...
def hook__setattr__(obj):
    if not hasattr(obj, "__attrproxy__"):
        ...
        obj.__class__ = new.classobj(
            C.__name__,
            (C,) + C.__bases__,
            {
                "__attrproxy__": [],
                "__setattr__": lambda self, k, v, osa=getattr(
                    obj, "__setattr__", None
                ), hook=hook: hook(self, k, v, osa),
            },
        )
