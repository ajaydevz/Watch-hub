...
class CodeChartBase(Flowable):
    ...
    def drawLabels(self, topLeft=""):
        ...
        for row in range(self.rows):
            ...
            self.canv.drawCentredString(
                0.5 * self.boxSize, (self.rows - row - 0.75) * self.boxSize, label
            )
        ...
        if topLeft:
            ...
            self.canv.drawCentredString(
                0.5 * self.boxSize, (self.rows + 0.25) * self.boxSize, topLeft
            )
...
class SingleByteEncodingChart(CodeChartBase):
    def __init__(
        ...
        pdfmetrics.registerFont(
            pdfmetrics.Font(self.fontName, self.faceName, self.encodingName)
        )
