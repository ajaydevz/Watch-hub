...
class AdjustableArrow(Widget):
    ...
    _attrMap = AttrMap(
        ...
        headProjection=AttrMapValue(
            isNumber, "how much the head projects from the stem"
        ),
        headLength=AttrMapValue(isNumber, "length of the head"),
        headSweep=AttrMapValue(
            isNumber, "howmuch the head sweeps back (-ve) or forwards (+ve)"
        ),
        ...
        right=AttrMapValue(
            isBoolean,
            desc="If True (default) the arrow is horizontal pointing right\nFalse means it points up",
        ),
        angle=AttrMapValue(
            isNumber,
            desc="angle of arrow default (0), right True 0 is horizontal to right else vertical up",
        ),
    )
...
class AdjustableArrowDrawing(_DrawingEditorMixin, Drawing):
    def __init__(self, width=100, height=63, *args, **kw):
        ...
        self._add(
            self, AdjustableArrow(), name="adjustableArrow", validate=None, desc=None
        )
