...
class _Symbol(Widget):
    ...
    def __init__(self):
        assert (
            self.__class__.__name__ != "_Symbol"
        ), "Abstract class _Symbol instantiated"
...
class ETriangle(_Symbol):
    ...
    def draw(self):
        ...
        triangle = shapes.Polygon(
            points=[self.x, self.y, self.x + s, self.y, self.x + (s / 2), self.y + s],
    ...
    def draw(self):
        ...
        g.add(
            shapes.Circle(
                ...
                fillColor=self.fillColor,
                strokeColor=self.strokeColor,
                strokeWidth=max(s / 38.0, self.strokeWidth),
            )
        )
    ...
    def draw(self):
        ...
        innerOctagon = shapes.Polygon(
            points=[
                ...
                self.y + athird + (s / 75),
                self.x + (s / 75),
                self.y + (athird * 2) - (s / 75),
                self.x + athird + (s / 75),
                self.y + s - (s / 75),
                self.x + (athird * 2) - (s / 75),
                (self.y + s) - (s / 75),
                (self.x + s) - (s / 75),
                self.y + (athird * 2) - (s / 75),
                (self.x + s) - (s / 75),
        ...
        if innerBarColor:
            g.add(
                shapes.Rect(
                    ...
                    width=s * 0.8,
                    height=s * 0.2,
                    fillColor=innerBarColor,
                    strokeColor=innerBarColor,
                    strokeLineCap=1,
                    strokeWidth=0,
                )
            )
...
class DangerSign(_Symbol):
    ...
    def draw(self):
        ...
        exmark = shapes.Polygon(
            points=[
                ((self.x + s / 2) - ew / 2),
                self.y + ae * 2.5,
                ((self.x + s / 2) + ew / 2),
                self.y + ae * 2.5,
                ((self.x + s / 2) + ((ew / 2)) + (ew / 6)),
                self.y + ae * 5.5,
                ((self.x + s / 2) - ((ew / 2)) - (ew / 6)),
                self.y + ae * 5.5,
            ],
...
class CrossHair(_Symbol):
    ...
    _attrMap = AttrMap(
        BASE=_Symbol,
        innerGap=AttrMapValue(
            EitherOr((isString, isNumberOrNone)),
            desc=' gap at centre as "x%" or points or None',
        ),
    )
