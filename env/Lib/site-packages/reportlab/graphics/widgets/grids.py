...
class Grid(Widget):
    ...
    _attrMap = AttrMap(
        ...
        useRects=AttrMapValue(
            OneOf((0, 1)), desc="Determines if stripes are drawn with solid rectangles."
        ),
        delta=AttrMapValue(
            isNumber, desc="Determines the width/height of the stripes."
        ),
        delta0=AttrMapValue(
            isNumber, desc="Determines the stripes initial width/height offset."
        ),
        deltaSteps=AttrMapValue(
            isListOfNumbers, desc="List of deltas to be used cyclically."
        ),
        ...
        fillColor=AttrMapValue(
            isColorOrNone, desc="Background color for entire rectangle."
        ),
        ...
        rectStrokeColor=AttrMapValue(
            isColorOrNone, desc="Color for outer rect stroke."
        ),
        rectStrokeWidth=AttrMapValue(
            isNumberOrNone, desc="Width for outer rect stroke."
        ),
    )
...
class ShadedRect(Widget):
    ...
    _attrMap = AttrMap(
        ...
        fillColorStart=AttrMapValue(
            isColorOrNone, desc="Start value of the color shade."
        ),
        ...
        strokeWidth=AttrMapValue(isNumber, desc="Width used for lines."),
        cylinderMode=AttrMapValue(
            isBoolean, desc="True if shading reverses in middle."
        ),
    )
    ...
    def _flipRectCorners(self):
        ...
        x, y, width, height, fillColorStart, fillColorEnd = (
            self.x,
            self.y,
            self.width,
            self.height,
...
class ShadedPolygon(Widget, LineShape):
    ...
    def draw(self):
        ...
        path = definePath(
            [("moveTo",) + P[0]] + [("lineTo",) + x for x in P[1:]] + ["closePath"],
            fillColor=None,
            strokeColor=None,
        )
        path.isClipPath = 1
