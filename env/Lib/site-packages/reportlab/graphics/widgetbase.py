    ...
    def verify(self):
        ...
        if self._attrMap is not None:
            ...
            for attr, metavalue in self._attrMap.items():
                ...
                assert metavalue.validate(value), (
                    "Invalid value %s for attribute %s in class %s" % args
                )
...
class ScaleWidget(Widget):
    ...
    def draw(self):
        return shapes.Group(
            transform=(self.scale, 0, 0, self.scale, self.x, self.y), *self.contents
        )
...
class TypedPropertyCollection(PropHolder):
    ...
    def __getitem__(self, x):
        ...
        try:
            ...
            for i in filter(
                lambda x, K=list(child.__dict__.keys()): x in K,
                list(child._attrMap.keys()),
            ):
                del child.__dict__[i]
            child.__dict__.update(
                dict(
                    __propholder_parent__=weakref_ref(self), __propholder_index__=x[:-1]
                )
            )
    ...
    def __setitem__(self, key, value):
        assert isinstance(value, self._value.__class__), (
            "This collection can only hold objects of type %s"
...
class StyleProperties(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        strokeLineCap=AttrMapValue(
            isNumber, desc="Line cap 0=butt, 1=round & 2=square", advancedUsage=1
        ),
        strokeLineJoin=AttrMapValue(
            isNumber, desc="Line join 0=miter, 1=round & 2=bevel", advancedUsage=1
        ),
        strokeMiterLimit=AttrMapValue(
            None, desc="miter limit control miter line joins", advancedUsage=1
        ),
        strokeDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="dashing patterns e.g. (1,3)"
        ),
...
class CandleStickProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        crossWidth=AttrMapValue(
            isNumberOrNone, desc="cross line width", advancedUsage=1
        ),
        crossLo=AttrMapValue(
            isNumberOrNone, desc="cross line low value", advancedUsage=1
        ),
        crossHi=AttrMapValue(
            isNumberOrNone, desc="cross line high value", advancedUsage=1
        ),
        boxWidth=AttrMapValue(
            isNumberOrNone, desc="width of the box part", advancedUsage=1
        ),
        boxFillColor=AttrMapValue(isColorOrNone, desc="fill color of box"),
        boxStrokeColor=AttrMapValue(
            NotSetOr(isColorOrNone), desc="stroke color of box"
        ),
        boxStrokeDashArray=AttrMapValue(
            NotSetOr(isListOfNumbersOrNone), desc="Dash array of the box."
        ),
        boxStrokeWidth=AttrMapValue(NotSetOr(isNumber), desc="Width of the box lines."),
        boxLo=AttrMapValue(
            isNumberOrNone, desc="low value of the box", advancedUsage=1
        ),
        boxMid=AttrMapValue(
            isNumberOrNone, desc="middle box line value", advancedUsage=1
        ),
        boxHi=AttrMapValue(
            isNumberOrNone, desc="high value of the box", advancedUsage=1
        ),
        boxSides=AttrMapValue(
            isBoolean, desc="whether to show box sides", advancedUsage=1
        ),
        position=AttrMapValue(
            isNumberOrNone, desc="position of the candle", advancedUsage=1
        ),
        chart=AttrMapValue(None, desc="our chart", advancedUsage=1),
        candleKind=AttrMapValue(
            OneOf("vertical", "horizontal"), desc="candle direction", advancedUsage=1
        ),
        axes=AttrMapValue(
            SequenceOf(isString, emptyOK=0, lo=2, hi=2),
            desc="candle direction",
            advancedUsage=1,
        ),
    )
    ...
    def __call__(self, _x, _y, _size, _color):
        ...
        boxStrokeDashArray = NotSetOr.conditionalValue(
            self.boxStrokeDashArray, strokeDashArray
        )
        ...
        if haveBox:
            ...
            if not self.boxSides:
                aLine(
                    position - 0.5 * boxWidth, boxHi, position + 0.5 * boxWidth, boxHi
                )
                aLine(
                    position - 0.5 * boxWidth, boxLo, position + 0.5 * boxWidth, boxLo
                )

            if boxMid is not None:
                aLine(
                    position - 0.5 * boxWidth, boxMid, position + 0.5 * boxWidth, boxMid
                )
        return G.__self__
