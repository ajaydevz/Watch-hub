    ...
    if backend == "_renderPM":
        try:
            ...
            try:
                ...
                raise RenderPMError(
                    """Cannot import desired renderPM backend, {backend}.
...
https://pairlist2.pair.net/mailman/listinfo/reportlab-users"""
        ...
        try:
            ...
            try:
                ...
                raise RenderPMError(
                    f"""cannot import desired renderPM backend {backend}
...
def Color2Hex(c):
    ...
    if c:
        return (
            ((0xFF & int(255 * c.red)) << 16)
            | ((0xFF & int(255 * c.green)) << 8)
            | (0xFF & int(255 * c.blue))
        )
    return c
...
class _PMRenderer(Renderer):
    ...
    def drawImage(self, image):
        ...
        self._canvas._aapixbuf(
            ...
            (
                im
                if self._canvas._backend
                == "rlPyCairo"  # rlPyCairo has a from_pil method
...
class PMCanvas:
    def __init__(
        self, w, h, dpi=72, bg=0xFFFFFF, configPIL=None, backend=None, backendFmt="RGB"
    ):
        ...
        self.__dict__["_backend"] = (
            "_renderPM" if type(self._gs._aapixbuf) == type(pow) else "rlPyCairo"
        )
    ...
    def saveToFile(self, fn, fmt=None):
        ...
        if fmt is None:
            if not isinstance(fn, str):
                raise ValueError(
                    "Invalid value '%s' for fn when fmt is None" % ascii(fn)
                )
        ...
        if fmt in ("GIF",):
            ...
            return _saveAsPICT(
                im, fn, fmt, transparent=configPIL.get("transparent", None)
            )
    ...
    def _saveToBMP(self, f):
        ...
        f.write(
            struct.pack(
                "=2sLLLLLLhh24x", "BM", len(pix) + 54, 0, 54, 40, width, height, 1, 24
            )
        )
        rowb = width * 3
...
def drawToPMCanvas(
    ...
    c = PMCanvas(
        d.width, d.height, dpi=dpi, bg=bg, configPIL=configPIL, backend=backend
    )
...
def test(outDir="pmout", shout=False):
    ...
    for drawing, docstring, name in getAllTestDrawings(
        doTTF=hasattr(_pmBackend, "ft_get_face")
    ):
        ...
        for k in E:
            ...
            try:
                ...
                if k == "pct":
                    ...
                    drawToFile(
                        drawing, fullpath, fmt=k, configPIL={"transparent": white}
                    )
                elif k in ["py", "svg"]:
