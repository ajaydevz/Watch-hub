...
class SlideBox(Widget):
    ...
    _attrMap = AttrMap(
        ...
        labelStrokeColor=AttrMapValue(
            isColorOrNone, desc="Colour for for number outlines"
        ),
        ...
        trianglePosition=AttrMapValue(
            isInt, desc="Which box is highlighted by the triangles"
        ),
        ...
        triangleStrokeColor=AttrMapValue(
            isColorOrNone, desc="Colour of indicator triangle outline"
        ),
        triangleStrokeWidth=AttrMapValue(
            isNumber, desc="Colour of indicator triangle outline"
        ),
        ...
        boxOutlineColor=AttrMapValue(
            isColorOrNone, desc="Colour used to outline the boxes (if any)"
        ),
        boxOutlineWidth=AttrMapValue(
            isNumberOrNone, desc="Width of the box outline (if any)"
        ),
        ...
        background=AttrMapValue(
            isColorOrNone, desc="Colour of the background to the drawing (if any)"
        ),
        sourceLabelText=AttrMapValue(
            isNoneOrString, desc="Text used for the 'source' label (can be empty)"
        ),
        sourceLabelOffset=AttrMapValue(isNumber, desc="Padding at bottom of drawing"),
        sourceLabelFontName=AttrMapValue(
            isString, desc="Name of font used for the 'source' label"
        ),
        sourceLabelFontSize=AttrMapValue(
            isNumber, desc="Font size for the 'source' label"
        ),
        sourceLabelFillColor=AttrMapValue(
            isColorOrNone, desc="Colour ink for the 'source' label (bottom right)"
        ),
    )
        ...
        for i in range(0, numShades):
            colorsList.append(
                colors.linearlyInterpolatedColor(
                    fillColorStart, fillColorEnd, 0, numShades - 1, i
                )
            )
        return colorsList
    ...
    def draw(self):
        ...
        ascent = getFont(self.labelFontName).face.ascent / 1000.0
        if ascent == 0:
