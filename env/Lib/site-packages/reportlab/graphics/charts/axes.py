...
def _findMin(V, x, default, special=None, extraMinMaxValues=None):
    ...
    return _findMinMaxValue(
        V, x, default, min, special=special, extraMinMaxValues=extraMinMaxValues
    )
...
def _findMax(V, x, default, special=None, extraMinMaxValues=None):
    ...
    return _findMinMaxValue(
        V, x, default, max, special=special, extraMinMaxValues=extraMinMaxValues
    )
...
class _AxisG(Widget):
    ...
    def _getLineFunc(self, start, end, parent=None):
        ...
        if _3d_dx is not None:
            ...
            return lambda v, s=start, e=end, f=f, _3d_dx=_3d_dx, _3d_dy=_3d_dy: f(
                v, s, e, _3d_dx=_3d_dx, _3d_dy=_3d_dy
            )
        else:
    ...
    def _makeLines(
        ...
        if exclude:
            exf = (
                self.isYAxis
                and (lambda l: l.y1 in exclude)
                or (lambda l: l.x1 in exclude)
            )
        else:
            exf = None
        ...
        if w and c and self.visibleGrid:
            ...
            if s or e:
                ...
                self._makeLines(
                    ...
                    self.gridStrokeLineJoin,
                    self.gridStrokeLineCap,
                    self.gridStrokeMiterLimit,
                    parent=parent,
                    exclude=exclude,
                    specials=getattr(self, "_gridSpecials", {}),
                )
        ...
        if s or e:
            ...
            try:
                self._makeLines(
                    ...
                    c,
                    w,
                    self.subGridStrokeDashArray,
                    self.subGridStrokeLineJoin,
                    self.subGridStrokeLineCap,
                    self.subGridStrokeMiterLimit,
                    parent=parent,
                    exclude=exclude,
                )
            finally:
...
class CALabel(PMVLabel):
    _attrMap = AttrMap(
        BASE=PMVLabel,
        labelPosFrac=AttrMapValue(
            isNumber,
            desc="where in the category range [0,1] the labels should be anchored",
        ),
    )
...
class CategoryAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        strokeDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="Dash array used for axis line."
        ),
        strokeLineCap=AttrMapValue(
            OneOf(0, 1, 2), desc="Line cap 0=butt, 1=round & 2=square"
        ),
        strokeLineJoin=AttrMapValue(
            OneOf(0, 1, 2), desc="Line join 0=miter, 1=round & 2=bevel"
        ),
        strokeMiterLimit=AttrMapValue(
            isNumber, desc="miter limit control miter line joins"
        ),
        ...
        gridStrokeDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="Dash array used for grid lines."
        ),
        gridStrokeLineCap=AttrMapValue(
            OneOf(0, 1, 2), desc="Grid Line cap 0=butt, 1=round & 2=square"
        ),
        gridStrokeLineJoin=AttrMapValue(
            OneOf(0, 1, 2), desc="Grid Line join 0=miter, 1=round & 2=bevel"
        ),
        gridStrokeMiterLimit=AttrMapValue(
            isNumber, desc="Grid miter limit control miter line joins"
        ),
        gridStart=AttrMapValue(
            isNumberOrNone, desc="Start of grid lines wrt axis origin"
        ),
        gridEnd=AttrMapValue(isNumberOrNone, desc="End of grid lines wrt axis origin"),
        drawGridLast=AttrMapValue(
            isBoolean, desc="if true draw gridlines after everything else."
        ),
        labels=AttrMapValue(None, desc="Handle of the axis labels."),
        categoryNames=AttrMapValue(
            isListOfStringsOrNone, desc="List of category names."
        ),
        joinAxis=AttrMapValue(None, desc="Join both axes if true."),
        joinAxisPos=AttrMapValue(
            isNumberOrNone, desc="Position at which to join with other axis."
        ),
        reverseDirection=AttrMapValue(
            isBoolean, desc="If true reverse category direction."
        ),
        style=AttrMapValue(
            OneOf("parallel", "stacked", "parallel_3d"),
            "How common category bars are plotted",
        ),
        labelAxisMode=AttrMapValue(
            OneOf("high", "low", "axis", "axispmv"),
            desc="Like joinAxisMode, but for the axis labels",
        ),
        tickShift=AttrMapValue(isBoolean, desc="Tick shift typically"),
        tickStrokeWidth=AttrMapValue(
            isNumberOrNone, desc="Width of ticks if specified."
        ),
        tickStrokeColor=AttrMapValue(
            isColorOrNone, desc="Color of ticks if specified."
        ),
        ...
        loLLen=AttrMapValue(
            isNumber, desc="extra line length before start of the axis"
        ),
        hiLLen=AttrMapValue(isNumber, desc="extra line length after end of the axis"),
        skipGrid=AttrMapValue(
            OneOf("none", "top", "both", "bottom"),
            "grid lines to skip top bottom both none",
        ),
    ...
    def __init__(self):
        assert (
            self.__class__.__name__ != "CategoryAxis"
        ), "Abstract Class CategoryAxis Instantiated"
        ...
        self.gridStrokeMiterLimit = self.strokeMiterLimit = STATE_DEFAULTS[
            "strokeMiterLimit"
        ]
    ...
    def configure(self, multiSeries, barWidth=None):
        ...
        self._barWidth = barWidth or (
            (self._length - self.loPad - self.hiPad) / float(self._catCount or 1)
        )
        self._calcTickmarkPositions()
        if self.labelAxisMode == "axispmv":
            self._pmv = [
                sum([series[i] for series in multiSeries])
...
def _assertYAxis(axis):
    assert axis.isYAxis, (
        "Cannot connect to other axes (%s), but Y- ones." % axis.__class__.__name__
    )


def _assertXAxis(axis):
    assert axis.isXAxis, (
        "Cannot connect to other axes (%s), but X- ones." % axis.__class__.__name__
    )
...
class _XTicks:
    ...
    def makeTicks(self):
        ...
        try:
            ...
            self._drawSubTicks(
                getattr(self, "subTickHi", 0), getattr(self, "subTickLo", 0), g
            )
            return g
        finally:
            self._y = yold
...
class XCategoryAxis(_XTicks, CategoryAxis):
    ...
    _attrMap = AttrMap(
        BASE=CategoryAxis,
        tickUp=AttrMapValue(isNumber, desc="Tick length up the axis."),
        tickDown=AttrMapValue(isNumber, desc="Tick length down the axis."),
        joinAxisMode=AttrMapValue(
            OneOf("bottom", "top", "value", "points", None),
    ...
    def makeAxis(self):
        ...
        axis = Line(
            self._x - self.loLLen,
            self._y,
            self._x + self._length + self.hiLLen,
            self._y,
        )
...
class YCategoryAxis(_YTicks, CategoryAxis):
    ...
    _attrMap = AttrMap(
        BASE=CategoryAxis,
        tickLeft=AttrMapValue(isNumber, desc="Tick length left of the axis."),
        tickRight=AttrMapValue(isNumber, desc="Tick length right of the axis."),
        joinAxisMode=AttrMapValue(
            OneOf(("left", "right", "value", "points", None)),
    ...
    def makeAxis(self):
        ...
        axis = Line(
            ...
            self._y - self.loLLen,
            self._x,
            self._y + self._length + self.hiLLen,
        )
        axis.strokeColor = self.strokeColor
        axis.strokeWidth = self.strokeWidth
        axis.strokeDashArray = self.strokeDashArray
...
class ValueAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        forceZero=AttrMapValue(
            EitherOr((isBoolean, OneOf("near"))), desc="Ensure zero in range if true."
        ),
        visible=AttrMapValue(isBoolean, desc="Display entire object, if true."),
        visibleAxis=AttrMapValue(isBoolean, desc="Display axis line, if true."),
        visibleLabels=AttrMapValue(isBoolean, desc="Display axis labels, if true."),
        ...
        minimumTickSpacing=AttrMapValue(
            isNumber, desc="Minimum value for distance between ticks."
        ),
        ...
        labelTextFormat=AttrMapValue(
            None, desc="Formatting string or function used for axis labels."
        ),
        labelTextPostFormat=AttrMapValue(None, desc="Extra Formatting string."),
        labelTextScale=AttrMapValue(
            isNumberOrNone, desc="Scaling for label tick values."
        ),
        ...
        valueSteps=AttrMapValue(
            isListOfNumbersOrNone, desc="List of step sizes used between ticks."
        ),
        avoidBoundFrac=AttrMapValue(
            EitherOr((isNumberOrNone, SequenceOf(isNumber, emptyOK=0, lo=2, hi=2))),
            desc="Fraction of interval to allow above and below.",
        ),
        avoidBoundSpace=AttrMapValue(
            EitherOr((isNumberOrNone, SequenceOf(isNumber, emptyOK=0, lo=2, hi=2))),
            desc="Space to allow above and below.",
        ),
        abf_ignore_zero=AttrMapValue(
            EitherOr((NoneOr(isBoolean), SequenceOf(isBoolean, emptyOK=0, lo=2, hi=2))),
            desc="Set to True to make the avoidBoundFrac calculations treat zero as non-special",
        ),
        rangeRound=AttrMapValue(
            OneOf("none", "both", "ceiling", "floor"), "How to round the axis limits"
        ),
        zrangePref=AttrMapValue(
            isNumberOrNone, desc="Zero range axis limit preference."
        ),
        style=AttrMapValue(
            OneOf("normal", "stacked", "parallel_3d"), "How values are plotted!"
        ),
        skipEndL=AttrMapValue(
            OneOf("none", "start", "end", "both"), desc="Skip high/low tick labels"
        ),
        origShiftIPC=AttrMapValue(
            isNumberOrNone, desc="Lowest label shift interval ratio."
        ),
        origShiftMin=AttrMapValue(isNumberOrNone, desc="Minimum amount to shift."),
        origShiftSpecialValue=AttrMapValue(
            isNumberOrNone, desc="special value for shift"
        ),
        tickAxisMode=AttrMapValue(
            OneOf("high", "low", "axis"), desc="Like joinAxisMode, but for the ticks"
        ),
        reverseDirection=AttrMapValue(
            isBoolean, desc="If true reverse category direction."
        ),
        ...
        visibleSubTicks=AttrMapValue(
            isBoolean, desc="Display axis sub ticks, if true."
        ),
        ...
        subGridStrokeDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="Dash array used for grid lines."
        ),
        subGridStrokeLineCap=AttrMapValue(
            OneOf(0, 1, 2), desc="Grid Line cap 0=butt, 1=round & 2=square"
        ),
        subGridStrokeLineJoin=AttrMapValue(
            OneOf(0, 1, 2), desc="Grid Line join 0=miter, 1=round & 2=bevel"
        ),
        subGridStrokeMiterLimit=AttrMapValue(
            isNumber, desc="Grid miter limit control miter line joins"
        ),
        subGridStart=AttrMapValue(
            isNumberOrNone, desc="Start of grid lines wrt axis origin"
        ),
        subGridEnd=AttrMapValue(
            isNumberOrNone, desc="End of grid lines wrt axis origin"
        ),
        tickStrokeWidth=AttrMapValue(isNumber, desc="Width of ticks if specified."),
        subTickStrokeWidth=AttrMapValue(
            isNumber, desc="Width of sub ticks if specified."
        ),
        subTickStrokeColor=AttrMapValue(
            isColorOrNone, desc="Color of sub ticks if specified."
        ),
        tickStrokeColor=AttrMapValue(
            isColorOrNone, desc="Color of ticks if specified."
        ),
        keepTickLabelsInside=AttrMapValue(
            isBoolean,
            desc="Ensure tick labels do not project beyond bounds of axis if true",
        ),
        skipGrid=AttrMapValue(
            OneOf("none", "top", "both", "bottom"),
            "grid lines to skip top bottom both none",
        ),
        requiredRange=AttrMapValue(
    ...
    def __init__(self, **kw):
        assert (
            self.__class__.__name__ != "ValueAxis"
        ), "Abstract Class ValueAxis Instantiated"
    ...
    def _setRange(self, dataSeries):
        ...
        if forceZero:
            if forceZero == "near":
                forceZero = min(abs(valueMin), abs(valueMax)) <= 5 * (
                    valueMax - valueMin
                )
            if forceZero:
                if valueMax < 0:
        ...
        while go and iter <= 10:
            ...
            go = 0
            if do_abf or do_abs:
                valueStep, T, fuzz = self._getValueStepAndTicks(
                    valueMin, valueMax, cache
                )
                if do_abf:
                    i0 = valueStep * abf[0]
                    i1 = valueStep * abf[1]
        ...
        if origShiftMin is not None or origShiftIPC is not None:
            ...
            if (
                origShiftSpecialValue is None
                or abs(origShiftSpecialValue - valueMin) < 1e-6
            ):
                if origShiftIPC:
    ...
    def _calcValueStep(self):
        ...
        if self.valueStep is None:
            ...
            rawInterval = rawRange / min(
                float(self.maximumTicks - 1),
                (float(self._length) / self.minimumTickSpacing),
            )
    ...
    def makeTickLabels(self):
        ...
        if f is None:
            f = self.labelTextFormat or (
                self._allIntTicks() and "%.0f" or _defaultLabelFormatter
            )
...
class NormalDateXValueAxis(XValueAxis):
    ...
    _attrMap = AttrMap(
        BASE=XValueAxis,
        bottomAxisLabelSlack=AttrMapValue(
            isNumber, desc="Fractional amount used to adjust label spacing"
        ),
        niceMonth=AttrMapValue(isBoolean, desc="Flag for displaying months 'nicely'."),
        forceEndDate=AttrMapValue(
            isBoolean, desc="Flag for enforced displaying of last date value."
        ),
        forceFirstDate=AttrMapValue(
            isBoolean, desc="Flag for enforced displaying of first date value."
        ),
        forceDatesEachYear=AttrMapValue(
            isListOfDaysAndMonths,
            desc='List of dates in format "31-Dec",'
        ...
        xLabelFormat=AttrMapValue(
            None, desc="Label format string (e.g. '{mm}/{yy}') or function."
        ),
        dayOfWeekName=AttrMapValue(
            SequenceOf(isString, emptyOK=0, lo=7, hi=7), desc="Weekday names."
        ),
        monthName=AttrMapValue(
            SequenceOf(isString, emptyOK=0, lo=12, hi=12), desc="Month names."
        ),
        dailyFreq=AttrMapValue(
            isBoolean,
            desc="True if we are to assume daily data to be ticked at end of month.",
        ),
        specifiedTickDates=AttrMapValue(
            NoneOr(SequenceOf(isNormalDate)),
            desc="Actual tick values to use; no calculations done",
        ),
        specialTickClear=AttrMapValue(
            isBoolean,
            desc="clear rather than delete close ticks when forced first/end dates",
        ),
        skipGrid=AttrMapValue(
            OneOf("none", "top", "both", "bottom"),
            "grid lines to skip top bottom both none",
    ...
    def _dateFormatter(self, v):
        ...
        try:
            normalDate._dayOfWeekName, normalDate._monthName = (
                self.dayOfWeekName,
                self.monthName,
            )
    ...
    def _xAxisTicker(self, xVals):
        ...
        if self.specifiedTickDates:
            ...
            if (
                self.forceFirstDate
                and firstDate == ticks[0]
                and (axisLength / float(ticks[-1] - ticks[0])) * (ticks[1] - ticks[0])
                <= W
            ):
            ...
            if (
                self.forceEndDate
                and endDate == ticks[-1]
                and (axisLength / float(ticks[-1] - ticks[0])) * (ticks[-1] - ticks[-2])
                <= W
            ):
                if self.specialTickClear:
                    labels[-2] = ""
        ...
        if self.forceDatesEachYear:
            ...
            if self.forceFirstDate and firstDate != ticks[0]:
                ...
                if (axisLength / float(ticks[-1] - ticks[0])) * (
                    ticks[1] - ticks[0]
                ) <= W:
                    if self.specialTickClear:
                        labels[1] = ""
            ...
            if _allInt(tv):
                ...
                if u == 1:
                    ...
                    if _allInt(fv):
                        ...
                        fmt = lambda x, uf=uf, fm=fm, fmt="%%d%s%%d%%s" % (
                            s,
                            s1,
                        ): fmt % (int(x / uf), int((x / uf - int(x / uf)) * fm))
                    else:
                        fmt = lambda x, uf=uf, fmt="%%.%df%s" % (stvl, s): fmt % (
                            x / uf
                        )
...
class AdjYValueAxis(YValueAxis):
    ...
    _attrMap = AttrMap(
        BASE=YValueAxis,
        leftAxisPercent=AttrMapValue(
            isBoolean, desc="When true add percent sign to label values."
        ),
        leftAxisOrigShiftIPC=AttrMapValue(
            isNumber, desc="Lowest label shift interval ratio."
        ),
        leftAxisOrigShiftMin=AttrMapValue(isNumber, desc="Minimum amount to shift."),
        leftAxisSkipLL0=AttrMapValue(
            EitherOr((isBoolean, isListOfNumbers)),
            desc="Skip/Keep lowest tick label when true/false.\nOr skiplist",
        ),
    ...
    def _rangeAdjust(self):
        ...
        T, L = ticks(
            ...
            split=1,
            n=n,
            percent=self.leftAxisPercent,
            grid=valueStep,
            labelVOffset=self.labelVOffset,
        )
        abf = self.avoidBoundFrac
        if abf:
            i1 = T[1] - T[0]
        ...
        if abs(self._valueMin - 100) < 1e-6:
            ...
            m = max(
                self.leftAxisOrigShiftIPC * self._valueStep,
...
class LogValueAxis(ValueAxis):
    ...
    def _setRange(self, dataSeries):
        ...
        aMin = _findMin(
            dataSeries, self._dataIndex, 0, extraMinMaxValues=self.extraMinMaxValues
        )
        aMax = _findMax(
            dataSeries, self._dataIndex, 0, extraMinMaxValues=self.extraMinMaxValues
        )
        ...
        if valueMin > valueMax:
            raise ValueError(
                "%s: valueMin=%r should not be greater than valueMax=%r!"
                % (self.__class__.__name__valueMin, valueMax)
            )
        if valueMin <= 0:
            raise ValueError(
                "%s: valueMin=%r negative values are not allowed!"
...
class LogXValueAxis(LogValueAxis, LogAxisLabellingSetup, XValueAxis):
    ...
    def scale(self, value):
        ...
        return self._x + self._scaleFactor * (
            math_log10(value) - math_log10(self._valueMin)
        )
...
class LogYValueAxis(LogValueAxis, LogAxisLabellingSetup, YValueAxis):
    ...
    def scale(self, value):
        ...
        return self._y + self._scaleFactor * (
            math_log10(value) - math_log10(self._valueMin)
        )
