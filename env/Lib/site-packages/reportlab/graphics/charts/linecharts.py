...
class LineChartProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        symbol=AttrMapValue(
            NoneOr(isSymbol), desc="Widget placed at data points.", advancedUsage=1
        ),
        ...
        lineStyle=AttrMapValue(
            NoneOr(OneOf("line", "joinedLine", "bar")),
...
class AbstractLineChart(PlotArea):
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        if lineStyle == "bar":
            ...
            strokeWidth = getattr(
                style, "strokeWidth", getattr(style, "strokeWidth", None)
            )
            L = Rect(
                x,
                y,
...
class HorizontalLineChart(LineChart):
    ...
    _attrMap = AttrMap(
        ...
        useAbsolute=AttrMapValue(
            isNumber, desc="Flag to use absolute spacing values.", advancedUsage=1
        ),
        ...
        inFill=AttrMapValue(
            isBoolean, desc="Whether infilling should be done.", advancedUsage=1
        ),
            ...
            if uSymbol:
                for colNo, (x, y) in enumerate(row):
                    symbol = uSymbol2Symbol(
                        tpcGetItem(uSymbol, colNo), x, y, rowStyle.strokeColor
                    )
                    if symbol:
                        g.add(symbol)
...
def sampleCandleStick():
    ...
    chart.valueAxis.valueMin = min(
        min(boxMid), min(hi), min(lo), min(boxLo), min(boxHi)
    )
    chart.valueAxis.valueMax = max(
        max(boxMid), max(hi), max(lo), max(boxLo), max(boxHi)
    )
    lines = chart.lines
    lines[0].strokeColor = None
    I = range(len(boxMid))
    chart.data = [boxMid]
    lines[0].symbol = candles = CandleSticks(
        chart=chart,
        boxFillColor=boxFillColor,
        boxWidth=boxWidth,
        crossWidth=crossWidth,
        strokeWidth=candleStrokeWidth,
        strokeColor=candleStrokeColor,
    )
    for i in I:
        candles[i].setProperties(
            dict(
                position=i,
                boxMid=boxMid[i],
                crossLo=lo[i],
