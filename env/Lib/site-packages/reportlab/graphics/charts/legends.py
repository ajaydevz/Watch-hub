...
class SubColProperty(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        align=AttrMapValue(
            OneOf("left", "right", "center", "centre", "numeric"),
        ...
        underlines=AttrMapValue(
            EitherOr(
                (
                    NoneOr(isInstanceOf(Line)),
                    SequenceOf(isInstanceOf(Line), emptyOK=0, lo=0, hi=0x7FFFFFFF),
                )
            ),
            desc="underline definitions",
        ),
        overlines=AttrMapValue(
            EitherOr(
                (
                    NoneOr(isInstanceOf(Line)),
                    SequenceOf(isInstanceOf(Line), emptyOK=0, lo=0, hi=0x7FFFFFFF),
        ...
        vAlign=AttrMapValue(
            OneOf("top", "bottom", "middle"), desc="vertical alignment in the row"
        ),
    )
...
class Legend(Widget):
    ...
    _attrMap = AttrMap(
        ...
        deltax=AttrMapValue(
            isNumberOrNone, desc="x-distance between neighbouring swatches"
        ),
        deltay=AttrMapValue(
            isNumberOrNone, desc="y-distance between neighbouring swatches"
        ),
        dxTextSpace=AttrMapValue(
            isNumber, desc="Distance between swatch rectangle and text"
        ),
        autoXPadding=AttrMapValue(
            isNumber, desc="x Padding between columns if deltax=None", advancedUsage=1
        ),
        autoYPadding=AttrMapValue(
            isNumber, desc="y Padding between rows if deltay=None", advancedUsage=1
        ),
        yGap=AttrMapValue(
            isNumber, desc="Additional gap between rows", advancedUsage=1
        ),
        ...
        alignment=AttrMapValue(
            OneOf("left", "right"), desc="Alignment of text with respect to swatches"
        ),
        ...
        callout=AttrMapValue(
            None, desc="a user callout(self,g,x,y,(color,text))", advancedUsage=1
        ),
        ...
        dividerColor=AttrMapValue(
            isColorOrNone, desc="dividerLines color", advancedUsage=1
        ),
        dividerDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="Dash array for dividerLines.", advancedUsage=1
        ),
        dividerOffsX=AttrMapValue(
            SequenceOf(isNumber, emptyOK=0, lo=2, hi=2),
            desc="divider lines X offsets",
            advancedUsage=1,
        ),
        dividerOffsY=AttrMapValue(
            isNumber, desc="dividerLines Y offset", advancedUsage=1
        ),
        colEndCallout=AttrMapValue(
            None, desc="a user callout(self,g, x, xt, y,width, lWidth)", advancedUsage=1
        ),
    ...
    def _getTexts(self, colorNamePairs):
        if not isAuto(colorNamePairs):
            ...
            chart = getattr(
                colorNamePairs, "chart", getattr(colorNamePairs, "obj", None)
            )
            texts = [
    ...
    def draw(self):
        ...
        if autoCP:
            ...
            if isAuto(swatchMarker):
                chart = getattr(
                    swatchMarker, "chart", getattr(swatchMarker, "obj", None)
                )
        ...
        for i in range(n):
            ...
            if callout:
                callout(self, g, thisx, y, (col, name))
            if alignment == "left":
                x = thisx
            ...
            for k, lines in enumerate(T):
                ...
                if fN == fontName:
                    ...
                    fA = getFont(fontName).face.ascent / 1000.0
                    if fA == 0:
                        fA = 0.718
                    fA *= fS
                ...
                y += fL
                for iy, a in (
                    (y - max(fL - fA, 0), "underlines"),
                    (y + fA, "overlines"),
                ):
                    il = getattr(sc, a, None)
                    if il:
                        if not isinstance(il, (tuple, list)):
                            il = (il,)
                        for l in il:
                            l = copy.copy(l)
                            l.y1 += iy
            ...
            if isAuto(col):
                ...
                c = chart.makeSwatchSample(
                    getattr(col, "index", i), swatchX, swatchY, dx, dy
                )
            elif isinstance(col, colors.Color):
                if isSymbol(swatchMarker):
                    c = uSymbol2Symbol(
                        swatchMarker, swatchX + dx / 2.0, swatchY + dy / 2.0, col
                    )
                else:
                    c = self._defaultSwatch(
                        swatchX,
                        swatchY,
                        dx,
                        dy,
            ...
            if c:
                g.add(c)
                if scallout:
                    scallout(self, g, thisx, y0, i, (col, name), c)
            ...
            if self.colEndCallout and (i % columnMaximum == lim or i == (n - 1)):
                ...
                self.colEndCallout(
                    self, g, thisx, xt, yd, jOffs[-1], jOffs[-1] + dx + dxTextSpace
                )
...
class LineSwatch(Widget):
    ...
    _attrMap = AttrMap(
        ...
        strokeDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="dash array for swatch line"
        ),
    )
