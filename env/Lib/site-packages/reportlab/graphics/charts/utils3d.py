...
def _draw_3d_bar(
    ...
    if xdepth or ydepth:
        if y0 != yhigh:  # non-zero height
            _add_3d_bar(
                x2, x2, y0, yhigh, xdepth, ydepth, fillColor=fillColorShaded
            )  # side

        _add_3d_bar(
            x1, x2, usd, usd, xdepth, ydepth, fillColor=fillColorShadedTop
        )  # top

    G.add(
        Polygon(
            (x1, y0, x2, y0, x2, yhigh, x1, yhigh),
            strokeColor=strokeColor,
            strokeWidth=strokeWidth,
            fillColor=fillColor,
            strokeLineJoin=1,
    ...
    if xdepth or ydepth:
        G.add(
            Line(
                ...
                usd,
                strokeWidth=strokeWidth,
                strokeColor=strokeColor or fillColorShaded,
            )
        )
...
def _make_3d_line_info(
    ...
    c = (
        slope > depth_slope
        and _getShaded(fillColor, fillColorShaded, shading)
        or fillColor
    )
    zy0 = z0 * theta_y
    zx0 = z0 * theta_x
    ...
    for x, y in D:
        ...
        y_1 = y + zy0
        P = Polygon(
            _ystrip_poly(x_0, x_1, y_0, y_1, xdepth, ydepth),
            fillColor=c,
            strokeColor=c,
...
class _Segment:
    ...
    def intersect(self, o, I):
        ...
        if ox0 > self.x1:
            return 1
        if o.s == self.s and o.i in (self.i - 1, self.i + 1):
            return
        a = self.a
        b = self.b
        oa = o.a
    ...
    print(
        find_intersections(
            [
                ...
                [(0.2666666667, 0.4), (0.1, 0.4), (0.1, 0.2), (0, 0), (1, 1)],
                [(0, 1), (0.4, 0.1), (1, 0.1)],
            ]
        )
    )
