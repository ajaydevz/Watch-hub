...
class WedgeLabel(Label):
    ...
    def _getBoxAnchor(self):
        ...
        if ba in ("autox", "autoy"):
            ...
            ba = (
                getattr(self, "_anti", None) and _ANGLE2RBOXANCHOR or _ANGLE2BOXANCHOR
            )[na]
            self._checkDXY(ba)
        return ba
...
class WedgeProperties(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        strokeDashArray=AttrMapValue(
            isListOfNumbersOrNone,
            desc="Style of the wedge border, expressed as a list of lengths of alternating dashes and blanks",
        ),
        ...
        fontSize=AttrMapValue(
            isNumber, desc="Size of the font of the label text in points"
        ),
        fontColor=AttrMapValue(
            isColorOrNone, desc="Color of the font of the label text"
        ),
        labelRadius=AttrMapValue(
            isNumber,
            desc="Distance between the center of the label box and the center of the pie, expressed in times the radius of the pie",
        ),
        label_dx=AttrMapValue(isNumber, desc="X Offset of the label"),
        label_dy=AttrMapValue(isNumber, desc="Y Offset of the label"),
        label_angle=AttrMapValue(
            isNumber,
            desc="Angle of the label, default (0) is horizontal, 90 is vertical, 180 is upside down",
        ),
        label_boxAnchor=AttrMapValue(isBoxAnchor, desc="Anchoring point of the label"),
        label_boxStrokeColor=AttrMapValue(
            isColorOrNone, desc="Border color for the label box"
        ),
        label_boxStrokeWidth=AttrMapValue(
            isNumber, desc="Border width for the label box"
        ),
        label_boxFillColor=AttrMapValue(
            isColorOrNone, desc="Filling color of the label box"
        ),
        label_strokeColor=AttrMapValue(
            isColorOrNone, desc="Border color for the label text"
        ),
        label_strokeWidth=AttrMapValue(
            isNumber, desc="Border width for the label text"
        ),
        ...
        label_pointer_strokeColor=AttrMapValue(
            isColorOrNone, desc="Color of indicator line"
        ),
        label_pointer_strokeWidth=AttrMapValue(
            isNumber, desc="StrokeWidth of indicator line"
        ),
        label_pointer_elbowLength=AttrMapValue(
            isNumber, desc="Length of final indicator line segment"
        ),
        label_pointer_edgePad=AttrMapValue(
            isNumber, desc="pad between pointer label and box"
        ),
        label_pointer_piePad=AttrMapValue(
            isNumber, desc="pad between pointer label and pie"
        ),
        ...
        shadingAmount=AttrMapValue(
            isNumberOrNone, desc="amount by which to shade fillColor"
        ),
        shadingAngle=AttrMapValue(
            isNumber, desc="shading changes at multiple of this angle (in degrees)"
        ),
        shadingDirection=AttrMapValue(
            OneOf("normal", "anti"),
            desc="Whether shading is at start or end of wedge/sector",
        ),
        shadingKind=AttrMapValue(
            OneOf(None, "lighten", "darken"), desc="use colors.Whiter or Blacker"
        ),
    )
    ...
    def __init__(self):
        ...
        self.label_topPadding = (
            self.label_leftPadding
        ) = self.label_rightPadding = self.label_bottomPadding = 0
        ...
        self.label_leading = (
            self.label_width
        ) = self.label_maxWidth = self.label_height = None
...
class AbstractPieChart(PlotArea):
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        strokeWidth = getattr(
            style, "strokeWidth", getattr(baseStyle, "strokeWidth", None)
        )
...
def boundsOverlap(P, Q):
    return not (
        P[0] > Q[2] - 1e-2
        or Q[0] > P[2] - 1e-2
        or P[1] > (0.5 * (Q[1] + Q[3])) - 1e-2
        or Q[1] > (0.5 * (P[1] + P[3])) - 1e-2
    )
...
class Pie(AbstractPieChart):
    _attrMap = AttrMap(
        BASE=AbstractPieChart,
        data=AttrMapValue(
            isListOfNumbers,
            desc="List of numbers defining wedge sizes; need not sum to 1",
        ),
        labels=AttrMapValue(
            isListOfStringsOrNone,
        ...
        direction=AttrMapValue(
            OneOf("clockwise", "anticlockwise"), desc="'clockwise' or 'anticlockwise'"
        ),
        slices=AttrMapValue(None, desc="Collection of wedge descriptor objects"),
        simpleLabels=AttrMapValue(
            isBoolean,
            desc="If true(default) use a simple String not an advanced WedgeLabel. A WedgeLabel is customisable using the properties prefixed label_ in the collection slices.",
        ),
        other_threshold=AttrMapValue(
            isNumber,
            desc="A value for doing threshholding, not used yet.",
            advancedUsage=1,
        ),
        checkLabelOverlap=AttrMapValue(
            EitherOr((isNumberInRange(0.05, 1), isBoolean)),
            desc="If true check and attempt to fix\n standard label overlaps(default off)",
            advancedUsage=1,
        ),
        pointerLabelMode=AttrMapValue(
            OneOf(None, "LeftRight", "LeftAndRight"), desc="", advancedUsage=1
        ),
        sameRadii=AttrMapValue(
            isBoolean,
            desc="If true make x/y radii the same(default off)",
            advancedUsage=1,
        ),
        ...
        innerRadiusFraction=AttrMapValue(
            isNumberOrNone, desc="fraction of radii to start wedges at"
        ),
        wedgeRecord=AttrMapValue(
            None, desc="callable(wedge,*args,**kwds)", advancedUsage=1
        ),
        sideLabels=AttrMapValue(
            isBoolean,
            desc="If true attempt to make piechart with labels along side and pointers",
        ),
        sideLabelsOffset=AttrMapValue(
            isNumber,
            desc="The fraction of the pie width that the labels are situated at from the edges of the pie",
        ),
        labelClass=AttrMapValue(
            NoneOr(isCallable), desc="A class factory to use for non simple labels"
        ),
    )
    ...
    def makePointerLabels(self, angles, plMode):
        ...
        for i, A in angles:
            ...
            l._origdata = dict(
                ...
                piePad=style.label_pointer_piePad,
                elbowLength=style.label_pointer_elbowLength,
            )
            maxW = max(w, maxW)
            sumH += h + 2
        ...
        if sumH < self.height and (aonR or nl == n):
            ...
            side = None
        G, lu, ru, mel = _fixPointerLabels(
            len(angles), L, self.x, self.y, self.width, self.height, side=side
        )
        if plMode == "LeftAndRight":
            lu = ru = max(lu, ru)
        x0 = self.x + lu
        ...
        for i, (a1, a2) in angles:
            ...
            if innerRadiusFraction:
                theWedge = Wedge(
                    ...
                    a2,
                    yradius=yradius,
                    radius1=xradius * innerRadiusFraction,
                    yradius1=yradius * innerRadiusFraction,
                )
            else:
                if aa >= _ANGLEHI:
                ...
                if nshades > 1:
                    ...
                    for ish in range(nshades):
                        ...
                        if innerRadiusFraction:
                            ...
                            shWedge = Wedge(
                                cx, cy, xradius, sha1, sha2, yradius=yradius
                            )
            ...
            if wedgeStyle.label_visible:
                if not sideLabels:
                    if text:
                        ...
                        labelY = cy + ry * sinAA
                        l = _addWedgeLabel(
                            self, text, averageAngle, labelX, labelY, wedgeStyle
                        )
                        L_add(l)
                    ...
                    if text:
                        ...
                        if (
                            (abs(averageAngle) < 90)
                            or (averageAngle > 270 and averageAngle < 450)
                            or (-450 < averageAngle < -270)
                        ):
                            labelX = (
                                (1 + self.sideLabelsOffset) * self.width
                                + self.x
                                + slices_popout
                            )
                            rx = 0
                            ...
                            labelX = (
                                self.x
                                - (self.sideLabelsOffset) * self.width
                                - slices_popout
                            )
                            rx = 0
                        labelY = cy + ry * sinAA
...
class LegendedPie(Pie):
    ...
    _attrMap = AttrMap(
        ...
        legendNumberFormat=AttrMapValue(
            None, desc="Formatting routine for number on right hand side of legend."
        ),
        legendNumberOffset=AttrMapValue(
            isNumber, desc="Horizontal space between legend and numbers on r/hand side"
        ),
        pieAndLegend_colors=AttrMapValue(
            isListOfColors, desc="Colours used for both swatches and pie"
        ),
        legend_names=AttrMapValue(
            isNoneOrListOfNoneOrStrings, desc="Names used in legend (or None)"
        ),
        legend_data=AttrMapValue(
            isNoneOrListOfNoneOrNumbers,
            desc="Numbers used on r/hand side of legend (or None)",
        ),
    ...
    def __init__(self):
        ...
        self.pieAndLegend_colors = [
            PCMYKColor(11, 11, 72, 0, spotName="PANTONE 458 CV"),
        ...
        for f in range(len(self.data)):
            self.legend1.colorNamePairs.append(
                (self.pieAndLegend_colors[f], self.legend_names[f])
            )
        ...
        for f in range(len(self.data)):
            if self.legend_names == None:
                ...
                try:
                    ...
                    self.slices[f].fillColor = self.pieAndLegend_colors[
                        f % len(self.pieAndLegend_colors)
                    ]
                    self.legend1.colorNamePairs.append(
                        (
                            self.pieAndLegend_colors[f % len(self.pieAndLegend_colors)],
                            self.legend_names[f],
                        )
                    )
...
class Pie3d(Pie):
    ...
    def CX(self, i, d):
        return (
            self._cx
            + (d and self._xdepth_3d or 0)
            + self._popout(i) * cos(_2rad(self._sl3d[i].mid))
        )

    def CY(self, i, d):
        return (
            self._cy
            + (d and self._ydepth_3d or 0)
            + self._popout(i) * sin(_2rad(self._sl3d[i].mid))
        )
    ...
    def draw(self):
        ...
        for i in range(n):
            ...
            if aa < _ANGLELO:
                continue
            fillColor = _getShaded(
                style.fillColor, style.fillColorShaded, style.shading
            )
            strokeColor = (
                _getShaded(style.strokeColor, style.strokeColorShaded, style.shading)
                or fillColor
            )
            ...
            if style.label_visible and text:
                ...
                labelY = OY(i, mid, 0)
                l = _addWedgeLabel(
                    self, text, mid, labelX, labelY, style, labelClass=WedgeLabel3d
                )
                L.append(l)
                if checkLabelOverlap:
                    l._origdata = {
...
def sample2():
    ...
    pc.x = 125
    pc.y = 25
    pc.data = [0.31, 0.148, 0.108, 0.076, 0.033, 0.03, 0.019, 0.126, 0.15]
