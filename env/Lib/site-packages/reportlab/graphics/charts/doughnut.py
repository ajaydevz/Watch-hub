...
class Doughnut(AbstractPieChart):
    _attrMap = AttrMap(
        ...
        width=AttrMapValue(
            isNumber, desc="width of doughnut bounding box. Need not be same as width."
        ),
        height=AttrMapValue(
            isNumber,
        ...
        startAngle=AttrMapValue(
            isNumber, desc="angle of first slice; like the compass, 0 is due North"
        ),
        ...
        simpleLabels=AttrMapValue(
            isBoolean, desc="If true(default) use String not super duper WedgeLabel"
        ),
        # advanced usage
        ...
        sideLabels=AttrMapValue(
            isBoolean,
            desc="If true attempt to make chart with labels along side and pointers",
            advancedUsage=1,
        ),
    ...
    def normalizeData(self, data=None):
        ...
        return (
            abs(sum) >= 1e-8
            and list(map(lambda x, f=360.0 / sum: f * x, data))
            or len(data) * [0]
        )
        ...
        if multi:
            ...
            for sn, series in enumerate(normData):
                for i, angle in enumerate(series):
                    ...
                    if len(series) > 1:
                        theSector = Wedge(
                            cx, cy, xr, a1, a2, yradius=yr, radius1=xr1, yradius1=yr1
                        )
                    else:
                        theSector = Wedge(
                            cx,
                    ...
                    if shader:
                        ...
                        if nshades > 1:
                            shader = (
                                colors.Whiter if shader == "lighten" else colors.Blacker
                            )
                    ...
                    if sn == 0 and sectorStyle.visible and sectorStyle.label_visible:
                        ...
                        if text:
                            ...
                            labelX = centerx + (
                                0.5 * self.width * cos(aveAngleRadians) * labelRadius
                            )
                            labelY = centery + (
                                0.5 * self.height * sin(aveAngleRadians) * labelRadius
                            )
                            l = _addWedgeLabel(
                                self, text, averageAngle, labelX, labelY, sectorStyle
                            )
            ...
            for i, angle in enumerate(normData):
                ...
                if labels[i] and sectorStyle.visible and sectorStyle.label_visible:
                    ...
                    ry = yradius * labelRadius
                    l = _addWedgeLabel(
                        self, labels[i], averageAngle, labelX, labelY, sectorStyle
                    )
                    if checkLabelOverlap:
                        l._origdata = {
                            "x": labelX,
