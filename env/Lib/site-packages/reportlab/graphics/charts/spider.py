...
class StrandProperty(PropHolder):
    _attrMap = AttrMap(
        ...
        strokeDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="dashing pattern, e.g. (3,2)"
        ),
        symbol=AttrMapValue(
            EitherOr((isStringOrNone, isSymbol)),
...
class SpokeProperty(PropHolder):
    _attrMap = AttrMap(
        ...
        strokeColor=AttrMapValue(isColorOrNone, desc="stroke color"),
        strokeDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="dashing pattern, e.g. (2,1)"
        ),
...
class StrandLabel(SpokeLabel):
    _attrMap = AttrMap(
        BASE=SpokeLabel,
        format=AttrMapValue(
            EitherOr((isStringOrNone, isCallable)), desc="Format for the label"
        ),
...
class SpiderChart(PlotArea):
    _attrMap = AttrMap(
        ...
        strandLabelClass=AttrMapValue(
            NoneOr(isCallable), desc="A class factory to use for the strand labels"
        ),
        spokeLabels=AttrMapValue(
            None, desc="collection of spoke label descriptor objects"
        ),
        spokeLabelClass=AttrMapValue(
            NoneOr(isCallable), desc="A class factory to use for the spoke labels"
        ),
    )
    ...
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        strokeWidth = getattr(
            style, "strokeWidth", getattr(baseStyle, "strokeWidth", 0)
        )
    ...
    def normalizeData(self, outer=0.0):
        ...
        data = self.data
        assert (
            min(list(map(min, data))) >= 0
        ), "Cannot do spider plots of negative numbers!"
    ...
    def draw(self):
        ...
        for row in data:
            ...
            for i in range(n):
                ...
                points.append(cx + car * r)
                points.append(cy + sar * r)
                L = self._innerDrawLabel(
                    strandLabels[(rowIdx, i)],
                    r,
                    cx,
                    cy,
                    angle,
                    car,
                    sar,
                    labelClass=self.labelClass("strand"),
                )
                if L:
                    labs.append(L)
