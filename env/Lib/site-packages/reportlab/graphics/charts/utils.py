...
def ticks(
    lower, upper, n=(4, 5, 6, 7, 8, 9), split=1, percent=0, grid=None, labelVOffset=0
):
    """
...
def makeCircularString(
    x, y, radius, angle, text, fontName, fontSize, inside=0, G=None, textAnchor="start"
):
    ...
    if textAnchor != "start":
        if textAnchor == "middle":
            ...
            phi += (
                sig
                * float(
                    numericXShift(textAnchor, text, width, fontName, fontSize, None)
                )
                / radius
            )
    ...
    for letter in text:
        ...
        h.add(
            String(
                0, 0, letter, fontName=fontName, fontSize=fontSize, textAnchor="start"
            )
        )
        h.translate(
            x + cos(phi) * radius, y + sin(phi) * radius
