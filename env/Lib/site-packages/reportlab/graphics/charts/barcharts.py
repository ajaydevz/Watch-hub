...
class BarChartProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        strokeDashArray=AttrMapValue(
            isListOfNumbersOrNone, desc="Dash array of a line."
        ),
        symbol=AttrMapValue(
            None, desc="A widget to be used instead of a normal bar.", advancedUsage=1
        ),
        name=AttrMapValue(
            isString, desc="Text to be associated with a bar (eg seriesname)"
        ),
        swatchMarker=AttrMapValue(
            NoneOr(isSymbol), desc="None or makeMarker('Diamond') ...", advancedUsage=1
        ),
        minDimen=AttrMapValue(
            isNumberOrNone, desc="minimum width/height that will be drawn."
        ),
        isLine=AttrMapValue(
            NoneOr(isBoolean), desc="if this bar should be drawn as a line"
        ),
    )
...
class BarChart(PlotArea):
    ...
    _attrMap = AttrMap(
        BASE=PlotArea,
        useAbsolute=AttrMapValue(
            EitherOr((isBoolean, EitherOr((isString, isNumber)))),
            desc="Flag to use absolute spacing values; use string of gsb for finer control\n(g=groupSpacing,s=barSpacing,b=barWidth).",
            advancedUsage=1,
        ),
        ...
        barLabelFormat=AttrMapValue(
            None,
            desc="Formatting string or function used for bar labels. Can be a list or list of lists of such.",
        ),
        barLabelCallOut=AttrMapValue(
            None,
            desc="Callout function(label)\nlabel._callOutInfo = (self,g,rowNo,colNo,x,y,width,height,x00,y00,x0,y0)",
            advancedUsage=1,
        ),
        barLabelArray=AttrMapValue(
            None,
            desc="explicit array of bar label values, must match size of data if present.",
        ),
        reversePlotOrder=AttrMapValue(
            isBoolean,
            desc="If true, reverse common category plot order.",
            advancedUsage=1,
        ),
        naLabel=AttrMapValue(
            NoneOrInstanceOfNA_Label,
            desc="Label to use for N/A values.",
            advancedUsage=1,
        ),
        annotations=AttrMapValue(
            None, desc="list of callables, will be called with self, xscale, yscale."
        ),
        categoryLabelBarSize=AttrMapValue(
            isNumber,
            desc="width to leave for a category label to go between categories.",
        ),
        categoryLabelBarOrder=AttrMapValue(
            OneOf("first", "last", "auto"),
            desc="where any label bar should appear first/last",
        ),
        barRecord=AttrMapValue(
            None,
            desc="callable(bar,label=labelText,value=value,**kwds) to record bar information",
            advancedUsage=1,
        ),
        zIndexOverrides=AttrMapValue(
            isStringOrNone,
            desc="""None (the default ie use old z ordering scheme) or a ',' separated list of key=value (int/float) for new zIndex ordering. If used defaults are
    background=0,
    categoryAxis=1,
    valueAxis=2,
    bars=3,
    ...
    annotations=7""",
        ...
        categoryNALabel=AttrMapValue(
            NoneOrInstanceOfNA_Label,
            desc="Label to use for a group of N/A values.",
            advancedUsage=1,
        ),
        seriesOrder=AttrMapValue(
            SequenceOf(
                SequenceOf(isInt, emptyOK=0, NoneOK=0, lo=1), emptyOK=0, NoneOK=1, lo=1
            ),
            "dynamic 'mixed' category style case",
        ),
    )
    ...
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        strokeColor = getattr(
            style, "strokeColor", getattr(baseStyle, "strokeColor", None)
        )
        fillColor = getattr(style, "fillColor", getattr(baseStyle, "fillColor", None))
        strokeDashArray = getattr(
            style, "strokeDashArray", getattr(baseStyle, "strokeDashArray", None)
        )
        strokeWidth = getattr(style, "strokeWidth", getattr(style, "strokeWidth", None))
        swatchMarker = getattr(
            style, "swatchMarker", getattr(baseStyle, "swatchMarker", None)
        )
        if swatchMarker:
            return uSymbol2Symbol(
                swatchMarker, x + width / 2.0, y + height / 2.0, fillColor
            )
            ...
            if S:
                S = uSymbol2Symbol(
                    S, x + width / 2.0, yh2, style.strokeColor or style.fillColor
                )
            if S and L:
                g = Group()
                g.add(L)
    ...
    def __init__(self):
        assert self.__class__.__name__ not in ("BarChart", "BarChart3D"), (
            "Abstract Class %s Instantiated" % self.__class__.__name__
        )
        ...
        self.categoryAxis._attrMap["style"].validate = OneOf(
            "stacked", "parallel", "parallel_3d", "mixed"
        )
    ...
    def _drawBegin(self, org, length):
        ...
        return (
            crossesAt
            if vA.forceZero or (crossesAt >= org and crossesAt <= org + length)
            else org
        )
    ...
    def calcBarPositions(self):
        ...
        bars = self.bars
        lineCount = sum(
            (int(bars.checkAttr(_, "isLine", False)) for _ in range(seriesCount))
        )
        ...
        if style != "mixed":
            ...
            lineSeen = 0
            for sb, sg in enumerate(
                self._seriesOrder
            ):  # the sub bar nos and series groups
    ...
    def _addNABarLabel(
        self, g, rowNo, colNo, x, y, width, height, calcOnly=False, na=None
    ):
        if na is None:
            na = self.naLabel
        if na and na.text:
            ...
            return self._addLabel(
                na.text, na, g, rowNo, colNo, x, y, width, height, calcOnly=calcOnly
            )

    def _addLabel(
        self, text, label, g, rowNo, colNo, x, y, width, height, calcOnly=False
    ):
        if label.visible:
            ...
            for colNo, (x, y, width, height) in enumerate(row):
                ...
                if None in (width, height):
                    if not catNAL or colNo in catNNA:
                        ...
                        r0 = self._addNABarLabel(
                            lg, rowNo, colNo, x, y, width, height, True, catNAL
                        )
                        if r0:
                            x, y, width, height = BP[rowNoL][colNo]
                            r1 = self._addNABarLabel(
                                lg, rowNoL, colNo, x, y, width, height, True, catNAL
                            )
                            x = (r0[0] + r1[0]) / 2.0
                            y = (r0[1] + r1[1]) / 2.0
                            self._addNABarLabel(
                                lg, rowNoL, colNo, x, y, 0.0001, 0.0001, na=catNAL
                            )
                        CBL.append(colNo)
                    if isLine:
                        linePts(None)
                ...
                if isLine:
                    ...
                    if symbol:
                        sym = uSymbol2Symbol(
                            tpcGetItem(symbol, colNo),
                            xL,
                            yL,
                            style.strokeColor or style.fillColor,
                        )
                        if sym:
                            lineSyms(sym)
                    ...
                    abs(width) > 1e-7
                    and abs(height) >= 1e-7
                    and (style.fillColor is not None or style.strokeColor is not None)
                ):
                    self._makeBar(g, x, y, width, height, rowNo, style)
                    if br:
                        br(
                            g.contents[-1],
                            label=self._getLabelText(rowNo, colNo),
                            value=self.data[rowNo][colNo],
                            rowNo=rowNo,
                            colNo=colNo,
                        )
        ...
        for i, ci in enumerate(C):
            ...
            if alo > ahi:
                if alo > maxS:
                    ...
                    maxP = (
                        flipXY
                        and (lo, v0, lo + alo, v0 + size, 0)
                        or (v0, lo, v0 + size, lo + alo, 0)
                    )
                    if maxS >= required:
                        break
            elif ahi > maxS:
                ...
                maxP = (
                    flipXY
                    and (hi - ahi, v0, hi, v0 + size, 1)
                    or (v0, hi - ahi, v0 + size, hi, 1)
                )
                if maxS >= required:
                    break
        return maxS, maxP
                ...
                if None in (width, height):
                    ...
                    if na and na.text:
                        ...
                        if height is None:
                            height = v
                        C(
                            self._computeLabelPosition(
                                na.text, na, rowNo, colNo, x, y, width, height
                            )
                        )
                    else:
                        C(None)
...
class BarChart3D(BarChart):
    ...
    def makeBars(self):
        ...
        if cAStyle == "stacked":
            ...
            fg_value = [
                _[1] for _ in sorted((((t[1], t[2], t[3], t[4]), t) for t in fg_value))
            ]
...
class VerticalBarChart3D(BarChart3D, VerticalBarChart):
    _cmpZ = lambda self, a, b: cmp(
        (-a[1], a[3], a[0], -a[4]), (-b[1], b[3], b[0], -b[4])
    )


class HorizontalBarChart3D(BarChart3D, HorizontalBarChart):
    _cmpZ = lambda self, a, b: cmp(
        (-a[1], a[4], a[0], -a[3]), (-b[1], b[4], b[0], -b[3])
    )  # t, z0, z1, x, y = a[:5]
...
def sampleV2c():
    ...
    data = [(2.4, -5.7, 2, 5, 9.99), (0.6, -4.9, -3, 4, 9.99)]
