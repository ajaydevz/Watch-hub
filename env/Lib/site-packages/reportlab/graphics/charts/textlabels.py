...
class Label(Widget):
    ...
    _attrMap = AttrMap(
        ...
        angle=AttrMapValue(
            isNumber,
            desc="angle of label: default (0), 90 is vertical, 180 is upside down, etc",
        ),
        ...
        maxWidth=AttrMapValue(
            isNumberOrNone, desc="maximum width the label can grow to"
        ),
        height=AttrMapValue(isNumberOrNone, desc="the height of the text"),
        textAnchor=AttrMapValue(
            isTextAnchor, desc="the anchoring point of the text inside the label"
        ),
        ...
        useAscentDescent=AttrMapValue(
            isBoolean,
            desc="If True then the font's Ascent & Descent will be used to compute default heights and baseline.",
        ),
        customDrawChanger=AttrMapValue(
            isNoneOrCallable,
            desc="An instance of CustomDrawChanger to modify the behavior at draw time",
            _advancedUsage=1,
        ),
        ddf=AttrMapValue(
            NoneOr(isSubclassOf(DirectDraw), "NoneOrDirectDraw"),
            desc="A DirectDrawFlowable instance",
            _advancedUsage=1,
        ),
        ddfKlass=AttrMapValue(
            NoneOr(isSubclassOf(Flowable), "NoneOrDirectDraw"),
            desc="A Flowable class for direct drawing (default is XPreformatted",
            _advancedUsage=1,
        ),
        ddfStyle=AttrMapValue(
            NoneOr((isSubclassOf(PropertySet), isInstanceOf(PropertySet))),
            desc="A style or style class for a ddfKlass or None",
            _advancedUsage=1,
        ),
    )
    ...
    def computeSize(self):
        ...
        if not ddfKlass:
            ...
            self._lines = simpleSplit(
                self._text, self.fontName, self.fontSize, self.maxWidth
            )
            if not self.width:
            ...
            if self.ddf is None:
                raise RuntimeError(
                    "DirectDrawFlowable class is not available you need the rlextra package as well as reportlab"
                )
            sty = dict(
            ...
            elif isinstance(self.ddfStyle, type) and issubclass(
                self.ddfStyle, PropertySet
            ):
    ...
    def _rawDraw(self):
        ...
        if ddfKlass:
            ...
            strokeColor, strokeWidth, leading = (
                self.strokeColor,
                self.strokeWidth,
                self._leading,
            )
...
class BarChartLabel(PMVLabel):
    ...
    _attrMap = AttrMap(
        ...
        fixedEnd=AttrMapValue(
            NoneOrInstanceOfLabelOffset, desc="None or fixed draw ends +/-"
        ),
        fixedStart=AttrMapValue(
            NoneOrInstanceOfLabelOffset, desc="None or fixed draw starts +/-"
        ),
...
class XLabel(Label):
    ...
    if False:
        ...
        def computeSize(self):
            # the thing will draw in its own coordinate system
            self._lineWidths = []
            sty = self._style = ParagraphStyle(
                "xlabel-generated",
                fontName=self.fontName,
                fontSize=self.fontSize,
                fillColor=self.fillColor,
