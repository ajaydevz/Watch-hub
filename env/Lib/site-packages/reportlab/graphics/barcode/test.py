...
def run():
    ...
    storyAdd(
        createBarcodeDrawing("ISBN", value="9781565924796", price="01299", quiet=False)
    )
    ...
    storyAdd(Paragraph("Label Size", styleN))
    storyAdd(XBox((1.75) * inch, 0.5 * inch, '1/2x1-3/4"'))
    ...
    if pylibdmtx:
        ...
        d = Drawing(100, 100)
        d.add(
            Rect(0, 0, 100, 100, strokeWidth=1, strokeColor=colors.red, fillColor=None)
        )
        addCross(d, 10, 10)
        ...
        d.add(
            DataMatrixWidget(
                value="1234567890", x=10, y=10, color="black", bgColor="lime"
            )
        )
        storyAdd(d)
...
def fullTest(fileName="test_full.pdf"):
    ...
    story.append(
        Paragraph(
            "ReportLab %s Barcode Test Suite - full output" % __RL_Version__, styleH
        )
    )
...
if __name__ == "__main__":
    ...
    createSample(
        "test_cbcim.png", createBarcodeImageInMemory("EAN13", value="123456789012")
    )
    createSample(
        "test_cbcim.gif",
        createBarcodeImageInMemory("EAN8", value="1234567", format="gif"),
    )
    createSample(
        "test_cbcim.pdf",
        createBarcodeImageInMemory(
            "UPCA", value="03600029145", format="pdf", barHeight=40
        ),
    )
    createSample(
        "test_cbcim.tiff",
