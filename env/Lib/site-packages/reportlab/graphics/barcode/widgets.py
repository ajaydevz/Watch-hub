...
class _BarcodeWidget(PlotArea):
    ...
    def rect(self, x, y, w, h, **kw):
        ...
        for k, v in (
            ("strokeColor", self.barStrokeColor),
    ...
    def draw(self):
        if not self._BCC:
            raise NotImplementedError(
                "Abstract class %s cannot be drawn" % self.__class__.__name__
            )
        self.canv = self
        G = Group()
        self._Gadd = G.add
...
def _BCW(doc, codeName, attrMap, mod, value, **kwds):
    ...
    ns["doc"] = ("\n\t'''%s'''" % doc) if doc else ""
    code = (
        """class %(name)s(_BarcodeWidget,%(codeName)s):%(doc)s
\t_BCC = %(codeName)s
\tcodeName = %(codeName)r
...
BarcodeI2of5 = _BCW(
    ...
    AttrMap(
        ...
        bearers=AttrMapValue(
            isNumber,
            """(float, in units of barWidth. default 3.0):
    ...
    1234,
    _tests=["12", "1234", "123456", "12345678", "1234567890"],
)
...
BarcodeCode128 = _BCW(
    ...
    "Code128",
    AttrMap(
        BASE=BarcodeI2of5, UNWANTED=("bearers", "checksum", "ratio", "checksum", "stop")
    ),
...
BarcodeFIM = _BCW(
    ...
    AttrMap(
        ...
        spaceWidth=AttrMapValue(
            isNumber,
            """(float or None, default 1/16in):
                        width of intercharacter gap. None means "use barWidth".""",
        ),
...
BarcodePOSTNET = _BCW(
    ...
    AttrMap(
        BASE=_BarcodeWidget,
        barWidth=AttrMapValue(
            isNumber, """(float, default 0.018*in): the bar width."""
        ),
        spaceWidth=AttrMapValue(
            isNumber,
            """(float or None, default 0.0275in): width of intercharacter gap.""",
        ),
        shortHeight=AttrMapValue(
            isNumber, """(float, default 0.05in): The short bar height."""
        ),
        barHeight=AttrMapValue(
            isNumber, """(float, default 0.125in): The full bar height."""
        ),
        fontName=AttrMapValue(isString, desc="human readable font"),
        fontSize=AttrMapValue(isNumber, desc="human readable font size"),
        humanReadable=AttrMapValue(isBoolean, desc="if human readable"),
...
BarcodeECC200DataMatrix = _BCW(
    ...
    AttrMap(
        BASE=_BarcodeWidget,
        x=AttrMapValue(
            isNumber, desc="X position of the lower-left corner of the barcode."
        ),
        y=AttrMapValue(
            isNumber, desc="Y position of the lower-left corner of the barcode."
        ),
