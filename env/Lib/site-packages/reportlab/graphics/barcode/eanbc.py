...
class Ean5BarcodeWidget(Ean13BarcodeWidget):
    ...
    _attrMap = AttrMap(
        BASE=Ean13BarcodeWidget,
        price=AttrMapValue(isBoolean, desc="whether to display the price or not"),
...
class ISBNBarcodeWidget(Ean13BarcodeWidget):
    ...
    _attrMap = AttrMap(
        BASE=Ean13BarcodeWidget,
        price=AttrMapValue(NoneOr(nDigits(5)), desc="None or the price to display"),
    )
    ...
    def _add_human_readable(self, s, gAdd):
        ...
        gAdd(
            String(
                x, y, isbn, fontName=fontName, fontSize=fontSize, fillColor=textColor
            )
        )
