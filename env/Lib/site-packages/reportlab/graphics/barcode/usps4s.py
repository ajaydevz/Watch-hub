...
class USPS_4State(Barcode):
    ...
    def __init__(self, value="01234567094987654321", routing="", **kwd):
        ...
        if not routing:
            ...
            raise ValueError(
                "routing length must be 5, 9 or 11 digits not %d" % len(routing)
            )
    ...
    def binary(self):
        ...
        if not value:
            ...
            try:
                ...
                raise ValueError(
                    "Problem converting %s, routing code must be 0, 5, 9 or 11 digits"
                    % routing
                )
            ...
            try:
                ...
                raise ValueError(
                    "Problem converting %s, barcode identifier must be 2 digits"
                    % svalue
                )
            ...
            for name, nd in (
                ...
                try:
                    ...
                    raise ValueError(
                        "Problem converting %s, %s must be %d digits"
                        % (svalue, name, nd)
                    )
            self._bvalue = value
        return value
    ...
    def codewords(self):
        ...
        if not self._codewords:
            ...
            A, C = divmod(A, 1365)
            A, B = divmod(A, 1365)
            assert 0 <= A <= 658, "improper value %s passed to _2codewords A-->%s" % (
                hex(int(value)),
                A,
            )
    ...
    _bits2bars = "T", "D", "A", "F"
    horizontalClearZone = property(
        lambda self: self.scale("hcz", self.dimensions, self.widthScale)
    )
    verticalClearZone = property(
        lambda self: self.scale("vcz", self.dimensions, self.heightScale)
    )
    ...
    def barHeight(self):
        ...
        return self.scale("F", self.tops, self.heightScale) - self.scale(
            "F", self.bottoms, self.heightScale
        )
    ...
    def value(self):
        ...
        return " ".join(
            (tracking[0:2], tracking[2:5], tracking[5:11], tracking[11:]) + routing
        )

    value = property(
        value, lambda self, value: self.__dict__.__setitem__("tracking", value)
    )
...
def _initNof13Table(N, lenT):
    ...
    for c in range(8192):
        ...
        r = _ru13(c)
        if r < c:
            continue  # we already looked at this pair
        if r == c:
            T[u] = c
            u -= 1
        else:
