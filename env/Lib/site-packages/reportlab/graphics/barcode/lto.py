...
class BaseLTOLabel(Standard39):
    ...
    def __init__(
        ...
        label = "%sL%s" % (
            (prefix + str(number or 0).zfill(6 - len(prefix)))[:6],
            subtype,
        )
        if len(label) != 8:
            raise ValueError(
                "Invalid set of parameters (%s, %s, %s)" % (prefix, number, subtype)
            )
        self.label = label
        Standard39.__init__(
            self,
        ...
        for i in range(self.NBBLOCKS):
            ...
            if self.colored and part.isdigit():
                canvas.setFillColorRGB(
                    *getattr(
                        colors, self.COLORSCHEME[int(part)], colors.Color(1, 1, 1)
                    ).rgb()
                )
            else:
...
def test():
    ...
    ypos += lineheight
    count += 1
    VerticalLTOLabel("RL", count, "3", border=0.0125, colored=True).drawOn(
        canvas, xpos, ypos
    )
    canvas.showPage()
    canvas.save()
