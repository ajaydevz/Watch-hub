        ...
        for i, c in enumerate(data):
            try:
                ...
                raise UnicodeEncodeError(
                    "qrkanji", data, i, i + 1, "illegal multibyte sequence"
                )
            c = c << 8 | d
            if 0x8140 <= c <= 0x9FFC:
                c -= 0x8140
    ...
    def unicode_to_qrhanzi(self, data):
        ...
        for i, c in enumerate(data):
            try:
                ...
                raise UnicodeEncodeError(
                    "qrhanzi", data, i, i + 1, "illegal multibyte sequence"
                )
            c = c << 8 | d
            if 0xA1A1 <= c <= 0xAAFE:
                c -= 0xA1A1
...
class QRCode:
    ...
    def makeImpl(self, test, maskPattern):
        ...
        if self.dataCache == None:
            self.dataCache = QRCode.createData(
                self.version, self.errorCorrectLevel, self.dataList
            )
    ...
    def _dataPosIterator(self):
        ...
        rows = (
            list(range(9, self.moduleCount - 8)),
        ...
        ppos = set(
            itertools.chain.from_iterable((p - 2, p - 1, p, p + 1, p + 2) for p in ppos)
        )
            ...
            for row in rows[rowidx]:
                for c in range(2):
                    ...
                    if row in ppos and c in ppos:
                        if not (
                            row < 11
                            and (c < 11 or c > maxpos)
                            or c < 11
                            and (row < 11 or row > maxpos)
                        ):
                            continue
    ...
    def mapData(self, data, maskPattern):
        ...
        for (col, row), dark in zip_longest(
            self.dataPosIterator(), bits, fillvalue=False
        ):
    ...
    def createBytes(buffer, rsBlocks):
        ...
        for block in rsBlocks:
            ...
            ecdata.append(
                [(modPoly.get(i) if i >= 0 else 0) for i in range(mLen - rLen, mLen)]
            )

        data = [
            d
            for dd in itertools.chain(zip_longest(*dcdata), zip_longest(*ecdata))
            for d in dd
            if d is not None
        ]
        return data
    ...
    def maskScoreRule1vert(cls, modules):
        ...
        for row in modules:
            ...
            if lastRow:
                ...
                scores = [
                    a and (b - 4 + 3)
                    for a, b in zip_longest(changed, lastCount, fillvalue=0)
                    if b >= 4
                ]
                score += sum(scores)
                lastCount = [
...
class QRPolynomial:
    ...
    def mod(self, e):
        ...
        num = [
            nn ^ QRMath.gexp(QRMath.glog(en) + ratio) for nn, en in zip(self.num, e.num)
        ]
