...
class DataMatrix(Barcode, _DMTXCheck):
    ...
    @anchor.setter
    def anchor(self, v):
        self._anchor = self._checkVal(
            "anchor", v, ("n", "ne", "e", "se", "s", "sw", "w", "nw", "c")
        )
        self._recalc = True
    ...
    def recalc(self):
                ...
                (
                    "".join(
                        (
                            ...
                            for j in range(i, i + rowLen, cellLen)
                        )
                    ).strip()
        ...
        if cellWidth:
            ...
            if len(cellWidth) > 2:
                raise ValueError(
                    "cellSize needs to be distance x distance not %r" % self._cellSize
                )
...
class DataMatrixWidget(Widget, _DMTXCheck):
    ...
    def draw(self):
        m = DataMatrix(
            value=self.value, **{k: getattr(self, k) for k in self._defaults}
        )
        m.canv = self
        m.y += m.height
        g = Group()
