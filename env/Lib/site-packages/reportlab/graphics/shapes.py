...
from reportlab.rl_config import (
    shapeChecking,
    verbose,
    defaultGraphicsFontName as _baseGFontName,
    _unset_,
    decimalSymbol,
)
...
class Shape(_SetKeyWordArgs, _DrawTimeResizeable):
    ...
    def copy(self):
        ...
        raise NotImplementedError(
            "No copy method implemented for %s" % self.__class__.__name__
        )
    ...
    def verify(self):
        ...
        if self._attrMap is not None:
            for key in self.__dict__.keys():
                if key[0] != "_":
                    assert (
                        key in self._attrMap
                    ), "Unexpected attribute %s found in %s" % (key, self)
            for attr, metavalue in self._attrMap.items():
                assert hasattr(self, attr), "Missing attribute %s from %s" % (
                    attr,
                    self,
                )
                ...
                assert metavalue.validate(
                    value
                ), "Invalid value %s for attribute %s in class %s" % (
                    value,
                    attr,
...
class Group(Shape):
    ...
    _attrMap = AttrMap(
        transform=AttrMapValue(
            isTransform, desc="Coordinate transformation to apply", advancedUsage=1
        ),
        ...
        fillOverprint=AttrMapValue(
            isBoolean, desc="Turn on fill overprinting", advancedUsage=1
        ),
        overprintMask=AttrMapValue(
            isBoolean, desc="overprinting for ordinary CMYK", advancedUsage=1
        ),
    )
    ...
    def insert(self, i, n, name=None):
        ...
        if n is not None:
            assert isValidChild(
                n
            ), "Can only insert Shape or UserNode objects in a Group"
            if i < 0:
                self.contents[i:i] = [n]
            else:
...
class Drawing(Group, Flowable):
    ...
    _xtraAttrMap = AttrMap(
        ...
        vAlign=AttrMapValue(
            OneOf("TOP", "BOTTOM", "CENTER", "CENTRE"),
        ...
        initialFontName=AttrMapValue(
            isStringOrNone, desc="override the STATE_DEFAULTS value for fontName"
        ),
        initialFontSize=AttrMapValue(
            isNumberOrNone, desc="override the STATE_DEFAULTS value for fontSize"
        ),
    )
    ...
    _attrMap = AttrMap(
        ...
        formats=AttrMapValue(
            SequenceOf(OneOf(*_saveModes), lo=1, emptyOK=0),
    ...
    def draw(self, showBoundary=_unset_):
        ...
        renderPDF.draw(
            ...
            if showBoundary is not _unset_
            else getattr(self, "_showBoundary", _unset_),
        )
    ...
    def save(
        self, formats=None, verbose=None, fnRoot=None, outDir=None, title="", **kw
    ):
        ...
        if not fnRoot:
            fnRoot = getattr(
                self, "fileNamePattern", (self.__class__.__name__ + "%03d")
            )
        ...
        if not os.path.isabs(outDir):
            outDir = os.path.join(
                getattr(self, "_override_CWD", os.path.dirname(sys.argv[0])), outDir
            )
        ...
        verbose = (
            verbose is not None and (verbose,) or (getattr(self, "verbose", verbose),)
        )[0]
    ...
    def asString(self, format, verbose=None, preview=0, **kw):
        ...
        if format == "pdf":
            ...
            return renderPS.drawToString(
                self, showBoundary=getattr(self, "showBorder", rl_config.showBoundary)
            )
        elif format == "py":
    ...
    def resized(self, kind="fit", lpad=0, rpad=0, bpad=0, tpad=0):
        ...
        if kind == "fit" or (
            kind == "expand"
            and (xL < lpad or xH > oW - rpad or yL < bpad or yH > oH - tpad)
        ):
...
class isStrokeDashArray(Validator):
    def test(self, x):
        return isListOfNumbersOrNone.test(x) or (
            isinstance(x, (list, tuple)) and isNumber(x[0]) and isListOfNumbers(x[1])
        )
...
class LineShape(Shape):
    ...
    _attrMap = AttrMap(
        ...
        strokeDashArray=AttrMapValue(
            isStrokeDashArray, desc="[numbers] or [phase,[numbers]]"
        ),
...
class SolidShape(LineShape):
    ...
    _attrMap = AttrMap(
        ...
        fillColor=AttrMapValue(
            isColorOrNone, desc="filling color of the shape, e.g. red"
        ),
    ...
    def __init__(self, points=[], **kw):
        ...
        if lenPoints:
            if isSeq(points[0]):
                ...
                assert (
                    len(points) % 2 == 0
                ), "Point list must have even number of elements!"
        self.points = points
...
class Hatching(Path):
    ...
    _attrMap = AttrMap(
        BASE=Path,
        xyLists=AttrMapValue(
            EitherOr(
                (isListOfNumbers, SequenceOf(isListOfNumbers, lo=1)), "xy list(s)"
            ),
            desc="list(s) of numbers in the form x1, y1, x2, y2 ... xn, yn",
        ),
        angles=AttrMapValue(
            EitherOr((isNumber, isListOfNumbers, "angle(s)")),
            desc="the angle or list of angles at which hatching lines should be drawn",
        ),
        spacings=AttrMapValue(
            EitherOr((isNumber, isListOfNumbers, "spacings(s)")),
            desc="orthogonal distance(s) between hatching lines",
        ),
    )
...
def numericXShift(
    tA, text, w, fontName, fontSize, encoding=None, pivotCharacter=decimalSymbol
):
...
class String(Shape):
    ...
    _attrMap = AttrMap(
        ...
        textAnchor=AttrMapValue(
            OneOf("start", "middle", "end", "numeric"),
            desc="treat (x,y) as one of the options below.",
        ),
        encoding=AttrMapValue(isString),
        textRenderMode=AttrMapValue(
            OneOf(0, 1, 2, 3, 4, 5, 6, 7),
            desc="Control whether text is filled/stroked etc etc",
        ),
    )
    encoding = "utf8"
    ...
    def getEast(self):
        return self.x + stringWidth(
            self.text, self.fontName, self.fontSize, self.encoding
        )
    ...
    def getBounds(self):
        ...
        if tA != "start":
            if tA == "middle":
                ...
                x -= numericXShift(
                    tA, t, w, self.fontName, self.fontSize, self.encoding
                )
