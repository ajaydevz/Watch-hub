...
def _errorDump(fontName, fontSize):
    ...
    code = (
        'raise RenderPMError("Error in setFont(%s,%s) missing the T1 files?\\nOriginally %s: %s")'
...
def __makeTextPathsCode__(tp=None, _TP=("freetype", "_renderPM")):
    ...
    if textPaths == "backend":
        ...
        raise ValueError(
            f"textPaths={textPaths!r} should be one of 'backend', 'freetype' or '_renderPM')"
        )
    ...
    for tp in TP:
        if tp == "freetype":
            ...
            class FTTextPath:
                ...
                def setFont(self, fontName):
                    if fontName not in self.faces:
                        ...
                        if not font:
                            raise ValueError(
                                f"font {fontName!r} has not been registered"
                            )
                ...
                def _text2Path(
                    self, text, x=0, y=0, fontName=_baseGFontName, fontSize=1000, **kwds
                ):
            ...
            return dict(
                text2PathDescription=text2PathDescription, FTTextPath=FTTextPath
            )
        elif tp == "_renderPM":
            try:
                import _rl_renderPM
                ...
                if font._dynamicFont:
                    ...
                    for i, (f, t) in enumerate(FT):
                        ...
                        for g in gs._stringPath(t, x, y):
                            P_extend(
                                processGlyph(
                                    g, truncate=truncate, pathReverse=pathReverse
                                )
                            )
                        if i != nm1:
            ...
            return dict(
                text2PathDescription=text2PathDescription,
                processGlyph=processGlyph,
                setFont=setFont,
            )
    else:
        ...
        def _(*args, **kwds):
            raise RuntimeError(
                f"""This installation of reportLab has neither PYCAIRO or RENDERPM extras installed.
