...
class PSCanvas:
    def __init__(self, size=(300, 300), PostScriptLevel=2):
        ...
        self._strokeColor = (
            ...
            self._font
        ) = self._fontSize = self._lineCap = self._lineJoin = self._color = None
    ...
    def drawImage(
        self, image, x1, y1, width=None, height=None
    ):  # Postscript Level2 version
...
"""
        ...
        if self._color != color:
            ...
            if color:
                if hasattr(color, "cyan"):
                    self.code_append(
                        "%s setcmykcolor"
                        % fp_str(color.cyan, color.magenta, color.yellow, color.black)
                    )
                else:
                    self.code_append(
                        "%s setrgbcolor" % fp_str(color.red, color.green, color.blue)
                    )
    ...
    def _issueT1String(self, fontObj, x, y, s, textRenderMode=0):
        ...
        for f, t in unicode2T1(s, [fontObj] + fontObj.substitutionFonts):
            if f != fc:
                ...
                code_append(
                    "(%s) findfont %s scalefont setfont" % (psName, fp_str(fontSize))
                )
    ...
    def drawString(self, x, y, s, angle=0, text_anchor="left", textRenderMode=0):
        ...
        if needFill or needStroke:
            ...
            if not self.code[self._fontCodeLoc]:
                ...
                self.code[self._fontCodeLoc] = "(%s) findfont %s scalefont setfont" % (
                    psName,
            ...
            if angle != 0:
                self.code_append(
                    "gsave %s translate %s rotate" % (fp_str(x, y), fp_str(angle))
                )
                x = y = 0
    ...
    def drawCentredString(self, x, y, text, text_anchor="middle", textRenderMode=0):
        self.drawString(
            x, y, text, text_anchor=text_anchor, textRenderMode=textRenderMode
        )

    def drawRightString(self, text, x, y, text_anchor="end", textRenderMode=0):
        self.drawString(
            text, x, y, text_anchor=text_anchor, textRenderMode=textRenderMode
        )
    ...
    def rect(self, x1, y1, x2, y2, stroke=1, fill=1):
        ...
        self.polygon(
            ((x1, y1), (x2, y1), (x2, y2), (x1, y2)), closed=1, stroke=stroke, fill=fill
        )
    ...
    def roundRect(self, x1, y1, x2, y2, rx=8, ry=8):
        ...
        ellipsePath = (
            "matrix currentmatrix %s %s translate %s %s scale 0 0 1 %s %s arc setmatrix"
        )
    ...
    def drawArc(self, x1, y1, x2, y2, startAng=0, extent=360, fromcenter=0):
        ...
        if self._strokeColor != None:
            ...
            self.setColor(self._strokeColor)
            (startx, starty) = (
                cx + rx * math.cos(startAngleRadians),
                cy + ry * math.sin(startAngleRadians),
            )
    ...
    def _drawImageLevel1(self, image, x1, y1, width=None, height=None):
        ...
        self.code.extend(
            [
                ...
                % imgwidth,  # scanline by multiples of image width
            ]
        )
        ...
        # piddlePDF again
        rawimage = (
            myimage.tobytes if hasattr(myimage, "tobytes") else myimage.tostring
        )()
...
/BitsPerComponent %d"""
            % (imwidth, imheight, imBitsPerComponent)
        )
        ...
        if self._canvas._fillColor or textRenderMode:
            ...
            if not text_anchor in ["start", "inherited"]:
                ...
                if text_anchor == "end":
                    ...
                    x -= numericXShift(
                        text_anchor, text, textLen, font, fontSize, encoding="winansi"
                    )
                else:
