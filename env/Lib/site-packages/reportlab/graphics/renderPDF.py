...
from reportlab.graphics.renderbase import (
    Renderer,
    getStateDelta,
    renderScaledDrawing,
    STATE_DEFAULTS,
...
class _PDFRenderer(Renderer):
    ...
    def applyStateChanges(self, delta, newState):
        ...
        for key, value in (
            sorted(delta.items()) if rl_config.invariant else delta.items()
        ):
            if key == "transform":
...
def drawToFile(
    d, fn, msg="", showBoundary=rl_config._unset_, autoSize=1, canvasKwds={}
):
