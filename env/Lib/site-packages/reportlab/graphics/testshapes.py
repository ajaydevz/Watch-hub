...
def _setup():
    ...
    if sys.platform == "win32":
        for name, ttf in [
            ("Adventurer Light SF", "Advlit.ttf"),
            ("ArialMS", "ARIAL.TTF"),
...
def getFailedDrawing(funcName):
    ...
    s = String(
        200, 40, "Error in generating function '%s'!" % funcName, textAnchor="middle"
    )
    D.add(s)
...
def getDrawing06():
    ...
    D.add(
        Rect(
            120, 150, 60, 30, strokeWidth=10, strokeColor=colors.yellow, fillColor=green
        )
    )  # square corners
...
def smallArrow():
    ...
    b = base64_decodebytes(
        b"""R0lGODdhCgAHAIMAAP/////29v/d3f+ysv9/f/9VVf9MTP8iIv8ICP8AAAAAAAAAAAAAAAAAAAAA
