...
class SVGCanvas:
    def __init__(self, size=(300, 300), encoding="utf-8", verbose=0, bom=False, **kwds):
        ...
        self._strokeColor = (
            ...
            self._lineWidth
        ) = self._font = self._fontSize = self._lineCap = self._lineJoin = None
        ...
        if not useClip:
            ...
            clipRect = transformNode(
                self.doc, "rect", x=0, y=0, width=self.width, height=self.height
            )
        ...
        if scaleGroupId:
            self.scaleTree = transformNode(
                self.doc, "g", id=scaleGroupId, transform="scale(1,1)"
            )
    ...
    def _genArcCode(self, x1, y1, x2, y2, startAng, extent):
        ...
        codeline = (
            "matrix currentmatrix %s %s translate %s %s scale 0 0 1 %s %s %s setmatrix"
        )
    ...
    def _fillAndStroke(
        self, code, clip=0, link_info=None, styles=AREA_STYLES, fillMode=None
    ):
        xtra = {}
        if fillMode:
        ...
        path = transformNode(
            ...
            "path",
            d=self.path,
            style=self._formatStyle(styles),
        )
        if link_info:
    ...
    def drawString(
        ...
        **_svgAttrs
        ...
        if (self._fillColor != None and needFill) or (
            self._strokeColor != None and needStroke
        ):
            if not text_anchor in ["start", "inherited", "left"]:
                ...
                if text_anchor == "end":
                    ...
                    x -= numericXShift(
                        text_anchor, s, textLen, self._font, self._fontSize
                    )
                else:
    ...
    def circle(self, xc, yc, r, link_info=None):
        circle = transformNode(
            self.doc, "circle", cx=xc, cy=yc, r=r, style=self._formatStyle(AREA_STYLES)
        )
    ...
    def drawCurve(self, x1, y1, x2, y2, x3, y3, x4, y4, closed=0):
        ...
        if self._strokeColor != None:
            self.code.append(
                (codeline % data) + ((closed and " closepath") or "") + " stroke"
            )
    ...
    def drawArc(self, x1, y1, x2, y2, startAng=0, extent=360, fromcenter=0):
        ...
        path = transformNode(
            self.doc, "path", d=" ".join(s.__self__), style=self._formatStyle()
        )
        self.currGroup.appendChild(path)
    ...
    def polygon(self, points, closed=0, link_info=None):
        ...
        if self._strokeColor != None or self._fillColor != None:
            ...
            polyline = transformNode(
                self.doc, "polygon", points=pts, style=self._formatStyle(AREA_STYLES)
            )
    ...
    def polyLine(self, points):
        ...
        if self._strokeColor != None:
            ...
            polyline = transformNode(
                ...
                "polyline",
                points=pts,
                style=self._formatStyle(AREA_STYLES, fill=None),
            )
            self.currGroup.appendChild(polyline)
...
class _SVGRenderer(Renderer):
    ...
    def drawCircle(self, circle):
        self._canvas.circle(
            circle.cx, circle.cy, circle.r, link_info=self._get_link_info_dict(circle)
        )

    def drawWedge(self, wedge):
        yradius, radius1, yradius1 = wedge._xtraRadii()
        if (
            (radius1 == 0 or radius1 is None)
            and (yradius1 == 0 or yradius1 is None)
            and not wedge.annular
        ):
            ...
            self._canvas.drawArc(
                x1, y1, x2, y2, startangledegrees, extent, fromcenter=1
            )
        else:
            P = wedge.asPolygon()
    ...
    def drawEllipse(self, ellipse):
        ...
        self._canvas.ellipse(
            x1, y1, x2, y2, link_info=self._get_link_info_dict(ellipse)
        )

    def drawPolygon(self, p):
        self._canvas.polygon(
            _pointsFromList(p.points), closed=1, link_info=self._get_link_info_dict(p)
        )
    ...
    def drawPath(self, path, fillMode=FILL_EVEN_ODD):
        ...
        if autoclose == "svg":
            rP()
            c._fillAndStroke(
                [], clip=path.isClipPath, link_info=link_info, fillMode=fillMode
            )
        elif autoclose == "pdf":
            rP(forceClose=True)
            c._fillAndStroke(
