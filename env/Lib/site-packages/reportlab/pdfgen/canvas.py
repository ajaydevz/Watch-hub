    ...
    def set(self, canv, a, v):
        ...
        if isinstance(d, bool):
            ...
            if v not in self.allowed[a]:
                raise ValueError(
                    "ExtGstate[%r] = %r not in allowed values %r"
                    % (a, v, self.allowed[a])
                )
...
class Canvas(_PDFColorSetter):
    ...
    def __init__(
        ...
        self._initialFontName = (
            initialFontName if initialFontName else rl_config.canvas_basefontname
        )
        self._initialFontSize = initialFontSize if initialFontSize is not None else 12
        self._initialLeading = (
            initialLeading
            if initialLeading is not None
            else self._initialFontSize * 1.2
        )
        ...
        if cM:
            ...
            if bw:
                ...
                if bleedW:
                    # scale everything
                    self.scale(
                        1 + (2.0 * bleedW) / pageWidth, 1 + (2.0 * bleedW) / pageHeight
                    )
    ...
    def bookmarkPage(
        self, key, fit="Fit", left=None, top=None, bottom=None, right=None, zoom=None
    ):
        """
    ...
    def freeTextAnnotation(
        self, contents, DA, Rect=None, addtopage=1, name=None, relative=0, **kw
    ):
        """DA is the default appearance string???"""
        Rect = self._absRect(Rect, relative)
        self._addAnnotation(
            pdfdoc.FreeTextAnnotation(Rect, contents, DA, **kw), name, addtopage
        )

    def textAnnotation(
        self, contents, Rect=None, addtopage=1, name=None, relative=0, **kw
    ):
        """Experimental, but works."""
        Rect = self._absRect(Rect, relative)
        self._addAnnotation(
            pdfdoc.TextAnnotation(Rect, contents, **kw), name, addtopage
        )
    ...
    def highlightAnnotation(
        ...
        **kw
        ...
        self._addAnnotation(
            pdfdoc.HighlightAnnotation(Rect, contents, QuadPoints, Color, **kw),
            name,
            addtopage,
        )
    ...
    def inkAnnotation(
        ...
        **kw
        ...
        self._addAnnotation(
            pdfdoc.InkAnnotation(Rect, contents, InkList, **kw), name, addtopage
        )
    ...
    def linkRect(
        ...
        **kw
        ...
        destination = self._bookmarkReference(
            destinationname
        )  # permitted to be undefined... must bind later...
        Rect = self._absRect(Rect, relative)
        kw["Rect"] = Rect
    ...
    def linkURL(
        ...
        **kw
        ...
        ann["Rect"] = PDFArray(
            self._absRect(rect, relative)
        )  # the whole page for testing
    ...
    def bezier(self, x1, y1, x2, y2, x3, y3, x4, y4):
        ...
        self._code.append(
            "n %s m %s c S" % (fp_str(x1, y1), fp_str(x2, y2, x3, y3, x4, y4))
        )
    ...
    def rect(self, x, y, width, height, stroke=1, fill=0):
        ...
        self._code.append(
            "n %s re " % fp_str(x, y, width, height)
    ...
    def linearGradient(self, x0, y0, x1, y1, colors, positions=None, extend=True):
        ...
        shading = PDFAxialShading(
            ...
            Extend=_gradientExtendStr(extend),
        )
        self.shade(shading)
    ...
    def drawString(
        self, x, y, text, mode=None, charSpace=0, direction=None, wordSpace=None
    ):
    ...
    def drawRightString(
        ...
        if wordSpace:
            width += (text.count(" ") + text.count("\xa0") - 1) * wordSpace
    ...
    def drawAlignedString(
        ...
        y,
        text,
        pivotChar=rl_config.decimalSymbol,
        mode=None,
        charSpace=0,
        direction=None,
        wordSpace=None,
        ...
        if (
            len(parts) == 1
            and digitPat.search(text) is not None
    ...
    def _strokeAndFill(self, stroke, fill, fillMode=None):
        self._code.append(
            PATH_OPS[stroke, fill, fillMode if fillMode is not None else self._fillMode]
        )
    ...
    def clipPath(self, aPath, stroke=1, fill=0, fillMode=None):
        ...
        if fillMode is None:
            fillMode = getattr(aPath, "_fillMode", self._fillMode)
        gc = aPath.getCode()
        pathops = PATH_OPS[stroke, fill, fillMode]
    ...
    def setPageTransition(
        self, effectname=None, duration=1, direction=0, dimension="H", motion="I"
    ):
    ...
    def drawBoundary(self):
        if not hasattr(self, "_drawBoundary"):
            ...
            self._drawBoundary = lambda sb, x, y, w, h: Frame._drawBoundary(
                self, sb, x, y, w, h
            )
