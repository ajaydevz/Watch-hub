...
class PDFImage:
    ...
    def cache_imagedata(self):
        ...
        cachedname = os.path.splitext(image)[0] + (
            rl_config.useA85 and ".a85" or ".bin"
        )
    ...
    def drawInlineImage(
        ...
        if not canvas.bottomup:
            y = y + height
        canvas._code.append(
            "q %s 0 0 %s cm" % (fp_str(self.width), fp_str(self.height, x, y))
        )
        width = self.width
        height = self.height
...
if __name__ == "__main__":
    srcfile = os.path.join(
        os.path.dirname(reportlab.__file__), "test", "pythonpowered.gif"
    )
