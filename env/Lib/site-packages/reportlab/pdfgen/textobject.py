...
if rtlSupport:
    try:
        ...
        def fribidiText(text, direction):
            return (
                log2vis(text, directionsMap.get(direction, DIR_ON), clean=True)
...
class PDFTextObject(_PDFColorSetter):
    ...
    def setFont(self, psfontname, size, leading=None):
        ...
        if font._dynamicFont:
            ...
            pdffontname = self._canvas._doc.getInternalFontName(psfontname)
            self._code.append(
                "%s %s Tf %s TL" % (pdffontname, fp_str(size), fp_str(leading))
            )
        ...
        if font._dynamicFont:
            ...
            for subset, t in font.splitString(text, canv._doc):
                if subset != self._curSubset:
                    ...
                    R.append(
                        "%s %s Tf %s TL"
                        % (pdffontname, fp_str(self._fontsize), fp_str(self._leading))
                    )
