    ...
    def format(self, maxwidth, maxheight, program, leading=0):
        ...
        while remainder and room:  # heightremaining>=self.leading and remainder:
            ...
            if linewidth < TOOSMALLSPACE:
                raise ValueError(
                    "indents %s %s too wide for space %s"
                    % (self.indent, self.rightIndent, maxwidth)
                )
            try:
                (
                    lineIsFull,
                    ...
                    maxLength,
                    justStrings,
                ) = self.fitLine(remainder, maxwidth)
            except:
        ...
        while not done and cursor < maxcursor:
            ...
            if isinstance(opcode, str) or hasattr(opcode, "width"):
                ...
                if aopcode > TOOSMALLSPACE:
                    ...
                    if (
                        nextLength > maxLength and not first
                    ):  # always do at least one thing
                ...
                if indicator == "nextLine":
                    ...
                    if isinstance(colorname, str):
                        ...
                        color = (
                            self.fontColor
                        ) = colorname  # assume its something sensible :)
                    line.append(opcode)
                elif indicator == "face":
                    # change font face
                    (i, fontname) = opcode
                    ...
                    raise ValueError(
                        "at format time don't understand indicator " + repr(indicator)
                    )
            else:
                raise ValueError(
                    "op must be string, float, instance, or tuple " + repr(opcode)
                )
            if not done:
                cursor += 1
                # first = 0
    ...
    def cleanProgram(self, line):
        ...
        while change:
            ...
            for index in rline:
                ...
                if (
                    isinstance(this, str)
                    or isinstance(next, str)
                    or hasattr(this, "width")
                    or hasattr(next, "width")
                ):
                    doswap = 0
                # only swap two tuples if the second one is an end operation and the first is something else
                    ...
                    if isinstance(next, tuple):
                        ...
                        doswap = 0
                        if (
                            nextindicator == "endLineOperation"
                            and thisindicator != "endLineOperation"
            ...
            if isinstance(opcode, str) or hasattr(opcode, "width"):
                if not indented:
                    ...
                    for handler in self.lineOpHandlers:
                        ...
                        handler.start_at(
                            self.x, self.y, self, canvas, textobject
                        )  # start underlining the next
                indented = 1
                ...
                if indicator == "nextLine":
                    ...
                    for handler in self.lineOpHandlers:
                        handler.end_at(
                            x, y, self, canvas, textobject
                        )  # finish, eg, underlining this line
...
class FastPara(Flowable):
    ...
    def wrap(self, availableWidth, availableHeight):
        ...
        if maxlength < spacewidth:
            return (
                spacewidth + rightIndent + firstindent,
                availableHeight,
            )  # need something wider than this!
        ...
        if self.lines is None:
            ...
            while cursor < nwords and not done:
                ...
                if not currentlength or nextlength < maxlength:
                    ...
                    # emit the line
                    lines.append(
                        (" ".join(currentline), currentlength, len(currentline))
                    )
                    currentline = []
                    currentlength = 0
...
class Para(Flowable):
    ...
    def wrap(self, availableWidth, availableHeight):
        ...
        if not self.formattedProgram:
            (formattedProgram, remainder, laststate, heightused) = p.format(
                availableWidth, availableHeight, program, leading
            )
        ...
        if style.firstLineIndent:
            ...
            for x in program:
                ...
                if isinstance(x, str) or hasattr(x, "width"):
                    break
            program.insert(
                count, ("indent", -style.firstLineIndent)
            )  # defaults to end if no visibles
    ...
    def compile_dl(self, attdict, content, extra, program):
        ...
        while contentcopy:
            ...
            if isinstance(e, str):
                ...
                if tagname != "dd" and tagname != "dt":
                    raise ValueError(
                        "don't expect %s here inside list, expect 'dd' or 'dt'"
                        % repr(tagname)
                    )
                if tagname == "dt":
                    if bullet:
                        raise ValueError(
                            "dt will not be displayed unless followed by a dd: "
                            + repr(bullet)
                        )
                    if content1:
        ...
        if bullet:
            raise ValueError(
                "dt will not be displayed unless followed by a dd" + repr(bullet)
            )
        ...
        if bl is None:
            if tagname == "ul":
                if typ == "disc":
                    ...
                    raise ValueError(
                        "unordered list type %s not implemented" % repr(typ)
                    )
                ...
                if typ == "1":
                    ...
                    raise ValueError(
                        "ordered bullet type %s not implemented" % repr(typ)
                    )
            else:
...
def EmbedInRml2pdf():
    ...
    from rlextra.rml2pdf.rml2pdf import (
        MapNode,
        Controller,
    )  # may not need to use superclass?
    ...
    class paraMapper(MapNode):
        ...
        def translate(self, nodetuple, controller, context, overrides):
            ...
            if result is None:
                result = Para(
                    mystyle, content, context=context, bulletText=bulletText
                )  # possible ref loop on context, break later
            return result
...
test_program = (
    ...
    + splitspace(
        "HOTLINK This is the first segment of the first paragraph. This is the first segment of the first paragraph. This is the first segment of the first paragraph. This is the first segment of the first paragraph. "
    )
    + [
    ...
    + splitspace(
        "This is the SECOND!!! segment of the first paragraph. This is the first segment of the first paragraph. This is the first segment of the first paragraph. This is the first segment of the first paragraph. This is the first segment of the first paragraph. "
    )
    + [
        ("nextLine", 0),
        ("align", TA_JUSTIFY),
    ...
    + splitspace(
        "This is the first segment of the first paragraph. This is the first segment of the first paragraph. This is the first segment of the first paragraph. This is the first segment of the first paragraph. "
    )
    + [
...
def test():
    ...
    if 1:
        ...
        while remainder:
            ...
            maxwidth = 7 * inch
            maxheight = 500
            (formattedprogram, remainder, laststate, height) = p.format(
                maxwidth, maxheight, remainder
            )
            if debug:
