...
RoundingRectLine = namedtuple(
    "RoundingRectLine", "xs ys xe ye weight color cap dash join"
)
...
class Table(Flowable):
    def __init__(
        ...
        if nrows:
            self._ncols = ncols = max(list(map(_rowLen, data)))
        ...
        self._longTableOptimize = (
            getattr(self, "_longTableOptimize", rl_config.longTableOptimize)
            if longTableOptimize is None
            else longTableOptimize
        )
        if not (nrows and ncols):
            if emptyTableAction == "error":
                raise ValueError(
                    f"{self.identity()} must have at least a row and column"
                )
                ...
                Preformatted.__init__(
                    self,
                    "%s(%d,%d)" % (self.__class__.__name__, nrows, ncols),
                    _emptyTableStyle,
                )
                ...
                raise ValueError(
                    f"{self.identitiy()} bad emptyTableAction: {emptyTableAction!a}"
                )
            return
        ...
        if not _seqCW:
            ...
            if rl_config.allowShortTableRows and isinstance(colWidths, list):
                ...
                raise ValueError(
                    f"{self.identity()} data error - {ncols} columns in data but {len(colWidths)} column widths"
                )
        if not _seqRH:
            rowHeights = nrows * [rowHeights]
        elif len(rowHeights) != nrows:
            raise ValueError(
                f"{self.identity()} data error - {nrows} rows in data but {len(rowHeights)} row heights"
            )
        for i, d in enumerate(data):
            ...
            if n != ncols:
                if rl_config.allowShortTableRows and isinstance(d, list):
                    ...
                    raise ValueError(
                        f"{self.identity()} expected {ncols} not {n} columns in row {i}!"
                    )
        ...
        if minRowHeights != None:
            ...
            if not lmrh:
                raise ValueError(
                    "{self.idenity()} Supplied mismatching minimum row heights of length {lmrh}"
                )
            elif lmrh < nrows:
                minRowHeights = minRowHeights + (
                    nrows - lmrh
                ) * minRowHeights.__class__((0,))
    ...
    def identity(self, maxLen=30):
        ...
        return "<%s@0x%8.8X %s rows x %s cols%s>%s" % (
            self.__class__.__name__,
            id(self),
            nr,
            nc,
            ...
            for c in self._cellListIter(C, aW, aH):
                ...
                if frame:
                    R(
                        LIIndenter(
                            c,
                            leftIndent=frame._leftExtraIndent,
                            rightIndent=frame._rightExtraIndent,
                        )
                    )
                else:
                    R(c)
    ...
    def _calc_width(self, availWidth, W=None):
        ...
        if None in W:  # some column widths are not given
            ...
            while None in W:
                ...
                for i, Vi in enumerate(V):
                    ...
                    if (
                        ji in colSpanCells and not span
                        ...
                        if t is None:
                            raise ValueError(
                                f"Flowable {v.identity()} in cell({i},{j}) can't have auto width\n{self.identity(30)}"
                            )
        ...
        self._colpositions = [
            0
        ]  # index -1 is right side boundary; we skip when processing cells
        for w in W:
            width = width + w
    ...
    def _calc_height(self, availHeight, availWidth, H=None, W=None):
        ...
        if None in H:
            ...
            while None in H:
                ...
                h = 0
                j = 0
                for j, (v, s, w) in enumerate(
                    list(zip(V, S, W))
                ):  # value, style, width (lengths must match)
                    ji = j, i
                    span = spanRanges.get(ji, None)
                    if ji in rowSpanCells and not span:
                        ...
                        if isinstance(v, (tuple, list, Flowable)):
                            ...
                            if w is None and not self._canGetWidth(v):
                                raise ValueError(
                                    f"Flowable {v[0].identity()} in cell({i},{j}) can't have auto width\n{self.identity(30)}"
                                )
                            if canv:
                                canv._fontname, canv._fontsize, canv._leading = (
                                    s.fontname,
                                    s.fontsize,
                                    s.leading or 1.2 * s.fontsize,
                                )
                            if ji in colSpanCells:
                                if not span:
                                    continue
                                w = max(
                                    colpositions[span[2] + 1] - colpositions[span[0]],
                                    w or 0,
                                )
                            dW, t = self._listCellGeom(
                                v, w or self._listValueWidth(v), s
                            )
                            if canv:
    ...
    def _calc(self, availWidth, availHeight):
        ...
        if (
            None in self._colWidths or "*" in self._colWidths
        ) and self._hasVariWidthElements():
                ...
                try:
                    ...
                    if col1 - col0 > 0:
                        for _ in range(col0 + 1, col1 + 1):
                            vBlocks.setdefault(colpositions[_], []).append(
                                (rowpositions[row1 + 1], rowpositions[row0])
                            )
                    if row1 - row0 > 0:
                        for _ in range(row0 + 1, row1 + 1):
                            hBlocks.setdefault(rowpositions[_], []).append(
                                (colpositions[col0], colpositions[col1 + 1])
                            )
                    x = colpositions[col0]
        ...
        for cmd in tblstyle.getCommands():
            if len(cmd) >= 3:
                ...
                if (
                    ...
                    raise ValueError(
                        f"""bad style command {cmd!r} illegal of invalid string coordinate
only rows may be strings with values in {_SPECIALROWS!r}"""
    ...
    def _drawLines(self):
        ...
        if rrd:  # we are collection some lines
            ...
            def rcCanvLine(xs, ys, xe, ye):
                if (
                    xs == xe and (xs >= rrd.x1 or xs <= rrd.x0)
                ) or (  # vertical line that needs to be saved
                    ys == ye and (ys >= rrd.y1 or ys <= rrd.y0)
                ):  # horizontal line that needs to be saved
                    aSL(
                        RoundingRectLine(xs, ys, xe, ye, weight, color, cap, dash, join)
                    )
                else:
        ...
        try:
            for (
                ...
                if join is not None and cjoin != join:
                    ...
                    cjoin = join
                sc, ec, sr, er = self.normCellRange(sc, ec, sr, er)
                getattr(self, _LineOpMap.get(op, "_drawUnknown"))(
                    (sc, sr), (ec, er), weight, color, count, space
                )
        finally:
            if rrd:
                canv.line = ocanvline
    ...
    def _drawHLines(self, start, end, weight, color, count, space):
        ...
        if count == 1:
            ...
            lf = lambda x0, y0, x1, y1, canvLine=canvLine, ws=weight + space, count=count: _multiLine(
                x0, x1, y0, canvLine, ws, count
            )
            for y in rp:
    ...
    def _cr_1_1(self, n, nRows, repeatRows, cmds, doInRowSplit, _srflMode=False):
        ...
        for c in cmds:
            ...
            if cS:
                ...
                self._addCommand(
                    (c[0],)
                    + ((sc, repeatRows.index(min(cS))), (ec, repeatRows.index(max(cS))))
                    + tuple(c[3:])
                )
            if er < n:
                continue
    ...
    def _splitRows(self, availHeight, doInRowSplit=0):
        ...
        n = self._getFirstPossibleSplitRowPosition(
        ...
        if lo:
            ...
            if hi < 0:
                hi += lim
            if n > hi:
                return self._splitRows(
                    availHeight - sum(self._rowHeights[hi:n]), doInRowSplit=doInRowSplit
                )
            elif n < lo:
                return []
        ...
        if not doInRowSplit:
            ...
            for column, (value, style, width) in enumerate(
                zip(cellvalues, cellStyles, cellWidths)
            ):
                ...
                if isinstance(value, (tuple, list)):
                    ...
                    lineHeight = 1.2 * style.fontsize
                    height = (
                        lineHeight * len(rows) + style.topPadding + style.bottomPadding
                    )
            ...
            if (minSplit + maxSplit > curRowHeight) or (
                ...
                for column in range(self._ncols):
                    cell = (column, n)
                    if (
                        cell in self._rowSpanCells
                        and self._spanRanges.get((column, n), None) is None
                    ):
                ...
                for cell in splitCells:
                    ...
                    res = self._splitCell(
                        self._cellvalues[span_sr][span_sc],
                        oldStyle,
                        oldHeight,
                        newHeight,
                        width,
                    ...
                    for cmd, start, end in spanCmds:
                        if (span_sc, span_sr) == start and (span_ec, span_er) == end:
                            ...
                            if n - 1 > span_sr or span_sc != span_ec:
                                newSpanCmds.append(
                                    (cmd, (span_sc, span_sr), (span_ec, n - 1))
                                )
                            if n < span_er or span_sc != span_ec:
                                newSpanCmds.append(
                                    (cmd, (span_sc, n), (span_ec, span_er))
                                )
                        else:
                    ...
                    for cmd, start, end, color in bkgrndcmds:
                        if start == (span_sc, span_sr):
                            ...
                            newbkgrndcmds.append(
                                (cmd, (start[0], n), (end[0], n), color)
                            )
                        else:
                ...
                for linecmd in self._linecmds:
                    ...
                    if (
                        (op == "BOX")
                        or (op == "GRID" and (sr <= n or er >= n))
                        or (op == "INNERGRID" and (sr < n or er > n))
                    ):
                ...
                for cell in splitCells:
                    ...
                    for linecmd in newlinecmds:
                        ...
                        if (
                            (op == "LINEABOVE" and er > span_sr and sr <= span_er)
                            or (op == "LINEBELOW" and er >= span_sr and sr < span_er)
                        ) and (sc <= span_ec and ec >= span_sc):
            ...
            for value, style, width in zip(cellvalues, cellStyles, cellWidths):
                ...
                R1.append(v[1])
                h0 = (
                    self._listCellGeom(v[0], width, style)[1]
                    + style.topPadding
                    + style.bottomPadding
                )
                R0Height = max(R0Height, h0)
                h1 = (
                    self._listCellGeom(v[1], width, style)[1]
                    + style.topPadding
                    + style.bottomPadding
                )
        ...
        if repeatRows:
            ...
            R1._cr_1_1(
                n, nrows, repeatRows, T._bkgrndcmds, doInRowSplit, _srflMode=True
            )
            ...
            if r != None:
                ...
                if y1 != y2:
                    ...
                    while 1:
                        ...
                        impossible[
                            y
                        ] = None  # split at position y is impossible because of overlapping rowspan
                        y += 1
    ...
    def _getFirstPossibleSplitRowPosition(self, availHeight, ignoreSpans=0):
        ...
        if self._nosplitCmds:
            self._getRowImpossible(
                impossible, self._rowNoSplitCells, self._nosplitRanges
            )
        h = 0
        n = 1
    ...
    def _makeRoundedCornersClip(self, FUZZ=rl_config._FUZZ):
        ...
        ncols = self._ncols
        ar = [
            min(self._rowHeights[i], self._colWidths[j], cornerRadii[k])
    ...
    def _restoreRoundingObscuredLines(self):
        ...
        for xs, ys, xe, ye, weight, color, cap, dash, join in SL:
            ...
            if ys == ye:
                ...
                c0 = (
                    _quadrantDef(
                        "left", ypos, (xs, ys), r0, kind=2, direction="left-right"
                    )
                    if xs < x0 + r0
                    else None
                )
                c1 = (
                    _quadrantDef(
                        "right", ypos, (xe, ye), r1, kind=1, direction="left-right"
                    )
                    if xe > x1 - r1
                    else None
                )
                ...
                c0 = (
                    _quadrantDef(
                        xpos, "bottom", (xs, ys), r0, kind=2, direction="bottom-top"
                    )
                    if ys < y0 + r0
                    else None
                )
                c1 = (
                    _quadrantDef(
                        xpos, "top", (xe, ye), r1, kind=1, direction="bottom-top"
                    )
                    if ye > y1 - r1
                    else None
                )
    ...
    def draw(self):
        ...
        if not self._spanCmds:
            ...
            if renderCB:
                for rowNo, (row, rowstyle, rowpos, rowheight) in enumerate(
                    zip(
                        self._cellvalues,
                        self._cellStyles,
                        self._rowpositions[1:],
                        self._rowHeights,
                    )
                ):
                    renderCB(self, "startRow", rowNo)
                    for colNo, (cellval, cellstyle, colpos, colwidth) in enumerate(
                        zip(row, rowstyle, self._colpositions[:-1], self._colWidths)
                    ):
                        renderCB(
                            self,
                            "startCell",
                        ...
                        self._drawCell(
                            cellval, cellstyle, (colpos, rowpos), (colwidth, rowheight)
                        )
                        renderCB(self, "endCell")
                    renderCB(self, "endRow")
            else:
                for row, rowstyle, rowpos, rowheight in zip(
                    self._cellvalues,
                    self._cellStyles,
                    self._rowpositions[1:],
                    self._rowHeights,
                ...
                if (
                    ...
                    if arg0 == "HORIZONTAL":
                        ...
                        if 4 <= len(arg) <= 5:
                            ...
                            canv.linearGradient(
                                ax0, ay0, ax1, ay1, C, positions=P, extend=extend
                            )
                        else:
                            raise ValueError(
                                f"Wrong length for {op!a} arguments {arg!a}"
                            )
                        ...
                        if 4 <= len(arg) <= 5:
                            ...
                            if ref == "width":
                                ...
                                raise ValueError(
                                    f"Bad radius, {arg[1]!a}, for {op!a} arguments {arg!r}"
                                )
                            r *= ref
                            extend = arg[2]
                            C = arg[3]
                            P = arg[4] if len(arg) == 4 else None
                            canv.radialGradient(
                                xc, yc, r, C, positions=P, extend=extend
                            )
                        else:
    ...
    def _drawCell(self, cellval, cellstyle, pos, size):
        ...
        if self._curcellstyle is not cellstyle:
            ...
            if (
                cur is None
                or cellstyle.leading != cur.leading
                or cellstyle.fontname != cur.fontname
                or cellstyle.fontsize != cur.fontsize
            ):
                self.canv.setFont(
                    cellstyle.fontname, cellstyle.fontsize, cellstyle.leading
                )
        ...
        if isinstance(cellval, (tuple, list, Flowable)):
            ...
            W = []
            H = []
            w, h = self._listCellGeom(
                cellval, colwidth, cellstyle, W=W, H=H, aH=rowheight
            )
            if valign == "TOP":
                ...
                y = (
                    rowpos
                    + (rowheight + cellstyle.bottomPadding - cellstyle.topPadding + h)
                    / 2.0
                )
            if cellval:
            ...
            for v, w, h in zip(cellval, W, H):
                if just == "LEFT":
                    ...
                    x = (
                        ...
                        + (
                            colwidth
                            + cellstyle.leftPadding
                            - cellstyle.rightPadding
                            - w
                        )
                        / 2.0
                    )
            ...
            if just == "LEFT":
                ...
                x = (
                    colpos
                    + (colwidth + cellstyle.leftPadding - cellstyle.rightPadding) * 0.5
                )
            elif just == "RIGHT":
            ...
            if valign == "BOTTOM":
                ...
                y = (
                    rowpos
                    + (
                        cellstyle.bottomPadding
                        + rowheight
                        - cellstyle.topPadding
                        + n * leading
                    )
                    / 2.0
                    - fontsize
        ...
        if cellstyle.href:
            ...
            self.canv.linkURL(
                cellstyle.href,
                (colpos, rowpos, colpos + colwidth, rowpos + rowheight),
                relative=1,
            )
        if cellstyle.destination:
            ...
            self.canv.linkRect(
                ...
                Rect=(colpos, rowpos, colpos + colwidth, rowpos + rowheight),
                relative=1,
            )
    ...
    def _setCornerRadii(self, cornerRadii):
        if isListOfNumbersOrNone(cornerRadii):
            self._cornerRadii = (
                None
                if not cornerRadii
                else list(cornerRadii) + (max(4 - len(cornerRadii), 0) * [0])
            )
        else:
            raise ValueError(
                f"cornerRadii should be None or a list/tuple of numeric radii\nnot {cornerRadii!a}"
            )
