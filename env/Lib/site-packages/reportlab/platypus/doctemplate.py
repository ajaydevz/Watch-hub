...
class LCActionFlowable(ActionFlowable):
    ...
    def wrap(self, availWidth, availHeight):
        ...
        raise NotImplementedError(
            "%s.wrap should never be called" % self.__class__.__name__
        )

    def draw(self):
        """Should never be called."""
        raise NotImplementedError(
            "%s.draw should never be called" % self.__class__.__name__
        )
...
class FrameActionFlowable(Flowable):
    ...
    def __init__(self, *arg, **kw):
        raise NotImplementedError(
            "%s.__init__ should never be called for abstract Class"
            % self.__class__.__name__
        )

    def frameAction(self, frame):
        raise NotImplementedError(
            "%s.frameAction should never be called for abstract Class"
            % self.__class__.__name__
        )
...
class PageTemplate:
    ...
    def __init__(
        ...
        if not isSeq(frames):
            frames = [frames]
        assert [
            x for x in frames if not isinstance(x, Frame)
...
def _ktAllow(f):
    ...
    return not (
        isinstance(f, (_ContainerSpace, DocIf, DocWhile))
        or getattr(f, "locChanger", False)
    )
...
class BaseDocTemplate:
    ...
    def _samePT(self, npt):
        ...
        if isinstance(npt, strTypes):
            return npt == (
                self.pageTemplates[self._nextPageTemplateIndex].id
                if hasattr(self, "_nextPageTemplateIndex")
                else self.pageTemplate.id
            )
    ...
    def handle_pageEnd(self):
        ...
        if self._emptyPages >= self._emptyPagesAllowed:
            if 1:
                ident = (
                    "More than %d pages generated without content - halting layout.  Likely that a flowable is too large for any frame."
                    % self._emptyPagesAllowed
                )
            ...
            self.canv.setPageRotation(
                getattr(self.pageTemplate, "rotation", self.rotation)
            )
            self.canv.showPage()
            self._setPageTemplate()
    ...
    def handle_frameEnd(self, resume=0):
        ...
        if hasattr(self, "_nextFrameIndex"):
            ...
            if hasattr(f, "lastFrame") or f is self.pageTemplate.frames[-1]:
                ...
                self.frame = self.pageTemplate.frames[
                    self.pageTemplate.frames.index(f) + 1
                ]
                self.frame._debug = self._debug
    ...
    def _peekNextFrame(self):
        ...
        if hasattr(f, "lastFrame") or f is self.pageTemplate.frames[-1]:
            if hasattr(self, "_nextPageTemplateCycle"):
                ...
                pageTemplate = self._peekNextPageTemplate(
                    self.pageTemplate.autoNextPageTemplate
                )
            else:
        ...
        if isinstance(fx, strTypes):
            for f in self.pageTemplate.frames:
                if f.id == fx:
                    ...
                    return
            raise ValueError(
                "can't find frame('%s') in %r(%s) which has frames %r"
                % (
                    fx,
                    self.pageTemplate,
                    self.pageTemplate.id,
                    [(f, f.id) for f in self.pageTemplate.frames],
                )
            )
    ...
    def handle_breakBefore(self, flowables):
        ...
        if (
            hasattr(first, "style")
            and hasattr(first.style, "pageBreakBefore")
            and first.style.pageBreakBefore == 1
        ):
            first._skipMeNextTime = 1
        ...
        if (
            hasattr(first, "style")
            and hasattr(first.style, "frameBreakBefore")
            and first.style.frameBreakBefore == 1
        ):
            first._skipMeNextTime = 1
            flowables.insert(0, FrameBreak())
            return
    ...
    def handle_flowable(self, flowables):
        ...
        if isinstance(f, PageBreak):
            ...
            if frame.add(f, canv, trySplit=self.allowSplitting):
                ...
                if n:
                    if not isinstance(
                        S[0], (PageBreak, SlowPageBreak, ActionFlowable, DDIndenter)
                    ):
                        if not frame.add(S[0], canv, trySplit=0):
                            ident = (
                                "Splitting error(n==%d) on page %d in\n%s\nS[0]=%s"
                                % (
                                    n,
                                    self.page,
                        ...
                        flowables[0:0] = S[
                            1:
                        ]  # put rest of splitted flowables back on the list
                        _addGeneratedContent(flowables, frame)
                    else:
    ...
    def _makeCanvas(self, filename=None, canvasmaker=canvas.Canvas):
        ...
        *NB* only the canvases created in self._startBuild will actually be used
        ...
        from reportlab.pdfbase.pdfdoc import (
            ViewerPreferencesPDFDictionary as VPD,
            checkPDFBoolean as cPDFB,
        )
    ...
    def build(self, flowables, filename=None, canvasmaker=canvas.Canvas):
        ...
        try:
            ...
            while len(flowables):
                if (
                    self._hanging
                    and self._hanging[-1] is PageBegin
                    and isinstance(flowables[0], PageBreakIfNotEmpty)
                ):
    ...
    def docExec(self, stmt, lifetime):
        ...
        try:
            if lifetime not in self._allowedLifetimes:
                raise ValueError(
                    "bad lifetime %r not in %r" % (lifetime, self._allowedLifetimes)
                )
            exec(stmt, NS)
        except:
...
if __name__ == "__main__":
    ...
    def run():
        ...
        SimpleDocTemplate("doctemplate.pdf").build(
            objects_to_draw, onFirstPage=myFirstPage, onLaterPages=myLaterPages
        )
