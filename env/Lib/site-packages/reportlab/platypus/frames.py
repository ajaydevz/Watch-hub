...
class Frame:
        ...
        try:
            ...
            if y < p - _FUZZ:
                if not rl_config.allowTableBoundsErrors and (
                    (h > self._aH or w > aW) and not trySplit
                ):
                    from reportlab.platypus.doctemplate import LayoutError

                    raise LayoutError(
                        "Flowable %s (%sx%s points) too large for frame (%sx%s points)."
                ...
                if fbg and fbg[-1].active:
                    ...
                    if not bgm:
                        ...
                        if (
                            bgm == "frame"
                            or bgm == "frame-permanent"
                            or (att and bgm == "frame-permanent-1")
                        ):
    ...
    def drawBoundary(self, canv, __boundary__=None):
        self._drawBoundary(
            canv,
            __boundary__ or self.showBoundary,
            self._x1,
            self._y1,
            self._x2 - self._x1,
            self._y2 - self._y1,
