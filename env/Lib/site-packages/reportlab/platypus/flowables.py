...
def splitLine(
    line_to_split, lines_splitted, maximum_length, split_characters, new_line_characters
):
...
class Image(Flowable):
    ...
    def __init__(
        ...
        self._dpi = useDPI
        if not fp and os.path.splitext(filename)[1] in [
            ".jpg",
            ".JPG",
            ".jpeg",
            ".JPEG",
    ...
    def _dpiAdjust(self):
        ...
        if dpi:
            if dpi[0] != 72:
                self.imageWidth *= 72.0 / dpi[0]
            if dpi[1] != 72:
                self.imageHeight *= 72.0 / dpi[1]
    ...
    def __getattr__(self, a):
        if a == "_img":
            ...
            if not isinstance(self._file, strTypes):
                self._file = None
                if self._lazy >= 2:
                    self._lazy = 1  # here we're assuming we cannot read again
            return self._img
...
class PageBreak(UseUpSpace):
    ...
    def __repr__(self):
        return "%s(%s)" % (
            self.__class__.__name__,
            repr(self.nextTemplate) if self.nextTemplate else "",
        )
...
def _flowableSublist(V):
    ...
    assert not [
        x for x in V if isinstance(x, LCActionFlowable)
    ], "LCActionFlowables not allowed in sublists"
    return V
...
class ParagraphAndImage(Flowable):
    ...
    def wrap(self, availWidth, availHeight):
        ...
        P.blPara = P.breakLines(
            [first_line_width] + nIW * [intermediate_widths] + [later_widths]
        )
        if self._side == "left":
    ...
    def draw(self):
        ...
        if self._side == "left":
            ...
            self.I.drawOn(
                canv,
                self.width - self.wI - self.xpad,
                self.height - self.hI - self.ypad,
            )
            self.P.drawOn(canv, 0, 0)
...
class PTOContainer(_Container, Flowable):
    ...
    def split(self, availWidth, availHeight):
        ...
        if len(SS) > 1:
            ...
            R2 = Hdr + R2I + C[i:]
        T = R1 + [
            PTOContainer(R2, [copy(x) for x in I.trailer], [copy(x) for x in I.header])
        ]
        return T
...
class KeepInFrame(_Container, Flowable):
    def __init__(
        ...
        assert maxHeight >= 0, "%s invalid maxHeight value %s" % (
            self.identity(),
            maxHeight,
        )
    ...
    def identity(self, maxLen=None):
        return "<%s at %s%s%s> size=%sx%s" % (
            ...
            and (" maxWidth=%s" % fp_str(getattr(self, "maxWidth", 0)))
            or "",
            getattr(self, "maxHeight", "")
            and (" maxHeight=%s" % fp_str(getattr(self, "maxHeight")))
            or "",
        )
    ...
    def wrap(self, availWidth, availHeight):
        ...
        if mode == "error" and (W > maxWidth + _FUZZ or H > maxHeight + _FUZZ):
            ...
            def func(x):
                x = float(x)
                W, H = _listWrapOn(
                    self._content, x * maxWidth, self.canv, fakeWidth=fakeWidth
                )
                W /= x
                H /= x
                return W, H
...
class ImageAndFlowables(_Container, _FindSplitterMixin, Flowable):
    ...
    def wrap(self, availWidth, availHeight):
        ...
        if hasattr(self, "_wrapArgs"):
            if (
                ...
                and getattr(I, "_oldDrawSize", None) is None
            ):
                return self.width, self.height
            self._reset()
            I._unRestrictSize()
    ...
    def split(self, availWidth, availHeight):
        if hasattr(self, "_wrapArgs"):
            ...
            if (
                self._wrapArgs != (availWidth, availHeight)
                or getattr(I, "_oldDrawSize", None) is not None
            ):
                self._reset()
                I._unRestrictSize()
        ...
        return [
            ImageAndFlowables(
                ...
                imageSide=self._side,
                imageHref=self.imageHref,
            )
        ] + C1
    ...
    def drawOn(self, canv, x, y, _sW=0):
        ...
        if self.imageHref:
            canv.linkURL(
                ...
                (
                    Ix,
                    y + self.height - self._itpad - self._hI,
                    Ix + self._wI,
                    y + self.height,
                ),
                relative=1,
...
class _AbsRect(NullDraw):
    ...
    def drawOn(self, canv, x, y, _sW=0):
        if self._width > _FUZZ and self._height > _FUZZ:
            st = (
                self._strokeColor
                and self._strokeWidth is not None
                and self._strokeWidth >= 0
            )
            if st or self._fillColor:
                ...
                canv.rect(
                    ...
                    self._width,
                    self._height,
                    stroke=1 if st else 0,
                    fill=1 if self._fillColor else 0,
                )
                canv.restoreState()
...
class _AbsLine(NullDraw):
    ...
    def __init__(
        self, x, y, x1, y1, strokeWidth=0, strokeColor=None, strokeDashArray=None
    ):
        self._x = x
        self._y = y
        self._x1 = x1
...
class BalancedColumns(_FindSplitterMixin, NullDraw):
    ...
    def __init__(
        ...
        if nCols < 2:
            raise ValueError(
                "nCols should be at least 2 not %r in %s" % (nCols, self.identitity())
            )
        self._content = _flowableSublist(F)
        self._nCols = nCols
        self.spaceAfter = spaceAfter
    ...
    def _generated_content(self, aW, aH):
        ...
        bpad = (
            frame._bottomPadding if self._bottomPadding is None else self._bottomPadding
        )
        ...
        def splitFunc(ah, endSlack=0):
            if ah not in _fres:
                ...
                for i in range(nCols):
                    wi, hi, c0, c1 = self._findSplit(
                        canv, cw, ah, content=cn, paraFix=False
                    )
                    w = max(w, wi)
                    h = max(h, hi)
                    c.append(c0)
                    if i == icheck:
                        wc, hc, cc0, cc1 = self._findSplit(
                            canv, cw, 2 * ah, content=c1, paraFix=False
                        )
                        if hc <= (1 + endSlack) * ah:
                            c.append(c1)
                            h = ah - 1e-6
        ...
        showBoundary = (
            self.showBoundary if self.showBoundary is not None else frame.showBoundary
        )
        ...
        F = [
            Frame(
                ...
                dx,
                fh,
                leftPadding=lpad if not i else hgap,
                bottomPadding=bpad,
                rightPadding=rpad if i == nCols - 1 else hgap,
                topPadding=tpad,
                id="%s-%d" % (self.name, i),
        ...
        if T is None:
            raise LayoutError(
                "%s used in non-doctemplate environment" % self.identity()
            )
        ...
        if indenter0:
            G.append(indenter0)
        doBox = (
            self._boxStrokeColor and self._boxStrokeWidth and self._boxStrokeWidth >= 0
        ) or self._boxFillColor
        doVLines = (
            self._vLinesStrokeColor
            and self._vLinesStrokeWidth
            and self._vLinesStrokeWidth >= 0
        )
                ...
                for i in range(1, nCols):
                    ...
                    vLines.append(
                        _AbsLine(
                            ...
                            vlx,
                            oby1,
                            strokeWidth=self._vLinesStrokeWidth,
                            strokeColor=self._vLinesStrokeColor,
                        )
                    )
        else:
            oby1 = ftop
...
class _FBGBag(ABag):
    def matches(self, frame, canv):
        fid = id(frame)
        return (
            (isinstance(self.fid, list) and fid in self.fid or fid == self.fid)
            and id(canv) == self.cid
            and self.pn == canv.getPageNumber()
        )
    ...
    def render(self, canv, frame, fbx, fby, fbw, fbh):
        if abs(fbw) > _FUZZ and abs(fbh) > _FUZZ:
            ...
            if self.fid == id(frame) and self.cid == id(canv) and self.pn == pn:
                ...
                canv.rect(
                    fbx, fby, fbw, fbh, stroke=1 if sc else 0, fill=1 if fbgc else 0
                )
                canv.restoreState()
...
class FrameBG(AnchorFlowable):
    ...
    def __repr__(self):
        return "%s(%s)" % (
            ...
            ", ".join(
                [
                    "%s=%r" % (i, getattr(self, i, None))
                    for i in "start color left right".split()
                ]
            ),
        )
        ...
        if self.start:
            ...
            frame._frameBGs.append(
                _FBGBag(
                    ...
                    start=self.start
                    if self.start in ("frame", "frame-permanent")
                    else None,
...
class FrameSplitter(NullDraw):
    ...
    def __init__(
        self, nextTemplate, nextFrames=[], gap=10, required=72, adjustHeight=True
    ):
        self.nextTemplate = nextTemplate
    ...
    def wrap(self, aW, aH):
        ...
        if aH < self.gap + self.required - _FUZZ:
            ...
            if templates is None:
                raise LayoutError(
                    "%s called in non-doctemplate environment" % self.identity()
                )
            T = [t for t in templates if t.id == self.nextTemplate]
            if not T:
                raise LayoutError(
                    "%s.nextTemplate=%s not found"
...
class DDIndenter(Flowable):
    ...
    def drawOn(self, canv, x, y, _sW=0):
        self._flowable.drawOn(
            canv,
            x + self._leftIndent,
            y,
            max(0, _sW - self._leftIndent - self._rightIndent),
        )
            ...
            try:
                ...
                if a not in ("spaceBefore", "spaceAfter"):
                    raise AttributeError(
                        f"{self!r} has no attribute {a} dict={self.__dict__}"
                    )
    ...
    def identity(self, maxLen=None):
        return "%s containing %s" % (
            self.__class__.__name__,
            self._flowable.identity(maxLen),
        )
        ...
        **kwds,
        ...
        if style:
            if not isinstance(style, ListStyle):
                raise ValueError(
                    "%s style argument (%r) not a ListStyle"
                    % (self.__class__.__name__, style)
                )
            self._style = style
            ...
            if auto:
                for v in start:
                    if v not in self._numberStyles:
                        raise ValueError(
                            "invalid start=%r or bullettype=%r"
                            % (start, self._bulletType)
                        )
            else:
        ...
        for d, f in self._flowablesIter():
            ...
            if d:
                ...
                if b:
                    if b.bulletType != bt:
                        raise ValueError(
                            "Included LIIndenter bulletType=%s != OrderedList bulletType=%s"
                            % (b.bulletType, bt)
                        )
                    value = int(b.value)
                else:
...
class DocIf(DocPara):
    ...
    def wrap(self, aW, aH):
        self.add_content(
            *self.checkBlock(self.blocks[int(bool(self.funcWrap(aW, aH)))])
        )
        return 0, 0
