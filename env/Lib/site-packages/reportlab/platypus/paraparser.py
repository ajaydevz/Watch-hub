...
class _ExValidate:
    ...
    def invalid(self, s):
        raise ValueError(
            "<%s> invalid value %r for attribute %s" % (self.tag, s, self.attr)
        )
...
class ParaParser(HTMLParser):
    ...
    def start_super(self, attributes):
        ...
        frag.fontSize = fontSizeNormalize(
            frag, "sups", frag.fontSize - min(sizeDelta, 0.2 * frag.fontSize)
        )
    ...
    def start_unichar(self, attr):
        if "name" in attr:
            ...
            try:
                ...
                self._syntax_error(
                    "<unichar/> invalid code attribute %s" % ascii(attr["code"])
                )
                v = "\0"
        else:
            v = None
            if attr:
                self._syntax_error(
                    "<unichar/> invalid attribute %s" % list(attr.keys())[0]
                )
    ...
    def _initial_frag(self, attr, attrMap, bullet=0):
        ...
        try:
            if bullet:
                ...
                frag.textColor = (
                    hasattr(style, "bulletColor")
                    and style.bulletColor
                    or style.textColor
                )
            else:
        ...
        self._defaultLinkUnderline = getattr(
            style, "linkUnderline", platypus_link_underline
        )
        return frag
    ...
    def _pop(self, tag):
        ...
        if tag == frag.__tag__:
            return frag
        raise ValueError(
            "Parse error: saw </%s> instead of expected </%s>" % (tag, frag.__tag__)
        )
    ...
    def getAttributes(self, attr, attrMap):
        ...
        for k, v in attr.items():
            ...
            if k in attrMap:
                ...
                self._syntax_error(
                    "invalid attribute name %s attrMap=%r"
                    % (k, list(sorted(attrMap.keys())))
                )
        return A
    ...
    def __init__(
        self, verbose=0, caseSensitive=0, ignoreUnknownTags=1, crashOnError=True
    ):
    ...
    def handle_data(self, data):
        ...
        if hasattr(frag, "cbDefn"):
            ...
            if data != "":
                self._syntax_error(
                    "No content allowed in %s tag" % frag._selfClosingTag
                )
            return
        else:
        ...
        if rv is None:
            ...
            for l in rv:
                ...
                if hasattr(l, "cbDefn"):
                    print(
                        "cbDefn",
                        getattr(l.cbDefn, "name", ""),
                        getattr(l.cbDefn, "label", ""),
                        l.cbDefn.kind,
                    )
                else:
                    print()
