...
captionStyle = ParagraphStyle(
    "Caption", fontName=_baseFontNameI, fontSize=10, alignment=TA_CENTER
)
    ...
    def _getCaptionPara(self):
        ...
        if self._captionData != (
            ...
            if isinstance(caption, Paragraph):
                ...
                raise ValueError(
                    "Figure caption of type %r is not a string or Paragraph"
                    % type(caption)
                )
...
class FlexFigure(Figure):
    ...
    def _scale(self, availWidth, availHeight):
        ...
        if (
            self._scaleFactor is None
            or self.width > availWidth
            or self.height > availHeight
        ):
            ...
            if self.scaleFactor is None:
                ...
                self._scaleFactor = min(
                    availWidth / self.width,
                    (availHeight - captionHeight) / self.figureHeight,
                )
...
class DrawingFigure(FlexFigure):
    ...
    def __init__(self, modulename, classname, caption, baseDir=None, background=None):
        ...
        self.drawing = klass()
        FlexFigure.__init__(
            self, self.drawing.width, self.drawing.height, caption, background
        )
        self.growToFit = 1
...
if _hasPageCatcher:
    ...
    class cachePageCatcherFigureNonA4(FlexFigure, PageCatcherCachingMixIn):
        ...
        def __init__(self, filename, pageNo, caption, width, height, background=None):
            ...
            self.pageNo = pageNo
            self.formName = (
                self.getFormName(self.filename, self.pageNo) + "_" + str(pageNo)
            )
    ...
    class cachePageCatcherFigure(cachePageCatcherFigureNonA4):
        ...
        def __init__(
            self, filename, pageNo, caption, width=595, height=842, background=None
        ):
            cachePageCatcherFigureNonA4.__init__(
                self, filename, pageNo, caption, width, height, background=background
            )
        ...
        def drawFigure(self):
            if not self.canv.hasForm(self.formName):
                if self.filename in self._cache:
                    ...
                    pdf = f.read()
                    f.close()
                    f, data = storeFormsInMemory(
                        pdf, pagenumbers=[self.pageNo], prefix=self.prefix
                    )
    ...
    class PageCatcherFigure(PageCatcherFigureNonA4):
        ...
        def __init__(
            ...
            PageCatcherFigureNonA4.__init__(
                self,
                filename,
                pageNo,
                caption,
...
def demo1(canvas):
    ...
    bodyStyle = ParagraphStyle(
        "Body", fontName=_baseFontName, fontSize=24, leading=28, spaceBefore=6
    )
