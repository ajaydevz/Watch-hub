...
def split(text, delim=None):
    ...
    if delim is not None and isBytes(delim):
...
def _justifyDrawParaLine(tx, offset, extraspace, words, last=0):
    ...
    simple = (
        ...
        or (-1e-8 < extraspace <= 1e-8)
        or (last and extraspace > -1e-8)
    )
    if not simple:
        nSpaces = len(words) + _nbspCount(text) - 1
        simple = nSpaces <= 0
...
def _justifyDrawParaLineRTL(tx, offset, extraspace, words, last=0):
    return (_rightDrawParaLine if last else _justifyDrawParaLine)(
        tx, offset, extraspace, words, last
    )
...
def _putFragLine(cur_x, tx, line, last, pKind):
    ...
    for i, f in enumerate(words):
        if hasattr(f, "cbDefn"):
            ...
            if i > 0:
                end_x = cur_x_s - (
                    0 if preformatted else _trailingSpaceLength(words[i - 1].text, tx)
                )
...
def _leftDrawParaLineX(tx, offset, line, last=0):
    ...
    if not simple:
        nSpaces = (
            line.wordCount
            + sum([_nbspCount(w.text) for w in line.words if not hasattr(w, "cbDefn")])
            - 1
        )
        simple = nSpaces <= 0
    if simple:
    ...
    try:
        ...
        if not simple:
            nSpaces = (
                line.wordCount
                + sum(
                    [_nbspCount(w.text) for w in line.words if not hasattr(w, "cbDefn")]
                )
                - 1
            )
            simple = nSpaces <= 0
...
def _justifyDrawParaLineX(tx, offset, line, last=0):
    ...
    extraSpace = line.extraSpace
    simple = (
        line.lineBreak or (-1e-8 < extraSpace <= 1e-8) or (last and extraSpace > -1e-8)
    )
    if not simple:
        nSpaces = (
            line.wordCount
...
def _justifyDrawParaLineXRTL(tx, offset, line, last=0):
    return (_rightDrawParaLineX if last else _justifyDrawParaLineX)(
        tx, offset, line, last
    )
    ...
    def shyphenate(self, newWidth, maxWidth):
        ...
        R = (
            ...
            + (
                []
                if not right
                else [(f, _SHYIndexedStr(right, [_ - x for _ in X[j + 1 :]]))]
            )
            + self[i1 + 1 :]
        )
...
def _reconstructSplitFrags(f):
    return (
        (_SHYWordHS if isinstance(f, _HSFrag) else _SHYWord)
        if isinstance(f, _SHYWord)
        else (
            (_SplitFragLL if isinstance(f, _SplitFragLL) else _HSFrag)
                ...
                if isinstance(f, _SplitFrag):
                    ...
                    if not W:
                        ...
                        if isinstance(lf, _SplitFragHY):
                            sty, t = W[-1]
                            Wlen -= (
                                stringWidth(t[-1], sty.fontName, sty.fontSize) + 1e-8
                            )
        ...
        for f in frags:
            ...
            if text != "":
                ...
                for w in S[:-1]:
                    ...
                    aR(
                        _SHYWordHS(W)
                        if shyIndices or isinstance(W, _SHYWord)
                        else _HSFrag(W)
                    )
                    W = []
                    shyIndices = False
...
uri_pat = re.compile(
    "(^(?:[a-z0-9\\.\\-\\+]*)://)(?:\\S+(?::\\S*)?@)?(?:(?:25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)){3}|\\[[0-9a-f:\\.]+\\]|([a-z\xa1-\uffff0-9](?:[a-z\xa1-\uffff0-9-]{0,61}[a-z\xa1-\uffff0-9])?(?:\\.(?!-)[a-z\xa1-\uffff0-9-]{1,63}(?<!-))*\\.(?!-)(?:[a-z\xa1-\uffff-]{2,63}|xn--[a-z0-9]{1,59})(?<!-)\\.?|localhost))(?::\\d{2,5})?(?:[/?#][^\\s]*)?\\Z",
    re.I,
)
...
def _uri_split_pairs(uri):
    ...
    slash = (
        "\\" if not scheme and "/" not in uri else "/"  # might be a microsoft pattern
    )
    R = ([(scheme, uri)] if scheme and uri else []) + list(
        _slash_parts(uri, scheme, slash)
    )
    R.reverse()
    return R
...
def _hyGenPair(
    ...
    if hyphenator and (
        _hy_letters_pat.match(s) or (_hy_shy_letters_pat.match(s) and "" not in H)
    ):
...
def _hyGenFragsPair(
    hyphenator, FW, newWidth, maxWidth, uriWasteReduce, embeddedHyphenation, hymwl
):
...
def _hyphenateFragWord(
    ...
    if len(FW) == 2:
        ...
        if R:
            ...
            return [
                (_SplitFragHY if jc else _SplitFragH)([hw + hylen, (f, h + jc)]),
                (_SplitFragHS if isinstance(FW, _HSFrag) else _SplitFrag)([tw, (f, t)]),
            ]
    else:
        R = _hyGenFragsPair(
            hyphenator,
        ...
        if R:
            jc, h, t = R
            return [
                (_SplitFragHY if jc else _SplitFragH)(h),
                (_SplitFragHS if isinstance(FW, _HSFrag) else _SplitFrag)(t),
            ]
...
def _split_blParaSimple(blPara, start, stop):
    ...
    if isinstance(f.words[-1], _SplitWordHY):
        f.words[-1].__class__ = (
            _SHYSplit if isinstance(f.words[-1], _SHYStr) else _SplitWordLL
        )
    return [f]
...
def _drawBullet(canvas, offset, cur_y, bulletText, style, rtl):
    ...
    if rtl or style.bulletAnchor != "start":
        ...
        if isStr(bulletText):
            ...
            if q:
                bulletWidth += 0.5 * stringWidth(
                    decimalSymbol, style.bulletFontName, style.bulletFontSize
                )
        else:
            ...
            for f in bulletText:
                ...
                if q:
                    t = t[: t.index(decimalSymbol)]
                    bulletWidth += 0.5 * stringWidth(
                        decimalSymbol, f.fontName, f.fontSize
                    )
...
def _do_under_line(i, x1, ws, tx, us_lines):
    ...
    x2 = (
        x1
        + tx._canvas.stringWidth(" ".join(tx.XtraState.lines[i][1]), tx._fontname, fs)
        + ws
    )
    for n, k, c, w, o, r, m, g in us_lines:
        ...
        y = (
            y0
            + r
            + _usConv(("-0.125*L" if underline else "0.25*L") if o == "" else o, values)
        )
        if not c:
            c = tc
        while m > 0:
...
def _do_post_text(tx):
    ...
    if xs.us_lines:
        ...
        for (((n, k, c, w, o, r, m, g), fs, tc, x1), fsmax), x2 in sorted(
            xs.us_lines.values()
        ):
            underline = k == "underline"
            values["f"] = fs
            ...
            y = (
                ...
                + r
                + _usConv(
                    o if o != "" else ("-0.125*L" if underline else "0.25*L"), values
                )
            )
...
class Paragraph(Flowable):
    ...
    def minWidth(self):
        ...
        if nFrags == 1 and not _processed_frags(frags):
            ...
            return max(
                stringWidth(w, fN, fS)
                for w in (split(f.text, " ") if hasattr(f, "text") else f.words)
            )
        else:
    ...
    def _split_blParaProcessed(self, blPara, start, stop):
        ...
        if isinstance(F[-1], _SplitFragHY):
            F[-1].__class__ = (
                _SHYWordHS if isinstance(F[-1], _SHYWord) else _SplitFragLL
            )
        return F
    ...
    def _get_split_blParaFunc(self):
        return (
            _split_blParaSimple
            if self.blPara.kind == 0
            else (
                _split_blParaHard
                if not _processed_frags(self.frags)
    ...
    def split(self, availWidth, availHeight):
        ...
        if style.endDots:
            ...
            style1 = style
        P1 = self.__class__(
            None, style1, bulletText=self.bulletText, frags=func(blPara, 0, s)
        )
        ...
        P1._JustifyLast = not (
            isinstance(blPara.lines[s - 1], FragLine)
    ...
    def breakLines(self, width):
        ...
        if hyphenator:
            if isStr(hyphenator):
                ...
                raise ValueError(
                    "hyphenator should be a language spec or a callable unicode -->  pairs not %r"
                    % hyphenator
                )
        else:
            hyphenator = None
        ...
        if nFrags == 1 and not (
            style.endDots
            or hasattr(frags[0], "cbDefn")
            or hasattr(frags[0], "backColor")
            ...
            while words:
                ...
                if newWidth > limWidth and not (
                    isinstance(word, _SplitWordH) or forcedSplit
                ):
                    if isinstance(word, _SHYStr):
                        ...
                        if hsw:
                            ...
                            if hsw:
                                ...
                                newWidth = currentWidth
                                word = None
                    if splitLongWords and not (
                        isinstance(word, _SplitWord) or forcedSplit
                    ):
                        nmw = min(lineno, maxlineno)
                        if wordWidth > max(maxWidths[nmw : nmw + 1]):
                            # a long word
                            words[0:0] = _splitWord(
                                word,
            ...
            while _words:
                ...
                if spaceShrinkage:
                    ...
                    for wi in words:
                        if wi._fkind == _FK_TEXT:
                            ns = wi.text.count(" ")
                            if ns:
                                spaceShrink += ns * stringWidth(
                                    " ", wi.fontName, wi.fontSize
                                )
                ...
                if (
                    not lineBreak
                    and newWidth > limWidth
                    and not isinstance(w, _SplitFragH)
                    ...
                    if isinstance(w, _SHYWord):
                        ...
                        if hsw:
                            ...
                            _words.insert(
                                1,
                                _InjectedFrag(
                                    [0, (f.clone(_fkind=_FK_BREAK, text=""), "")]
                                ),
                            )
                            ...
                            len(FW) > 1
                        ):  # only if we are not the first word on the line
                            nMW = maxWidths[
                                min(maxlineno, lineno)
                            ]  # next maxWidth or current one
                            if hyphenation2 or w._fsww + 1e-8 <= nMW:
                                ...
                                if hsw:
                                    ...
                                    FW.pop(
                                        -1
                                    )  # remove this as we are doing this one again
                                    continue
                        # else: try to split an overlong word
                    elif attemptHyphenation:
                        hyOk = not getattr(f, "nobr", False)
                        hsw = _hyphenateFragWord(
                            hyphenator if hyOk else None,
                            w,
                            newWidth,
                            limWidth,
                    ...
                    if splitLongWords and not isinstance(w, _SplitFrag):
                        ...
                        if wordWidth > max(maxWidths[nmw : nmw + 1]):
                            # a long word
                            _words[0:0] = _splitFragWord(
                                w,
                                maxWidth - spaceWidth - currentWidth,
                                maxWidths,
                                lineno,
                            )
                            _words.insert(
                ...
                if not endLine:
                    ...
                    if calcBounds:
                        if f._fkind == _FK_IMG:
                            descent, ascent = imgVRange(
                                imgNormV(f.cbDefn.height, fontSize),
                                f.cbDefn.valign,
                                fontSize,
                            )
                        else:
                    ...
                    spaceWidth = (
                        stringWidth(" ", fontName, fontSize)
                        if isinstance(w, _HSFrag)
                        else 0
                    )  # of the space following this word
                    ...
                    for i in w[2:]:
                        ...
                        if calcBounds:
                            if g._fkind == _FK_IMG:
                                descent, ascent = imgVRange(
                                    imgNormV(g.cbDefn.height, fontSize),
                                    g.cbDefn.valign,
                                    fontSize,
                                )
                            else:
                    ...
                    if calcBounds:
                        if g._fkind == _FK_IMG:
                            ...
                            maxAscent, minDescent = getAscentDescent(
                                g.fontName, maxSize
                            )
                    else:
    ...
    def drawPara(self, debug=0):
        ...
        if nLines > 0:
            ...
            if blPara.kind == 0:
                ...
                if bulletText:
                    offset = _drawBullet(
                        canvas, offset, cur_y, bulletText, style, rtl=bRTL
                    )
            ...
            if full:
                ...
                aR(
                    ...
                    % (
                        ...
                        ", ".join(
                            (
                                "(%s,%r)" % (fx[0].__class__.__name__, fx[1])
                                for fx in f[1:]
                            )
                        ),
                        i,
...
if __name__ == "__main__":  # NORUNTESTS
    ...
    def dumpParagraphFrags(P):
        print(
            "dumpParagraphFrags(<Paragraph @ %d>) minWidth() = %.2f"
            % (id(P), P.minWidth())
        )
        frags = P.frags
        n = len(frags)
        for l in range(n):
            print(
                ...
                % (
                    ...
                    " ".join(
                        [
                            "%s=%s" % (k, getattr(frags[l], k))
                            for k in frags[l].__dict__
                            if k != text
                        ]
                    ),
                )
    ...
    def dumpProcessedFrags(P, label="processed_frags"):
        if isinstance(P2.frags[0], list):
            ...
            for _ in P2.frags:
                _S(_showWord(_))
            print(
                "from reportlab.platypus.paragraph import _HSFrag, _SplitFragHS, _SplitFragHY, _SplitFrag, _getFragWords\nfrom reportlab.platypus.paraparser import ParaFrag\nfrom reportlab.lib.colors import Color"
            )
...
It should be a normal<onDraw name="_indexAdd" label="normal"/> paragraph, set in Courier 12 pt."""
        P = Paragraph(
            text, style=ParagraphStyle("style1", fontName="Courier", fontSize=10)
        )
        dumpParagraphFrags(P)
