...
class MultiCol(_Container, _FindSplitterMixin, Flowable):
    def __init__(
        self, contents, widths, minHeightNeeded=36, spaceBefore=None, spaceAfter=None
    ):
        if len(contents) != len(widths):
            raise ValueError(
                "%r len(contents)=%d not the same as len(widths)=%d"
                % (self, len(contents), len(widths))
            )
        ...
        for i, w in enumerate(widths):
            if isinstance(w, strTypes):
                ...
                try:
                    ...
                    raise ValueError(
                        "%s: nWidths failed with value %r" % (self, widths[i])
                    )
                if pc:
                    w = w * 0.01 * aW
    ...
    def split(self, aW, aH):
        ...
        for faW, F in zip(widths, self.contents):
            if not F:
                ...
                fW, fH0, S0, S1 = self._findSplit(
                    canv, faW, aH, content=F, paraFix=False
                )
                if S0 is F:
                    return []  # we failed to find a split
            S[0].append(S0)
            S[1].append(S1)
