...
class RadioGroup(PDFObject):
    ...
    def format(self, doc):
        ...
        d = len(kids)
        if d < 2:
            raise ValueError("RadioGroup:%s has %d < 2 RadioBoxes" % (self.T, d))
...
class AcroForm(PDFObject):
    ...
    def zdMark(self, c, size, ds, iFontName):
        ...
        dy = 0
        return (
            "BT %(iFontName)s %(fs)s Tf %(dx)s %(dy)s Td %(fs)s TL (%(c)s) Tj ET"
            % vars()
        )
    ...
    def stdColors(t, b, f):
        if (
            isinstance(f, CMYKColor)
            or isinstance(t, CMYKColor)
            or isinstance(b, CMYKColor)
        ):
            return (
                t or CMYKColor(0, 0, 0, 0.9),
                b or CMYKColor(0, 0, 0, 0.9),
                f or CMYKColor(0.12, 0.157, 0, 0),
            )
        else:
            return (
                t or Color(0.1, 0.1, 0.1),
                b or Color(0.1, 0.1, 0.1),
                f or Color(0.8, 0.843, 1),
            )
    ...
    def radio(
        ...
        if name not in self._radios:
            ...
            if fieldFlags != group.Ff:
                raise ValueError(
                    "radio.%s.%s created with different flags" % (name, value)
                )
        if not value:
    ...
    def txAP(
        ...
        if opaqueColor(borderColor) and borderWidth:
            ...
            if borderStyle == "underlined":
                stream(
                    "%(streamStroke)s %(borderWidth)s w 0 %(hbw)s m %(width)s %(hbw)s l s"
                )
            elif borderStyle in ("dashed", "inset", "bevelled", "solid"):
                if borderStyle == "dashed":
                    dash = "\n[%s ] 0 d\n" % fp_str(dashLen)
        ...
        return self.makeStream(
            ...
            height,
            stream,
            Resources=PDFFromString(
                "<< /ProcSet [/PDF /Text] /Font %(rFontName)s >>" % vars()
            ),
        )
    ...
    def makeFont(self, fontName):
        ...
        if fontName not in self.formFontNames:
            raise ValueError(
                "form font name, %r, is not one of the standard 14 fonts" % fontName
            )
        fn = self.formFontNames[fontName]
        ref = self.getRefStr(
            PDFFromString(
                "<< /BaseFont /%s /Subtype /Type1 /Name /%s /Type /Font /Encoding %s >>"
    ...
    def _textfield(
        ...
        TF = dict(
            ...
            # H=PDFName('N'),
            DA=PDFString(
                "/%s %d Tf %s" % (iFontName, fontSize, self.streamFillColor(textColor))
            ),
        )
        if Opt:
            TF["Opt"] = Opt
