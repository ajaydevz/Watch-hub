...
def textFieldAbsolute(
    canvas, title, x, y, width, height, value="", maxlen=1000000, multiline=0
):
    ...
    return theform.textField(
        canvas, title, x, y, x + width, y + height, value, maxlen, multiline
    )


def textFieldRelative(
    canvas, title, xR, yR, width, height, value="", maxlen=1000000, multiline=0
):
    ...
    return textFieldAbsolute(
        canvas, title, xA, yA, width, height, value, maxlen, multiline
    )
...
class AcroForm(PDFObject):
    ...
    def buttonField(
        self, canvas, title, value, xmin, ymin, width=16.7704, height=14.907
    ):
        # determine the page ref
        doc = canvas._doc
        page = doc.thisPageRef()
...
def FormResources():
    return PDFPattern(
        ...
        Encoding=PDFPattern(
            EncodingPattern, PDFDocEncoding=PDFPattern(PDFDocEncodingPattern)
        ),
...
def SelectField(
    ...
    if value not in options:
        raise ValueError(
            "value %s must be one of options %s" % (repr(value), repr(options))
        )
    fontname = FORMFONTNAMES[font]
...
def ButtonField(title, value, xmin, ymin, page, width=16.7704, height=14.907):
    ...
    return PDFPattern(
        ...
        APDOff=ButtonStream(
            "0.749 g 0 0 %(width)s %(height)s re f\r\n" % vars(),
            width=width,
            height=height,
        ),
        APDYes=ButtonStream(
