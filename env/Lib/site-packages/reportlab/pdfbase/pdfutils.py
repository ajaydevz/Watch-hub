...
def makeA85Image(filename, IMG=None, detectJpeg=False):
    ...
    append("BI")
    append(
        "/W %s /H %s /BPC 8 /CS /%s /F [/A85 /Fl]"
        % (imgwidth, imgheight, _mode2cs[img.mode])
    )
    append("ID")
    # use a flate filter and Ascii Base 85
    assert (
        len(raw) == imgwidth * imgheight * _mode2bpp[img.mode]
...
def readJPEGInfo(image):
    ...
    noParamMarkers = [0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8, 0x01]
...
class _fusc:
    ...
    def encrypt(self, s):
        return self.__rotate(
            asciiBase85Encode("".join(map(chr, self.__fusc(list(map(ord, s)))))),
            self._n,
        )
    ...
    def decrypt(self, s):
        return "".join(
            map(
                chr,
                self.__fusc(
                    list(map(ord, asciiBase85Decode(self.__rotate(s, -self._n))))
                ),
            )
        )
    ...
    def __fusc(self, s):
        ...
        return list(
            map(
                lambda x, y: x ^ y,
                s,
                list(map(ord, ((int(slen / self._klen) + 1) * self._k)[:slen])),
            )
        )
