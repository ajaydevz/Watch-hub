...
def format(element, document, toplevel=0):
    ...
    if isinstance(element, PDFObject):
        if not toplevel and hasattr(element, __RefOnly__):
            ...
            if (
                not rl_config.invariant
                and rl_config.pdfComments
                and hasattr(element, __Comment__)
            ):
...
class PDFDocument(PDFObject):
    ...
    def __init__(
        ...
        self.outline = self.Outlines = cat.Outlines = (
            PDFOutlines0() if dummyoutline else PDFOutlines()
        )
        self.info = PDFInfo()
    ...
    def updateSignature(self, thing):
        ...
        if self._ID:
            return  # but not if its used already!
    ...
    def SaveToFile(self, filename, canvas):
        if getattr(self, "_savedToFile", False):
            raise RuntimeError(
                "class %s instances can only be saved once" % self.__class__.__name__
            )
        ...
        if hasattr(getattr(filename, "write", None), "__call__"):
            ...
            if isinstance(filename, int):
                ...
                filename
            ):  # try to fix bug reported by Robert Schroll <rschroll at gmail.com>
        ...
        if psfontname in fm:
            ...
            try:
                ...
                if fontObj._dynamicFont:
                    raise PDFError(
                        "getInternalFontName(%s) called for a dynamic font"
                        % repr(psfontname)
                    )
    ...
    def addColor(self, cmyk):
        ...
        if name not in self.idToObject:
            sep = PDFSeparationCMYKColor(
                cmyk
            ).value()  # PDFArray([/Separation /name /DeviceCMYK tint_tf])
    ...
    def format(self):
        ...
        if counter - 1 != lno:
            raise ValueError(
                "counter %s doesn't match number to id dictionary %s" % (counter, lno)
            )
        # now add the xref
    ...
    def Reference(self, obj, name=None):
        ...
        if hasattr(obj, __InternalName__):
            ...
            if name is not None and name != intname:
                raise ValueError(
                    "attempt to reregister object %s with new name %s"
                    % (repr(intname), repr(name))
                )
            if intname not in idToObject:
                raise ValueError(
                    "object of type %s named as %s, but not registered"
                    % (type(obj), ascii(intname))
                )
...
class PDFDictionary(PDFObject):
    ...
    def format(self, document, IND=b"\n "):
        ...
        if not isinstance(dict, OrderedDict):
            keys.sort()
        L = [
            (format(PDFName(k), document) + b" " + format(dict[k], document))
            for k in keys
        ]
...
class ViewerPreferencesPDFDictionary(CheckedPDFDictionary):
    validate = dict(
        ...
        NonFullScreenPageMode=checkPDFNames(
            *"UseNone UseOutlines UseThumbs UseOC".split()
        ),
...
class PDFIndirectObject(PDFObject):
    ...
    def format(self, document):
        ...
        fcontent = format(
            self.content, document, toplevel=1
        )  # yes this is at top level
        return (
            pdfdocEnc("%s %s obj\n" % (n, v))
            + fcontent
            + (b"" if fcontent.endswith(b"\n") else b"\n")
            + b"endobj\n"
        )
...
class PDFObjectReference(PDFObject):
    ...
    def format(self, document):
        try:
            ...
            raise KeyError(
                "forward reference to %s not resolved upon final formatting"
                % repr(self.name)
            )
...
class PDFCrossReferenceSubsection(PDFObject):
    ...
    def format(self, document):
        ...
        for id in idsequence:
            ...
            if num in taken:
                raise ValueError(
                    "object number collision %s %s %s"
                    % (num, repr(id), repr(taken[id]))
                )
            if num > lastentrynumber or num < firstentrynumber:
                raise ValueError(
                    "object number %s not in range %s..%s"
                    % (num, firstentrynumber, lastentrynumber)
                )
...
class PDFPage(PDFCatalog):
    ...
    def check_format(self, document):
        ...
        self.MediaBox = self.MediaBox or PDFArray(
            self.Rotate in (90, 270)
            and [0, 0, self.pageheight, self.pagewidth]
            or [0, 0, self.pagewidth, self.pageheight]
        )
        if not self.Annots:
            self.Annots = None
        else:
        ...
        if not self.Contents:
            ...
            if not stream:
                ...
                if self.compression:
                    S.filters = (
                        rl_config.useA85
                        and [PDFBase85Encode, PDFZCompress]
                        or [PDFZCompress]
                    )
                S.content = stream
    ...
    def format(self, document):
        try:
            ...
            annotateException(
                ...
                % list(set([x for x in tmp if tmp.count(x) > 1]))
            )
        labels = []
...
class PDFOutlines(PDFObject):
    ...
    def addOutlineEntry(self, destinationname, level=0, title=None, closed=None):
        ...
        if level > currentlevel:
            if level > currentlevel + 1:
                raise ValueError(
                    "can't jump from outline level %s to level %s, need intermediates (destinationname=%r, title=%r)"
                    % (currentlevel, level, destinationname, title)
                )
        ...
        while level < currentlevel:
            ...
            if isinstance(lastinprevious, tuple):
                ...
                raise ValueError(
                    "cannot reset existing sections: " + repr(lastinprevious)
                )
            else:
                name = lastinprevious
                sectionlist = current
    ...
    def translateNames(self, canvas, object):
        ...
        raise TypeError(
            "in outline, destination name must be string: got a %s" % type(object)
        )
    ...
    def prepare(self, document, canvas):
        ...
        (self.first, self.last) = self.maketree(
            document, self.mydestinations, toplevel=1
        )
        self.ready = 1
        ...
        for index in range(nelts):
            ...
            if isinstance(elt, dict):
                ...
                try:
                    ...
                    raise ValueError(
                        "destination tree elt tuple should have two elts, got %s"
                        % len(elt)
                    )
                eltobj.Count = count(subsections, closedict)
                (eltobj.First, eltobj.Last) = self.maketree(
                    document, subsections, eltref
                )
            else:
                raise ValueError(
                    "destination tree elt should be dict or tuple, got %s" % type(elt)
                )
            try:
                [(Title, Dest)] = list(leafdict.items())
            except:
                raise ValueError(
                    "bad outline leaf dictionary, should have one entry " + bytestr(elt)
                )
...
class PDFInfo(PDFObject):
    ...
    def format(self, document):
        ...
        D["ModDate"] = D["CreationDate"] = PDFDate(
            ts=document._timeStamp, dateFormatter=self._dateFormatter
        )
...
class Annotation(PDFObject):
    ...
    def AnnotationDict(self, **kw):
        ...
        for name in d.keys():
            if name not in permitted:
                raise ValueError(
                    "%s bad annotation dictionary name %s"
    ...
    def Dict(self):
        raise ValueError(
            "DictString undefined for virtual superclass Annotation, must overload"
        )
        # but usually
...
class TextAnnotation(HighlightAnnotation):
    permitted = HighlightAnnotation.permitted + ("Open", "Name")

    def __init__(self, Rect, Contents, **kw):
        HighlightAnnotation.__init__(
            self,
            Rect,
            Contents,
...
class Destination(PDFObject):
    ...
    def format(self, document):
        ...
        if f is None:
            raise ValueError(
                "format not resolved, probably missing URL scheme or undefined destination target for '%s'"
                % self.name
            )
        p = self.page
        if p is None:
            raise ValueError(
                "Page not bound, probably missing URL scheme or undefined destination target for '%s'"
                % self.name
            )
        f.page = p
...
class PDFType1Font(PDFObject):
    ...
    local_attributes = (
        "FirstChar LastChar Widths Encoding ToUnicode FontDescriptor".split()
    )
...
class PDFFormXObject(PDFObject):
    ...
    def format(self, document):
        self.BBox = self.BBox or PDFArray(
            [self.lowerx, self.lowery, self.upperx, self.uppery]
        )
        ...
        if not self.Resources:
            ...
            self.Resources = resources
        if self.compression:
            self.Contents.filters = (
                rl_config.useA85 and [PDFBase85Encode, PDFZCompress] or [PDFZCompress]
            )
...
class PDFImageXObject(PDFObject):
    ...
    def _checkTransparency(self, im):
        if self.mask == "auto":
            if im._dataA:
                self.mask = None
                self._smask = PDFImageXObject(
                    _digester(im._dataA.getRGBData()), im._dataA, mask=None
                )
...
class PDFSeparationCMYKColor:
    def __init__(self, cmyk):
        ...
        if not isinstance(cmyk, CMYKColor):
            ...
            raise ValueError(
                "%s needs a CMYKColor argument with a spotName"
...
class PDFFunction(PDFObject):
    ...
    def Dict(self, document):
        raise ValueError(
            "Dict undefined for virtual superclass PDFShading, must overload"
        )
        # but usually
