            ...
            if glyphName is not None:
                try:
                    ...
                    if reportlab.rl_config.warnOnMissingFontGlyphs:
                        print(
                            'typeface "%s" does not have a glyph "%s", bad font!'
...
def _pfbCheck(p, d, m, fn):
    if chr(d[p]) != PFB_MARKER or chr(d[p + 1]) != m:
        raise ValueError(
            ...
            % (fn, ord(PFB_MARKER), ord(m), p, d[p], d[p + 1])
        )
    if m == PFB_EOF:
        return
    p = p + 2
    ...
    if p + l > len(d):
        raise ValueError(
            "Bad pfb file'%s' needed %d+%d bytes have only %d!" % (fn, p, l, len(d))
        )
    return p, p + l
        ...
        if fontName not in _fonts:
            if faceName in _dynFaceNames:
                ...
                if not ofont._dynamicFont:
                    raise ValueError(
                        "Attempt to register fonts %r %r for face %r"
                        % (ofont, font, faceName)
                    )
                else:
    ...
    try:
        ...
        if faceName in standardFonts:
            ...
            (
                ...
                face.bold,
                face.italic,
            ) = _fontdata.standardFontAttributes[faceName]
...
def findFontAndRegister(fontName):
    ...
    assert type(fontName) is str, "fontName=%s is not required type str" % ascii(
        fontName
    )
...
def testStringWidthAlgorithms():
    ...
    words = rawdata.split()
    print(
        "test %d shorter strings (average length %0.2f chars)..."
