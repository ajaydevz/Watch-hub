...
class TTFontParser:
    ...
    def readTTCHeader(self):
        ...
        if self.ttcVersion not in self.ttcVersions:
            raise TTFError(
                '"%s" is not a %s file: can\'t read version 0x%8.8x'
                % (self.filename, self.fileKind, self.ttcVersion)
            )
    ...
    def getSubfont(self, subfontIndex):
        if self.fileKind != "TTC":
            raise TTFError(
                '"%s" is not a TTC file: use this method'
                % (self.filename, self.fileKind)
            )
        try:
            pos = self.subfontOffsets[subfontIndex]
        except IndexError:
            raise TTFError(
                'TTC file "%s": bad subfontIndex %s not in [0,%d]'
    ...
    def readTableDirectory(self):
        try:
            ...
            raise TTFError(
                'Corrupt %s file "%s" cannot read Table Directory'
                % (self.fileKind, self.filename)
            )
        if self.validate:
            self.checksumTables()
    ...
    def readHeader(self):
        ...
        try:
            ...
            raise TTFError(
                '"%s" is not a %s file: can\'t read version'
                % (self.filename, self.fileKind)
            )

        if version == 0x4F54544F:
            raise TTFError(
                '%s file "%s": postscript outlines are not supported'
                % (self.fileKind, self.filename)
            )
        ...
        for t in self.tables:
            ...
            if xchecksum != checksum:
                raise TTFError(
                    ...
                    % (self.filename, hex32(checksum), t["tag"], hex32(xchecksum))
                )
...
class TTFontFile(TTFontParser):
    ...
    def __init__(self, file, charInfo=1, validate=0, subfontIndex=0):
        ...
        if isStr(subfontIndex):  # bytes or unicode
            ...
            while True:
                ...
                if (
                    isBytes(subfontIndex)
                    and subfontIndex == self.name
                    or subfontIndex == self.name.ustr
                ):  # we found it
                    return
                if not sfi:
                    __dict__.update(
                        dict(_ttf_data=self._ttf_data, filename=self.filename)
                    )
                sfi += 1
                if sfi >= numSubfonts:
                    raise ValueError(
                        "cannot find %r subfont %r" % (self.filename, subfontIndex)
                    )
                self.__dict__.clear()
                self.__dict__.update(__dict__)
        else:
            TTFontParser.__init__(
                self, file, validate=validate, subfontIndex=subfontIndex
            )
            self.extractInfo(charInfo)
    ...
    def extractInfo(self, charInfo=1):
        ...
        for i in range(numRecords):
            ...
            if (
                ...
                try:
                    ...
                    N = TTFNameBytes(
                        self.get_chunk(string_data_offset + offset, length), "utf_16_be"
                    )
                finally:
                    self._pos = opos
            elif (
                ...
                N = TTFNameBytes(
                    self.get_chunk(string_data_offset + offset, length), "mac_roman"
                )
        ...
        if not psName:
            if rl_config.autoGenerateTTFMissingTTFName:
                ...
                if not fn:
                    psName = bytestr(
                        "_RL_%s_%s_TTF" % (time.time(), self.__class__._agfnc)
                    )
        ...
        psName = psName.__class__(
            psName.replace(b" ", b"-")
        )  # Dinu Gherman's fix for font names with spaces

        for c in psName:
            if char2int(c) > 126 or c in b" [](){}<>/%":
                raise TTFError(
                    "psName=%r contains invalid character %s" % (psName, ascii(c))
                )
        self.name = psName
        ...
        if "OS/2" in self.table:
            ...
            if fsType == 0x0002 or (fsType & 0x0300):
                subsettingAllowed = (
                    os.path.basename(self.filename) not in rl_config.allowTTFSubsetting
                )
        ...
        if metricDataFormat != 0:
            raise TTFError(
                "Unknown horizontal metric data format (%d)" % metricDataFormat
            )
            ...
            for unichar in range(256):
                if T[unichar] == 0:
                    ...
                    if last != -1:
                        ...
                        unichar < SH[0].firstCode
                        or unichar >= SH[0].firstCode + SH[0].entryCount
                        or SH[0].idRangeOffset + (unichar - SH[0].firstCode)
                        >= entryCount
                    ):
                        glyph = 0
                    else:
    ...
    def makeSubset(self, subset):
        ...
        cmap = [
            0,
            1,  # version, number of tables
            1,
            0,
            0,
            12,  # platform, encoding, offset (hi,lo)
            6,
            length,
        ...
        for n in range(numGlyphs):
            ...
            if glyphLen > 2 and unpack(">h", data[:2])[0] < 0:
                ...
                while flags & GF_MORE_COMPONENTS:
                    ...
                    glyphIdx = unpack(">H", data[pos_in_glyph + 2 : pos_in_glyph + 4])[
                        0
                    ]
...
class TTFontFace(TTFontFile, pdfmetrics.TypeFace):
    ...
    def __init__(self, filename, validate=0, subfontIndex=0):
        ...
        TTFontFile.__init__(
            self, filename, validate=validate, subfontIndex=subfontIndex
        )
    ...
    def addSubsetObjects(self, doc, fontname, subset):
        ...
        fontFileRef = doc.Reference(
            fontFile, "fontFile:%s(%s)" % (self.filename, fontname)
        )
...
class TTFont:
    ...
    def getSubsetInternalName(self, subset, doc):
        ...
        if subset < 0 or subset >= len(state.subsets):
            raise IndexError(
                "Subset %d does not exist in font %s" % (subset, self.fontName)
            )
    ...
    def addObjects(self, doc):
        ...
        for n, subset in enumerate(state.subsets):
            ...
            baseFontName = (
                b"".join((SUBSETN(n), b"+", self.face.name, self.face.subfontNameX))
            ).decode("pdfdoc")
            ...
            pdfFont.ToUnicode = doc.Reference(
                cmapStream, "toUnicodeCMap:" + baseFontName
            )
