...
canvas_basefontname = (
    "Helvetica"  # this is used to initialize the canvas; if you override to make
)
...
ignoreContainerActions = (
    1  # if true then action flowables in flowable _Containers will be ignored
)
...
listWrapOnFakeWidth = (
    1  # set to 0/False to force platypus.flowables._listWrapOn to report correct widths
)
...
underlineOffset = (
    "-0.125*F"  # fraction of fontSize from baseline to draw underlines at.
)
...
errorOnDuplicatePageLabelPage = (
    0  # if True will cause repeated PageLabel page numbers to raise an error.
)
autoGenerateMissingTTFName = (
    0  # if true we try to auto generate any missing TTF font name
)
...
trustedSchemes = [
    ...
    "rml",
    "data",
    "https",  # these url schemes are trusted
    "http",
    "ftp",
]
...
toColorCanUse = (
    "rl_extended_literal_eval"  # change to None or 'rl_safe_eval' depending on trust
)
